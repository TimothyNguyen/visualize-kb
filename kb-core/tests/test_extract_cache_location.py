...
kb-core-out/cache/ inside it. It now defaults to the current working directory;
...
import kb_core.cache as cache
from kb_core.cache import load_cached, file_hash
...
def test_default_cache_lands_in_cwd_not_source_tree(tmp_path, monkeypatch):
    ...
    # the key-root, leaving a stray kb_core-out/ in a writable corpus, #1774).
    assert not (corpus / "kb-core-out").exists(), (
        "cache/stat-index written into the analyzed source tree (#1774)"
    )
    assert (work / "kb-core-out" / "cache").is_dir(), "cache should land under CWD"
...
def test_default_cache_does_not_leave_stat_index_in_source_tree(tmp_path, monkeypatch):
    ...
    assert not (corpus / "kb-core-out").exists(), "stat-index leaked into the corpus"
    assert (work / "kb-core-out" / "cache" / "stat-index.json").exists(), (
...
def test_explicit_cache_root_still_wins(tmp_path, monkeypatch):
    ...
    assert (out / "kb-core-out" / "cache").is_dir()
    assert not (corpus / "kb-core-out").exists()
    assert not (work / "kb-core-out").exists()
