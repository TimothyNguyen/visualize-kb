"""`kb-core extract --code-only` indexes code without an LLM key (#1734).
...
def _run(repo: Path, *extra: str):
    ...
    env["KB_CORE_OUT"] = str(repo / "kb-core-out")
    return subprocess.run(
        [PYTHON, "-m", "kb-core", "extract", ".", *extra],
...
def test_extract_usage_advertises_code_only(tmp_path):
    ...
    by triggering the no-key error. `kb-core extract` with no path prints usage."""
    r = subprocess.run(
        [PYTHON, "-m", "kb-core", "extract"],
...
def _run_relative_out(repo: Path, *extra: str):
    """Like _run but with a RELATIVE KB_CORE_OUT so --out/--output controls the
    parent dir (an absolute KB_CORE_OUT would override the flag)."""
    env = {k: v for k, v in os.environ.items() if k not in _KEY_VARS}
    env["KB_CORE_OUT"] = "kb-core-out"
    return subprocess.run(
        [PYTHON, "-m", "kb-core", "extract", ".", *extra],
        cwd=repo, capture_output=True, text=True, env=env,
    )
...
def test_output_flag_is_alias_of_out(tmp_path):
    ...
    to the default `<path>/kb-core-out/`). It is now an alias of `--out`."""
    repo = tmp_path / "repo"
    repo.mkdir()
    (repo / "app.py").write_text("def hello():\n    return 1\n")
    ...
    assert (custom / "kb-core-out" / "graph.json").exists(), "--output was ignored (#2004)"
    assert not (repo / "kb-core-out").exists(), "output must not go to the default dir"
...
def test_output_flag_inline_form(tmp_path):
    ...
    assert r.returncode == 0, r.stderr
    assert (custom / "kb-core-out" / "graph.json").exists()


def test_no_gitignore_indexes_vcs_ignored_code_but_keeps_kb_coreignore(tmp_path):
    repo = tmp_path / "repo"
    ...
    (repo / "proj" / ".kb_coreignore").write_text("hidden/\n")
    ...
    graph = json.loads((repo / "kb-core-out" / "graph.json").read_text())
...
def test_no_gitignore_setting_persists_across_flagless_extract(tmp_path):
    ...
    `kb-core extract` must NOT clobber it back to honoring .gitignore (which
    ...
    def _sources():
        g = json.loads((repo / "kb-core-out" / "graph.json").read_text())
...
def test_explicit_exclude_replaces_persisted_setting_with_custom_out(tmp_path):
    ...
    graph_out = out_root / "kb-core-out"
    def _sources():
    ...
    assert json.loads((graph_out / ".kb_core_build.json").read_text()) == {
        "excludes": ["generated"]
    }
