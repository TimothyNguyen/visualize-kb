...
from kb_core.llm import label_communities, generate_community_labels
...
def test_label_communities_happy_path(monkeypatch):
    ...
    monkeypatch.setattr("kb_core.llm._call_llm", fake_call)
...
def test_label_cli_passes_model_override(tmp_path, monkeypatch):
    import kb_core.__main__ as cli
    ...
    monkeypatch.setattr("kb_core.llm.generate_community_labels", fake_generate)
    monkeypatch.setattr("kb_core.export.to_html", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        sys,
        "argv",
        [
...
def test_label_cli_missing_only_preserves_existing_labels(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["kb-core", "label", str(tmp_path), "--missing-only", "--backend", "gemini", "--no-viz"],
    )

    cli.main()

    assert set(captured["communities"]) == {1}
    labels = json.loads((out / ".kb_core_labels.json").read_text(encoding="utf-8"))
    assert labels == {"0": "Order Management", "1": "Payment Flow"}


def test_label_communities_partial_reply_fills_placeholder(monkeypatch):
    G, communities = _graph()
    monkeypatch.setattr("kb_core.llm._call_llm",
...
def test_label_communities_all_batches_fail_raises(monkeypatch):
    ...
    monkeypatch.setattr("kb_core.llm._call_llm", always_fail)
...
def test_label_communities_parallel_matches_sequential(monkeypatch):
    ...
    monkeypatch.setattr("kb_core.llm._label_batch_with_retry", fake_batch)
...
def test_cluster_commands_render_aggregated_html_above_viz_limit(
    ...
    monkeypatch.setattr(
        "kb_core.llm.generate_community_labels",
    ...
    argv = ["kb-core", command, str(tmp_path)]
...
def test_cluster_only_preserves_but_does_not_claim_unusable_aggregate(
    ...
    monkeypatch.setattr(cli, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        importlib.import_module("kb_core.cluster"),
        "cluster",
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["kb-core", "cluster-only", str(tmp_path), "--no-label"],
    )
...
def test_cluster_only_restores_html_after_unexpected_render_failure(
    ...
    monkeypatch.setattr(cli, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        importlib.import_module("kb_core.export"),
        "to_html",
        fail_render,
    )
    monkeypatch.setattr(
...
def test_cluster_only_marks_html_stale_before_report_generation(
    ...
    monkeypatch.setenv("KB_CORE_VIZ_NODE_LIMIT", "3")
    monkeypatch.setattr(cli, "_check_skill_version", lambda _: None)
    monkeypatch.setattr("kb_core.report.generate", interrupt_report)
    monkeypatch.setattr(
        sys,
        "argv",
        ["kb-core", "cluster-only", str(tmp_path), "--no-label"],
...
def test_cluster_only_refused_graph_write_preserves_existing_stale_marker(
    ...
    monkeypatch.setattr(cli, "_check_skill_version", lambda _: None)
    monkeypatch.setattr("kb_core.export.to_json", lambda *args, **kwargs: False)
    monkeypatch.setattr(
        sys,
        "argv",
        ["kb-core", "cluster-only", str(tmp_path), "--no-label"],
...
def test_cluster_only_no_label_does_not_persist_placeholders(tmp_path, monkeypatch):
    """#2073: --no-label must not write .kb_core_labels.json with 'Community N'
    ...
    monkeypatch.setattr(cli, "_check_skill_version", lambda _: None)
    monkeypatch.setattr("kb_core.export.to_html", lambda *a, **k: None)
    # No-backend fallback shape: returns placeholders, which must not clobber hubs.
    monkeypatch.setattr("kb_core.llm.generate_community_labels",
                        lambda G, comms, **k: ({cid: f"Community {cid}" for cid in comms}, "none"))

    monkeypatch.setattr(sys, "argv", ["kb-core", "cluster-only", str(tmp_path), "--no-label", "--no-viz"])
    cli.main()
    assert not labels_path.exists(), "--no-label persisted a placeholder labels file (#2073)"
    assert not (out / ".kb_core_labels.json.sig").exists()

    # A later normal run generates real labels (no sticky placeholders blocking it).
    monkeypatch.setattr(sys, "argv", ["kb-core", "cluster-only", str(tmp_path), "--no-viz"])
    cli.main()
...
def test_cluster_only_heals_persisted_placeholder_but_reuses_genuine(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kb_core.llm.generate_community_labels", _fail_generate)
...
def test_label_prompt_lines_use_bare_cid_keys():
    ...
    from kb_core.llm import _community_label_lines
...
def test_label_cli_drops_sentinel_and_bare_key_echoes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        sys, "argv",
        ["kb-core", "label", str(tmp_path), "--backend", "gemini", "--no-viz"],
    )
