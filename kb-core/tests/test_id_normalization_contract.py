...
Both callers now delegate to :mod:`kb_core.ids`, so they share one
...
from kb_core.build import _normalize_id
from kb_core.extract import _make_id
from kb_core.ids import make_id, normalize_id
...
def test_both_callers_share_one_implementation():
    ...
    underlying function object as kb_core.ids.normalize_id."""
    # build._normalize_id is imported directly from kb_core.ids.
    ...
    from kb_core.mcp_ingest import _make_id as _mcp_make_id
    from kb_core.symbol_resolution import _bash_make_id
