"""Tests for kb-core devin install / uninstall commands."""
...
def _skill_path_user(tmp_path):
    return tmp_path / ".config" / "devin" / "skills" / "kb-core" / "SKILL.md"


def _skill_path_project(project_dir):
    return project_dir / ".devin" / "skills" / "kb-core" / "SKILL.md"


def _rules_path(project_dir):
    return project_dir / ".windsurf" / "rules" / "kb_core.md"


# ---------------------------------------------------------------------------
# User-scope install (kb_core install --platform devin / kb_core devin install)
# ---------------------------------------------------------------------------

def test_devin_install_user_creates_skill_file(tmp_path):
    """User-scope install copies skill to ~/.config/devin/skills/kb-core/SKILL.md."""
...
def test_devin_skill_file_references_kb_core_query(tmp_path):
...
# ---------------------------------------------------------------------------
# Project-scope install (kb_core devin install --project)
# ---------------------------------------------------------------------------

def test_devin_install_project_creates_skill_file(tmp_path, monkeypatch):
    """Project-scope install copies skill to .devin/skills/kb-core/SKILL.md."""
    from kb_core.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    monkeypatch.chdir(project)
    monkeypatch.setattr(sys, "argv", ["kb-core", "devin", "install", "--project"])
...
def test_devin_install_project_creates_rules_file(tmp_path, monkeypatch):
    """Project-scope install writes .windsurf/rules/kb_core.md."""
    from kb_core.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    ...
    assert rules.exists()
    assert "kb-core" in rules.read_text()
    assert "GRAPH_REPORT.md" in rules.read_text()


def test_devin_rules_content_recommends_kb_core_query(tmp_path):
    """The rules file installed by devin must use query-first policy."""
    from kb_core.__main__ import _devin_rules_install
    _devin_rules_install(tmp_path)
    content = _rules_path(tmp_path).read_text()
    assert "kb-core query" in content
...
def test_devin_uninstall_user_removes_skill_file(tmp_path):
    ...
    from kb_core.__main__ import _remove_skill_file
    with patch("kb_core.__main__.Path.home", return_value=tmp_path):
...
def test_devin_uninstall_user_noop_when_not_installed(tmp_path, capsys):
    ...
    try:
        os.chdir(tmp_path)
        with patch("kb_core.__main__.Path.home", return_value=tmp_path):
            sys.argv = ["kb-core", "devin", "uninstall"]
            main()
    finally:
        os.chdir(old_cwd)
    out = capsys.readouterr().out
...
def test_devin_uninstall_project_removes_skill_file(tmp_path, monkeypatch):
    """Project-scope uninstall removes .devin/skills/kb-core/SKILL.md."""
    from kb_core.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    ...
    with patch("kb_core.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["kb-core", "devin", "install", "--project"])
        main()
        monkeypatch.setattr(sys, "argv", ["kb-core", "devin", "uninstall", "--project"])
        main()
    assert not _skill_path_project(project).exists()


def test_devin_uninstall_project_removes_rules_file(tmp_path, monkeypatch):
    """Project-scope uninstall removes .windsurf/rules/kb_core.md."""
    from kb_core.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
...
def test_devin_rules_uninstall_noop_when_not_installed(tmp_path):
    ...
    from kb_core.__main__ import _devin_rules_uninstall
...
def test_devin_skill_file_exists_in_package():
    ...
    import kb_core
    skill = Path(kb_core.__file__).parent / "skill-devin.md"
...
def test_devin_skill_file_uses_python_c_syntax():
    ...
    All mature kb-core skills use the interpreter-detection pattern
    ``$(cat kb-core-out/.kb_core_python) -c "..."`` rather than bare
    ``python -c "..."`` so they work in pipx / venv environments.
    """
    import kb_core
    skill = (Path(kb_core.__file__).parent / "skill-devin.md").read_text()
    assert '.kb_core_python) -c "' in skill, (
...
def test_devin_platform_skill_destination_user_scope(tmp_path):
    """User-scope destination must be ~/.config/devin/skills/kb-core/SKILL.md."""
    from kb_core.__main__ import _platform_skill_destination
    with patch("kb_core.__main__.Path.home", return_value=tmp_path):
        dst = _platform_skill_destination("devin", project=False)
    assert dst == tmp_path / ".config" / "devin" / "skills" / "kb-core" / "SKILL.md"


def test_devin_in_main_help_text(capsys, monkeypatch):
    """`kb-core --help` must list devin in the platform list and in the per-platform section."""
    from kb_core.__main__ import main
    ...
    assert "|devin)" in captured or "|devin |" in captured or "|devin" in captured, (
        "devin missing from `kb-core --help` platform list"
    )
...
def test_devin_platform_skill_destination_project_scope(tmp_path):
    """Project-scope destination must be <project>/.devin/skills/kb-core/SKILL.md."""
    from kb_core.__main__ import _platform_skill_destination
    dst = _platform_skill_destination("devin", project=True, project_dir=tmp_path)
    assert dst == tmp_path / ".devin" / "skills" / "kb-core" / "SKILL.md"
