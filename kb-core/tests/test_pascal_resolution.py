...
file's own scope. kb_core.pascal_resolution closes that gap as a
...
from kb_core.extract import extract, extract_pascal
...
def test_calls_resolve_across_files_via_inherits_chain(tmp_path):
    # cache_root only controls where kb_core-out/cache/ is written -- it has
...
def test_pascal_resolver_registered():
    from kb_core.resolver_registry import registered_resolvers
