"""`kb-core query` must render every edge between visited nodes (#2323).
...
from kb_core.serve import _bfs, _dfs, _filter_graph_by_context, _query_graph_text
...
def test_query_cli_renders_the_edge_between_two_seeds(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["kb-core", "query", "checkout discounted_total", "--graph", str(graph_path)],
    )
