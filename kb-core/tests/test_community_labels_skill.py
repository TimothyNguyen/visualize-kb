...
2. A template lint over the generated ``kb-core/skill*.md`` bodies (and the
   ...
   ``kb-core-out/graph.json`` with ``community_labels=labels``. This locks the
...
def test_skill_step5_reexports_graph_json_with_curated_labels(path: Path):
    ...
    ``to_json(...kb-core-out/graph.json...)`` call in that block without the
    ...
    for block in post_labels_blocks:
        assert "to_json(G, communities, 'kb-core-out/graph.json', community_labels=labels)" in block, (
            f"{path.name}: the post-labels Step-5 block must re-export "
            f"kb-core-out/graph.json with community_labels=labels (#2490)"
        )
        # No label-less graph.json export may coexist in the post-labels block.
        for line in block.splitlines():
            if "to_json(" in line and "kb-core-out/graph.json" in line:
