...
`kb-core install` hard-exit with "not found in package" for real users.
...
def _skill_bodies() -> list[Path]:
    ...
    one of these). A body missing from the wheel makes `kb-core install
...
def wheel_namelist(tmp_path_factory) -> set[str]:
    ...
    wheels = list(out.glob("kb-core-*.whl"))
