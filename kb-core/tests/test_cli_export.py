"""Integration tests for kb-core export subcommands and CLI commands.
...
def _make_graph(tmp_path: Path) -> Path:
    """Build a minimal graph.json + analysis/labels files in tmp_path/kb-core-out/."""
    out = tmp_path / "kb-core-out"
    out.mkdir()

    extraction = json.loads((FIXTURES / "extraction.json").read_text())
    from kb_core.build import build_from_json
    from kb_core.cluster import cluster, score_all
    from kb_core.analyze import god_nodes, surprising_connections
    ...
    (out / ".kb_core_analysis.json").write_text(json.dumps(analysis))
    (out / ".kb_core_labels.json").write_text(
...
def test_export_html_creates_file(tmp_path):
    ...
    html = tmp_path / "kb-core-out" / "graph.html"
    assert html.exists()
...
def test_export_wiki_creates_articles(tmp_path):
    ...
    assert r.returncode == 0, r.stderr
    wiki = tmp_path / "kb-core-out" / "wiki"
    assert wiki.exists()
...
# ── kb_core export falkordb (cypher) ────────────────────────────────────────
...
def test_query_uses_kb_core_out_env(tmp_path):
    out = _make_graph(tmp_path)
    custom_out = tmp_path / "custom-graph"
    out.rename(custom_out)
    env = os.environ.copy()
    env["KB_CORE_OUT"] = custom_out.name
...
def test_extract_writes_to_kb_core_out_env(tmp_path):
    """#1423: `kb-core extract` honours KB_CORE_OUT for where it WRITES, not only
    where readers look — previously it hardcoded kb-core-out/ and ignored the
    override. Code-only corpus, so no LLM backend is needed."""
    (tmp_path / "m.py").write_text("def a():\n    return b()\n\n\ndef b():\n    return 1\n")
    env = os.environ.copy()
    env["KB_CORE_OUT"] = "custom-out"
    ...
    assert not (tmp_path / "kb-core-out").exists(), "extract ignored KB_CORE_OUT and wrote kb-core-out/"
...
def test_path_uses_kb_core_out_env(tmp_path):
    out = _make_graph(tmp_path)
    custom_out = tmp_path / "custom-graph"
    out.rename(custom_out)
    env = os.environ.copy()
...
def test_explain_uses_kb_core_out_env(tmp_path):
    out = _make_graph(tmp_path)
    custom_out = tmp_path / "custom-graph"
    out.rename(custom_out)
    env = os.environ.copy()
...
def test_cluster_only_creates_output_dir_when_missing(tmp_path):
    """cluster-only must not crash with FileNotFoundError when kb-core-out/ is absent (#934)."""
    # Build graph.json somewhere other than the default kb_core-out/ location
    # so we can point --graph at it while kb_core-out/ doesn't exist yet.
    ...
    assert r.returncode == 0, r.stderr
    assert (tmp_path / "kb-core-out" / "GRAPH_REPORT.md").exists()


def test_cluster_only_graph_in_kb_core_out_writes_beside_it(tmp_path):
    """#1747 Case 2: `cluster-only --graph <elsewhere>/kb-core-out/graph.json`
    must write GRAPH_REPORT.md and the re-clustered graph beside that graph, not
    into a stray kb-core-out/ in the CWD."""
    project = tmp_path / "project"
    project.mkdir()
    out_dir = _make_graph(project)  # project/kb_core-out/graph.json
...
def test_extract_out_does_not_pollute_corpus(tmp_path):
    ...
    assert r.returncode == 0, r.stderr
    assert (out / "kb-core-out" / "graph.json").exists()   # graph in --out
    assert not (corpus / "kb-core-out").exists()           # corpus untouched
...
def test_cluster_only_persists_analysis_sidecar(tmp_path):
    """cluster-only must refresh .kb_core_analysis.json alongside graph.json.
    ...
    """
    out = _make_graph(tmp_path)
    analysis_path = out / ".kb_core_analysis.json"
...
def test_cluster_only_remaps_labels_to_previous_cids(tmp_path):
    ...
    .kb_core_labels.json keeps tracking the same conceptual communities after
    ...
    labels_json = out / ".kb_core_labels.json"
...
# ── communities-fallback when .kb_core_analysis.json is absent ──────────────
# The watch / post-commit rebuild path only writes graph.json + GRAPH_REPORT.md;
# it does NOT regenerate .kb_core_analysis.json. The full `kb_core extract`
...
# Without these tests, `kb_core export html|obsidian|wiki|svg|graphml|neo4j`
...
def test_export_html_falls_back_to_node_community_attribute(tmp_path):
    """When .kb_core_analysis.json is absent, export html should reconstruct
    ...
    (out / ".kb_core_analysis.json").unlink()
...
def test_export_html_fallback_recovers_multiple_communities(tmp_path):
    ...
    analysis = json.loads((out / ".kb_core_analysis.json").read_text(encoding="utf-8"))
...
def test_graph_json_node_ids_are_portable_across_checkout_paths(tmp_path):
    ...
    def _build(root: Path):
        ...
        assert r.returncode == 0, r.stderr
        data = json.loads((root / "kb-core-out" / "graph.json").read_text())
...
def test_cluster_only_warns_when_labeling_flags_are_ignored(tmp_path):
    """#2534 case 1: with a saved .kb_core_labels.json the reuse branch never
    ...
    _make_graph(tmp_path)  # persists .kb_core_labels.json -> reuse branch
...
# ── kb_core query names its graph (#2789) ───────────────────────────────────
...
def test_query_command_header_names_the_graph(tmp_path):
    ...
    assert first_line.startswith("Graph: kb-core-out/graph.json ("), first_line
