...
- back up to <name>.kb-core-bak before any modifying write,
...
def test_claude_install_preserves_existing_settings(tmp_path):
    """#2167 core case: every key kb-core does not own must survive install."""
    seeded = {
    ...
    # top-level keys kb_core does not own are untouched
    assert result["mcpServers"] == seeded["mcpServers"]
    assert result["enabledPlugins"] == seeded["enabledPlugins"]
    assert result["theme"] == "dark"
    # hooks sections kb_core does not manage are untouched
    assert result["hooks"]["PostToolUse"] == seeded["hooks"]["PostToolUse"]
    # the user's own PreToolUse entry survives alongside kb_core's
    pre_tool = result["hooks"]["PreToolUse"]
    assert seeded["hooks"]["PreToolUse"][0] in pre_tool
    kb_core_hooks = [h for h in pre_tool if "kb-core" in str(h)]
    assert len(kb_core_hooks) == 2
    # strict=True lands on the read guard
    assert any(h["hooks"][0]["command"].endswith("--strict") for h in kb_core_hooks)
...
def test_bom_settings_are_merged_not_clobbered(tmp_path, installer):
    ...
    assert any("kb-core" in str(h) for h in result["hooks"][section])
...
def test_invalid_json_aborts_without_clobbering(tmp_path, installer, capsys):
    ...
    assert not settings_path.with_name(settings_path.name + ".kb-core-bak").exists()
...
def test_backup_written_before_modify_and_stable_on_reinstall(tmp_path, installer):
    ...
    # nor the backup may be rewritten (the backup keeps the pre-kb_core content).
    _run(tmp_path, installer)
...
def test_non_dict_hook_entry_is_preserved_not_fatal(tmp_path, installer):
    ...
    assert any("kb-core" in str(h) for h in entries)
