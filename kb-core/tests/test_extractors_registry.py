...
own ``kb-core/extractors/<lang>.py`` module, kept a verbatim re-export in
``kb_core.extract`` (the facade every existing importer uses), and seeded a
``kb_core.extractors.LANGUAGE_EXTRACTORS`` registry. Three things must stay
true for that split to be behavior-preserving:

- the function is importable from its new per-language module,
- ``kb_core.extract`` still re-exports the SAME function object (facade
...
import kb_core.extract as facade
from kb_core.extractors import LANGUAGE_EXTRACTORS
...
def test_every_registry_extractor_is_reexported_from_facade():
    ...
    assert not missing, f"registry extractors not re-exported from kb_core.extract: {missing}"
