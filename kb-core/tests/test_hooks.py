...
import pytest
from kb_core.hooks import install, uninstall, status, _hooks_dir, _HOOK_MARKER, _CHECKOUT_MARKER
...
def test_hook_skips_head_on_exe():
    ...
    from kb_core.hooks import _PYTHON_DETECT
...
def test_install_embeds_pinned_interpreter(tmp_path):
    ...
    kb-core launcher on PATH (uv tool / pipx isolation, #1127).

    When kb-core is installed via `uv tool install kb-core` or `pipx install
    kb-core`, the interpreter lives in an isolated venv and the launcher is in
    ~/.local/bin.  GUI git clients and CI runners often run with a minimal PATH
    that omits that directory, so `command -v kb-core` fails, the python3/python
    fallbacks cannot import kb-core (wrong venv), and the hook silently exits 0.
...
def test_hook_check_no_additionalContext(tmp_path):
    """kb-core hook-check must not emit additionalContext — Codex Desktop rejects it."""
    import sys
    ...
    result = subprocess.run(
        [sys.executable, "-m", "kb-core", "hook-check"],
        cwd=tmp_path,
        capture_output=True,
        text=True,
    )
...
from kb_core.hooks import (  # noqa: E402
    _HOOK_SCRIPT,
    _CHECKOUT_SCRIPT,
...
def test_hooks_limit_windows_workers_by_default(name, script):
    ...
    assert 'export KB_CORE_MAX_WORKERS="${KB_CORE_MAX_WORKERS:-1}"' in script


def _launcher_payload(script: str) -> str:
    """Extract the `python -c "<payload>"` the hook hands to KB_CORE_PYTHON.
...
def test_rebuild_bodies_read_kb_core_root(name, body):
    """The rebuild must honour the persisted scan root rather than hardcoding the
    repo top (#1173). Both bodies read <output-dir>/.kb_core_root and pass the
    recovered root to _rebuild_code instead of the bare Path('.')."""
    assert ".kb_core_root" in body, f"{name} ignores .kb_core_root (#1173)"
    # The output dir is resolved from KB_CORE_OUT at hook-run time, not hardcoded
    # to kb_core-out/, so a renamed output dir is still found (#1423).
    assert "KB_CORE_OUT" in body, f"{name} ignores the KB_CORE_OUT override (#1423)"
...
def test_rebuild_bodies_with_kb_core_root_are_valid_python():
    """The .kb_core_root snippet must parse so a quoting slip can't ship a hook
...
def test_detached_launch_targets_kb_core_python():
    """The launcher must run via the resolved $KB_CORE_PYTHON, not a bare
    `python`, so it uses the same interpreter the detection block selected."""
    snippet = _detached_launch(_REBUILD_BODY_COMMIT)
    assert snippet.startswith('"$KB_CORE_PYTHON" -c "')
...
def test_windows_hookspath_rejected_no_junk_dir_on_posix(tmp_path, monkeypatch, winpath):
    ...
    monkeypatch.setattr("kb_core.hooks.os.name", "posix")
    repo = _make_git_repo(tmp_path)
...
def test_probes_use_find_spec_not_full_import():
    """`python -c "import kb-core"` executes the FULL package import — 10s+ on a
    ...
    from kb_core.hooks import _PYTHON_DETECT
    assert '-c "import kb-core"' not in _PYTHON_DETECT, (
...
def test_shebang_read_is_null_byte_safe():
    """On Windows, `command -v kb-core` can return the launcher path WITHOUT its
...
def test_probe_prefers_sibling_python_exe_on_windows_layouts():
    """pip on Windows puts Scripts/kb-core(.exe) beside ..\\python.exe (or
...
def _detect_run(tmp_path, home, stub_bin, env_extra=None):
    ...
    environment — dead pin, no .kb_core_python, an unparseable launcher first
    on PATH, and a python3 that cannot import kb-core (#2852's uv-tool
    Windows machine reproduced on POSIX) — and report what KB_CORE_PYTHON
...
def _broken_uv_machine(tmp_path):
    """#2852's machine: the only kb-core-importable python lives in the uv
    tool venv; the launcher on PATH is a binary trampoline reached WITHOUT its
    .exe suffix (Git-Bash command -v); ambient python3 cannot import kb_core.
    Returns (home, stub_bin)."""
    home = tmp_path / "home"
    stub_bin = tmp_path / "stubbin"
    stub_bin.mkdir(parents=True)
    launcher = stub_bin / "kb-core"
    launcher.write_bytes(b"MZ\x90\x00\x03" + bytes(range(60)))
    launcher.chmod(0o755)
    # Ambient pythons answer the probe with "no module named kb_core" —
...
def _tool_venv(home, tool, rel, ok):
    ...
    ok=False simulates a venv without kb-core (the probe must reject it)."""
...
def test_uv_tool_env_rescues_hook_when_pin_and_launcher_fail(tmp_path):
    """#2852: `uv tool install` puts kb-core in an isolated venv no ambient
    ...
    mine = _tool_venv(home, "kb-core", "bin/python", ok=True)
...
def test_uv_tool_env_honors_uv_tool_dir_and_windows_layout(tmp_path):
    ...
    py = tools / "kb-core" / "Scripts" / "python.exe"
...
@pytest.mark.skipif(shutil.which("sh") is None, reason="sh required to run emitted probe chain")
def test_uv_tool_env_without_kb_core_still_fails_loudly(tmp_path):
    """The scan must not adopt a tool env whose python lacks kb-core; the
...
def test_file_path_allowlist_accepts_windows_backslash_path(winpath, tmp_path):
    """#2126: the .kb_core_python FILE allowlist must accept real Windows paths
...
def test_shebang_allowlist_accepts_windows_backslash_path(shebang_path, tmp_path):
    ...
    pattern = _extract_case_pattern('KB_CORE_PYTHON="" ;;')
    _assert_harness_can_reject(pattern, tmp_path)
...
def test_python_detect_allowlists_still_reject_shell_metacharacters(dangerous, tmp_path):
    ...
    for marker in ('_FROM_FILE=""', 'KB_CORE_PYTHON="" ;;'):
...
def test_hooks_honor_skip_env(name, script):
    """KB_CORE_SKIP_HOOK=1 must suppress BOTH hooks. post-checkout previously
    lacked the check, so the var stopped commit rebuilds but not branch-switch
    ones (#1809)."""
    assert '[ "${KB_CORE_SKIP_HOOK:-0}" = "1" ] && exit 0' in script, (
        f"{name} does not honor KB_CORE_SKIP_HOOK"
    )
...
def test_checkout_hook_skips_same_head_noop_at_runtime():
    ...
    from kb_core.hooks import _CHECKOUT_SCRIPT
    ...
    # kb_core-out check / detached launch so nothing is actually rebuilt.
...
def _worktree_guard_snippet() -> str:
    from kb_core.hooks import _WORKTREE_GUARD
...
def test_install_registers_merge_driver(tmp_path):
    """install() must set merge.kb-core.* via git config and add the
    .gitattributes line that README/CHANGELOG 0.7.0 document (#1902)."""
    repo = _make_git_repo(tmp_path)
    result = install(repo)
    res = subprocess.run(
        ["git", "-C", str(repo), "config", "--get", "merge.kb-core.driver"],
        capture_output=True, text=True,
    )
    ...
    assert any(
        "graph.json" in line and "merge=kb-core" in line
...
def test_install_merge_driver_idempotent(tmp_path):
    ...
    matches = [l for l in lines if "merge=kb-core" in l]
...
def test_install_preserves_existing_gitattributes(tmp_path):
    ...
    assert "merge=kb-core" in content


def test_uninstall_removes_merge_driver_keeps_other_attrs(tmp_path):
    """uninstall() must unset merge.kb-core.* and remove only the kb-core
    ...
    content = (repo / ".gitattributes").read_text(encoding="utf-8")
    assert "*.png binary" in content
    assert "merge=kb-core" not in content


@pytest.mark.parametrize("exe", [
    r"C:\Users\First Last\AppData\Roaming\uv\tools\kb-core\Scripts\python.exe",
    r"C:\Program Files\Python312\python.exe",
    "/home/first last/.local/share/uv/tools/kb-core/bin/python",
])
def test_pinned_python_accepts_paths_containing_spaces(exe, monkeypatch):
    ...
    from kb_core.hooks import _pinned_python
...
def test_merge_driver_quotes_interpreter_with_spaces(tmp_path, monkeypatch):
    ...
    from kb_core.hooks import install

    exe = r"C:\Users\First Last\AppData\Roaming\uv\tools\kb-core\Scripts\python.exe"
    repo = _make_git_repo(tmp_path)
    monkeypatch.setattr(_sys, "executable", exe)
    install(repo)
    ...
    assert driver.endswith("-m kb-core merge-driver %O %A %B")
...
def test_kb_corerc_parsing(tmp_path):
    """Test 1: .kb_corerc parsing for valid and invalid values."""
    from kb_core.hooks import _load_kb_corerc

    rc = tmp_path / ".kb_corerc"
    rc.write_text("# comment\nviz_node_limit=0\n", encoding="utf-8")
    cfg = _load_kb_corerc(tmp_path)
    ...
    with pytest.raises(ValueError, match="Invalid viz_node_limit"):
        _load_kb_corerc(tmp_path)


def test_no_config_preserves_existing_hook(tmp_path):
    """Test 2: Without .kb_corerc, generated hooks omit KB_CORE_VIZ_NODE_LIMIT export."""
    repo = _make_git_repo(tmp_path)
    install(repo)
    commit_hook = (repo / ".git" / "hooks" / "post-commit").read_text()
    checkout_hook = (repo / ".git" / "hooks" / "post-checkout").read_text()
    assert "KB_CORE_VIZ_NODE_LIMIT" not in commit_hook
    assert "KB_CORE_VIZ_NODE_LIMIT" not in checkout_hook


def test_config_baked_into_generated_hook(tmp_path):
    """Test 3: viz_node_limit from .kb_corerc is baked into both hooks."""
    repo = _make_git_repo(tmp_path)
    (repo / ".kb_corerc").write_text("viz_node_limit=0\n", encoding="utf-8")
    install(repo)

    commit_hook = (repo / ".git" / "hooks" / "post-commit").read_text()
    checkout_hook = (repo / ".git" / "hooks" / "post-checkout").read_text()

    assert 'export KB_CORE_VIZ_NODE_LIMIT="${KB_CORE_VIZ_NODE_LIMIT:-0}"' in commit_hook
    assert 'export KB_CORE_VIZ_NODE_LIMIT="${KB_CORE_VIZ_NODE_LIMIT:-0}"' in checkout_hook
...
def test_baked_viz_limit_yields_to_an_explicit_per_run_override(tmp_path):
    ...
    KB_CORE_VIZ_NODE_LIMIT: the baked line uses the `${VAR:-<n>}` default form,
    so an already-set env value wins (mirrors KB_CORE_MAX_WORKERS)."""
    repo = _make_git_repo(tmp_path)
    (repo / ".kb_corerc").write_text("viz_node_limit=100\n", encoding="utf-8")
    install(repo)
    commit_hook = (repo / ".git" / "hooks" / "post-commit").read_text()

    # default form, not an unconditional assignment that would override the env
    assert 'export KB_CORE_VIZ_NODE_LIMIT="${KB_CORE_VIZ_NODE_LIMIT:-100}"' in commit_hook
    assert 'export KB_CORE_VIZ_NODE_LIMIT="100"' not in commit_hook
    # prove the shell semantics: an explicit env value survives the export line
    line = 'export KB_CORE_VIZ_NODE_LIMIT="${KB_CORE_VIZ_NODE_LIMIT:-100}"'
    out = subprocess.run(
        ["sh", "-c", f'KB_CORE_VIZ_NODE_LIMIT=7; {line}; echo "$KB_CORE_VIZ_NODE_LIMIT"'],
        capture_output=True, text=True, check=True,
    )
    assert out.stdout.strip() == "7"


def test_status_survives_a_malformed_kb_corerc(tmp_path):
    """A typo in the committed .kb_corerc must not turn the read-only `status`
    diagnostic into a traceback; it reports the problem and continues."""
    repo = _make_git_repo(tmp_path)
    install(repo)
    (repo / ".kb_corerc").write_text("viz_node_limit=not-an-int\n", encoding="utf-8")
...
def test_changing_config_updates_existing_hook(tmp_path):
    """Test 4: Re-running install updates existing KB Core hook block with new config."""
    repo = _make_git_repo(tmp_path)
    rc = repo / ".kb_corerc"
    ...
    commit_hook = (repo / ".git" / "hooks" / "post-commit").read_text()
    assert 'export KB_CORE_VIZ_NODE_LIMIT="${KB_CORE_VIZ_NODE_LIMIT:-0}"' in commit_hook
    assert 'export KB_CORE_VIZ_NODE_LIMIT="${KB_CORE_VIZ_NODE_LIMIT:-5000}"' not in commit_hook
    assert commit_hook.count("# kb-core-hook-start") == 1


def test_user_hook_content_survives_update(tmp_path):
    """Test 5: User hook content outside kb-core markers survives hook update."""
    repo = _make_git_repo(tmp_path)
    ...
    assert 'export KB_CORE_VIZ_NODE_LIMIT="${KB_CORE_VIZ_NODE_LIMIT:-0}"' in content
    assert "old block" not in content


def test_status_reports_configuration(tmp_path):
    """Test 6: kb-core hook status exposes configured viz node limit and detects out-of-date hooks."""
    repo = _make_git_repo(tmp_path)
    rc = repo / ".kb_corerc"
    rc.write_text("viz_node_limit=0\n", encoding="utf-8")
    install(repo)
...
def test_both_hooks_configured(tmp_path):
    ...
    repo = _make_git_repo(tmp_path)
    (repo / ".kb_corerc").write_text("viz_node_limit=42\n", encoding="utf-8")
    install(repo)

    for name in ("post-commit", "post-checkout"):
        hook_text = (repo / ".git" / "hooks" / name).read_text()
        assert 'export KB_CORE_VIZ_NODE_LIMIT="${KB_CORE_VIZ_NODE_LIMIT:-42}"' in hook_text
...
def test_rebuild_bodies_tolerate_a_bom_in_kb_core_root(name, body):
    ...
    assert "encoding='utf-8-sig'" in body, (
        f"{name} rebuild body must read .kb_core_root BOM-tolerantly"
    )
