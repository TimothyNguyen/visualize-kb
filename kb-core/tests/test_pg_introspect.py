...
def test_pg_introspect_import_error():
    ...
    # #1906: the PyPI package is kb_core (double-y), so the install hint must match
    assert "kb-core[postgres]" in str(exc_info.value)
...
def test_pg_introspect_grammar_error_surfaces():
    ...
    with patch.dict("sys.modules", {"psycopg": mock_psycopg}):
        with patch(
            "kb_core.pg_introspect.extract_sql",
            return_value={
                "nodes": [],
                "edges": [],
