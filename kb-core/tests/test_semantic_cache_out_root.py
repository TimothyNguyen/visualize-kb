...
#1990 — `kb-core extract --out` saves recovery checkpoints in the wrong directory.
...
def _semantic_dir(root: Path, mode: str | None = None) -> Path:
    ...
    return root / "kb-core-out" / "cache" / kind
...
def test_save_semantic_cache_no_corpus_kb_core_out_created(tmp_path):
    """With cache_root set, no kb-core-out/ dir should be created inside corpus."""
    corpus = tmp_path / "corpus"
    out = tmp_path / "out"
    corpus.mkdir()
    out.mkdir()
    ...
    assert not (corpus / "kb-core-out").exists(), (
        "kb-core-out/ must not be created inside the corpus when cache_root is set"
    )
