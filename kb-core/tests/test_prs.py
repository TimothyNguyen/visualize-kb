...
class TestFetchWorktrees:
    def test_normal_case_maps_branch_to_path(self):
        ...
        with patch("kb_core.prs.subprocess.run", return_value=mock_result):
...
class TestDetectDefaultBranch:
    ...
    def test_falls_back_to_git_symbolic_ref(self):
        ...
        with patch("kb_core.prs._gh", return_value=None), patch(
            "kb_core.prs.subprocess.run", return_value=mock_result
        ):
    ...
    def test_gh_returns_empty_dict_falls_back(self):
        ...
        with patch("kb_core.prs._gh", return_value={}), patch(
            "kb_core.prs.subprocess.run", return_value=mock_result
        ):
...
class TestSubprocessOutputEncoding:
    ...
    def test_detect_default_branch_decodes_output_as_utf8(self):
        ...
        with patch("kb_core.prs._gh", return_value=None), \
