...
Since #522 it runs as the shell-agnostic `kb-core hook-guard search` subcommand
...
from kb_core.__main__ import _claude_pretooluse_hooks
...
def _run(command, cwd, *, graph: bool):
    ...
    return subprocess.run(
        [sys.executable, "-m", "kb-core", "hook-guard", "search"],
...
def test_hook_command_has_no_backslashes(monkeypatch):
    ...
    # escape character and strips it (C:\Users\me\kb_core.EXE -> C:Usersme...),
    # breaking every guard. The emitted command must use forward slashes.
    from kb_core.__main__ import _resolve_kb_core_exe
    monkeypatch.setattr("shutil.which", lambda _name: r"C:\Users\me\kb_core.EXE")
    assert _resolve_kb_core_exe() == "C:/Users/me/kb_core.EXE"
...
def test_command_has_no_shell_syntax():
    ...
    assert "kb-core" in cmd and "hook-guard search" in cmd
...
def test_nudges_on_search_commands_with_graph(tmp_path):
    for command in (
        ...
        assert "kb-core query" in out, f"{command!r} should nudge"
...
def test_nudge_payload_is_valid_pretooluse_json(tmp_path):
    ...
    assert "kb-core query" in payload["hookSpecificOutput"]["additionalContext"]


def test_fails_open_on_malformed_stdin(tmp_path):
    (tmp_path / "kb-core-out").mkdir()
    (tmp_path / "kb-core-out" / "graph.json").write_text("{}", encoding="utf-8")
    r = subprocess.run(
        [sys.executable, "-m", "kb-core", "hook-guard", "search"],
...
def test_honors_kb_core_out_override(tmp_path):
    """The guard resolves the graph via KB_CORE_OUT, not a hardcoded path."""
    ...
    assert "kb-core query" in r.stdout
...
def test_grep_tool_input_nudges_with_graph(tmp_path):
    for tool_input in (
        ...
        assert "kb-core query" in out, f"Grep input {tool_input!r} should nudge"
...
def test_bash_non_search_with_stray_pattern_key_does_not_nudge(tmp_path):
    ...
    (tmp_path / "kb-core-out").mkdir(parents=True, exist_ok=True)
    (tmp_path / "kb-core-out" / "graph.json").write_text("{}", encoding="utf-8")
