...
skillgen renders kb-core's committed skill artifacts from human-edited
...
def test_no_version_or_timestamp_in_output():
    ...
    from kb_core.__main__ import __version__
...
def _claude_artifacts():
    ...
    core = next(a for a in arts if a.path == "kb_core/skill.md")
    refs = {a.path.rsplit("/", 1)[-1]: a.content for a in arts if a.path != "kb_core/skill.md"}
...
def test_lean_core_has_no_reference_only_content():
    ...
    assert "from kb_core.build import build_merge" not in core
    assert "kb-core cluster-only ." not in core
    ...
    assert "kb-core export wiki" not in core
    assert "kb-core export neo4j" not in core
    # The add / watch / hook flows live only in their references.
    assert "from kb_core.ingest import ingest" not in core
    assert "kb-core hook install" not in core
    assert "python3 -m kb_core.watch" not in core
...
def test_lean_core_runs_default_pipeline_with_zero_references():
    ...
    for needed in (
        "### Step 1 - Ensure kb-core is installed",
        "### Step 2 - Detect files",
...
def test_extraction_states_no_api_key_required_for_every_host():
    ...
    for a in bodies:
        assert "kb-core needs no API key" in a.content, a.path
        ...
        if tip in a.content:
            assert a.content.index("kb-core needs no API key") < a.content.index(tip), \
...
def test_query_heading_is_homed_in_core_stub_only():
    ...
    assert "## For /kb-core query" in core_headings
    assert "## For /kb-core query" not in query_headings
    # The deeper query content moved into the reference.
    assert "## For /kb-core path" in query_headings
    assert "## For /kb-core explain" in query_headings
    assert "## For /kb-core path" not in core_headings
...
UNIFIED_DESCRIPTION = (
    ...
    "or project content — especially when kb-core-out/ exists, where the question "
    "should be treated as a kb-core query first. Turns any input (code, docs, "
...
def test_descriptions_are_unified():
    ...
    for key, p in platforms.items():
        ...
        assert "treat the question as a /kb-core query." not in body, f"[{key}] kept old wording"
        assert "clustered communities" not in body, f"[{key}] kept old wording"


def test_windows_frontmatter_name_and_shell_and_extra():
    """windows: name must be `kb-core` (folder-name rule, #1635), powershell
    ...
    # (kb_core); a `kb_core-windows` name broke skill discovery (#1635).
    assert core.startswith("---\nname: kb-core\n")
    assert "```powershell" in core
    assert "function Find-KB CorePython" in core
...
def test_every_platform_query_has_expansion_and_fallback():
    ...
    for key in ("claude", "codex", "windows", "copilot", "vscode"):
        ...
        invocation = "$kb-core" if key == "codex" else "/kb-core"
        assert f"## For {invocation} path" in q
        assert f"## For {invocation} explain" in q
...
def test_powershell_hosts_carry_no_bash_only_shell():
    ...
    for key in _powershell_platform_keys():
        ...
        assert "```powershell" in core
        assert "'@ | & (Get-Content kb-core-out\\.kb_core_python) -" in core, (
        ...
        assert "Get-ChildItem kb-core-out -Filter '.kb_core_chunk_*.json'" in core
...
def test_windows_and_posix_cores_have_step_and_2490_parity():
    ...
    line_2490 = "to_json(G, communities, 'kb-core-out/graph.json', community_labels=labels)"
...
def test_powershell_translator_rejects_unknown_bash():
    ...
    assert gen._rm_to_remove_item("rm -f kb-core-out/.needs_update 2>/dev/null || true") == (
        "Remove-Item -Force -ErrorAction SilentlyContinue kb-core-out\\.needs_update"
    )
...
def test_always_on_included_in_full_render_not_per_platform():
    ...
    assert "kb_core/always_on/claude-md.md" in full
    assert "kb_core/always_on/claude-md.md" not in claude_only
...
def test_always_on_roundtrip_is_byte_faithful():
    ...
    kb_core.__main__, so the packaged markdown must round-trip exactly or those
    ...
    rendered_agents = next(
        ...
        if a.path == "kb_core/always_on/agents-md.md"
    )
    old_instruction = (
    ...
    assert 'skill: "kb-core"' not in rendered_agents
...
def test_extracted_constants_equal_the_packaged_always_on_files():
    ...
    from kb_core import __main__ as mainmod
...
def test_always_on_files_are_guarded_by_check(tmp_path):
    ...
    mutated = [
        ...
        if a.path == "kb_core/always_on/claude-md.md"
        else a
        for a in arts
    ]
...
def test_windows_skill_writes_marker_files_without_a_bom():
    ...
    it. The BOM then rode into `.kb_core_python` / `.kb_core_root`, so every
    ...
    """
    core, _ = _platform_artifacts("windows")
    for marker in (".kb_core_python", ".kb_core_root"):
        assert f"Out-File -FilePath kb-core-out\\{marker} -Encoding utf8" not in core, (
            f"the windows render still writes {marker} with a BOM-emitting Out-File"
        )
        assert f"WriteAllText((Join-Path $PWD 'kb-core-out\\{marker}')" in core, (
