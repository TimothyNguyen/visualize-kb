...
Every platform kb-core knows about installs its SKILL.md at a specific real
...
def test_skill_roundtrip_at_real_destination(platform, project, tmp_path, monkeypatch):
    ...
    with patch("kb_core.__main__.Path.home", return_value=home):
        ...
        assert (dst.parent / ".kb_core_version").read_text() == mainmod.__version__
        ...
        assert not (dst.parent / ".kb_core_version").exists()
...
def test_vscode_install_uninstall_roundtrip(tmp_path, monkeypatch):
    ...
    with patch("kb_core.__main__.Path.home", return_value=home):
        ...
        skill = home / ".copilot" / "skills" / "kb-core" / "SKILL.md"
        instructions = project_dir / ".github" / "copilot-instructions.md"
        assert skill.exists()
        assert instructions.exists()
        assert "## kb-core" in instructions.read_text(encoding="utf-8")
        ...
        # The kb_core section is stripped from the instructions file.
        if instructions.exists():
            assert "## kb-core" not in instructions.read_text(encoding="utf-8")
...
def test_monolith_to_progressive_upgrade(tmp_path, fake_progressive_bundle):
    ...
    (skill_dir / ".kb_core_version").write_text("0.0.1", encoding="utf-8")
    assert not (skill_dir / "references").exists()
...
def test_failed_copytree_leaves_no_partial_references(tmp_path, fake_progressive_bundle):
    ...
    with patch("kb_core.__main__.shutil.copytree", side_effect=boom):
