...
`kb-core hook-guard read` subcommand (not inline bash), so it works on Windows
too. These tests invoke that subcommand with crafted stdin JSON and assert it
nudges only for a source/doc file outside kb-core-out/ when a graph exists, and
...
def _env():
    # The guard resolves the graph via KB_CORE_OUT (default "kb_core-out",
...
def _run(tool_input, cwd, *, graph: bool):
    ...
    return subprocess.run(
        [sys.executable, "-m", "kb-core", "hook-guard", "read"],
...
def test_command_has_no_shell_syntax():
    ...
    assert "kb-core" in cmd and "hook-guard read" in cmd
...
def test_silent_on_kb_core_out_targets(tmp_path):
    """Reading the graph's own report must not start a go-read-the-graph loop."""
    out = _run({"file_path": "kb-core-out/GRAPH_REPORT.md"}, tmp_path, graph=True).stdout
    assert out.strip() == ""
...
def test_nudges_on_framework_source(tmp_path):
    ...
    for path in ("src/components/Hero.astro", "src/App.vue", "src/Card.svelte"):
        out = _run({"file_path": path}, tmp_path, graph=True).stdout
        assert "kb-core query" in out, f"{path} should nudge"
