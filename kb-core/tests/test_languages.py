...
def test_cpp_string_tests_punctuation_only_names_stay_distinct(tmp_path):
    ...
    r = extract_cpp(f)
    from kb_core.extract import _make_id, _file_stem
...
def test_metal_is_code_extension():
    from kb_core.detect import CODE_EXTENSIONS
...
def test_kotlin_emits_in_file_calls():
    ...
    `identifier` rename — see kb-core-kmp's PythonParityTest."""
    r = extract_kotlin(FIXTURES / "sample.kt")
    calls = _calls(r)
...
def test_swift_extension_across_files_merges_into_canonical_type():
    ...
    r = extract(paths, cache_root=Path("/tmp/kb-core-test-no-cache"))
...
from kb_core.extract import extract_elixir
...
# ── Objective-C ──────────────────────────────────────────────────────────────
from kb_core.extract import extract_objc
...
def test_objc_header_dispatch_routes_objc_not_c(tmp_path):
    ...
    from kb_core.extract import _get_extractor, extract_objc as _eo, extract_c as _ec
...
from kb_core.extract import extract_markdown
...
def test_markdown_frontmatter_fallback_parses_flat_keys():
    ...
    from kb_core.extractors.markdown import _parse_frontmatter_fallback
...
def test_markdown_heading_id_is_stable_regardless_of_frontmatter():
    ...
    from kb_core.extractors.base import _make_id, _file_stem
    r = _md_extract("---\ntitle: Doc\ntags: [a, b]\n---\n\n# Overview\n\n## Details\n")
...
# ── Zig ───────────────────────────────────────────────────────────────────────
from kb_core.extract import extract_zig
