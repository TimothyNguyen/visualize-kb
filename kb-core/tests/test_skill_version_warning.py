...
`_check_skill_version` used to advise `kb-core install` on ANY version
...
def _make_skill(tmp_path: Path, stamped: str) -> Path:
    skill_dst = tmp_path / "skills" / "kb-core" / "SKILL.md"
    skill_dst.parent.mkdir(parents=True, exist_ok=True)
    skill_dst.write_text("# kb-core skill\n", encoding="utf-8")
    (skill_dst.parent / ".kb_core_version").write_text(stamped, encoding="utf-8")
    return skill_dst
...
def test_skill_older_than_package_recommends_install(tmp_path, monkeypatch, capsys):
    ...
    assert "Run 'kb-core install' to update" in err
...
def test_skill_newer_than_package_recommends_upgrade_not_install(tmp_path, monkeypatch, capsys):
    ...
    assert "Run 'kb-core install' to update" not in err
