...
`sed q`) is routine. kb-core used to keep writing after the reader disconnected,
...
def test_help_survives_reader_closing_pipe_early():
    """`kb-core --help | head -n1` must leave kb-core exiting 0, not 255."""
    producer = subprocess.Popen(
        [PYTHON, "-m", "kb-core", "--help"], stdout=subprocess.PIPE
    )
...
def test_small_buffered_output_survives_reader_that_reads_nothing():
    ...
    producer = subprocess.Popen(
        [PYTHON, "-m", "kb-core", "--version"], stdout=subprocess.PIPE
    )
    reader = subprocess.Popen(
