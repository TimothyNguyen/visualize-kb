"""`kb-core merge-graphs` tolerates inputs that disagree on graph type (#1606).
...
def _run(args, cwd):
    return subprocess.run([PYTHON, "-m", "kb-core"] + args, cwd=cwd,
...
def test_merge_graphs_mixed_directed_and_multigraph(tmp_path):
    a = tmp_path / "r1" / "kb-core-out" / "graph.json"
    b = tmp_path / "r2" / "kb-core-out" / "graph.json"
    c = tmp_path / "r3" / "kb-core-out" / "graph.json"
...
def test_merge_graphs_same_named_repo_dirs_do_not_collapse(tmp_path):
    # #1729: two graphs under a same-named repo dir (src/kb_core-out and
    # frontend/src/kb_core-out both → tag "src") share the `src::` prefix, so a
    ...
    a = tmp_path / "src" / "kb-core-out" / "graph.json"
    b = tmp_path / "frontend" / "src" / "kb-core-out" / "graph.json"
...
def test_distinct_repo_tags_unit(tmp_path):
    from kb_core.build import distinct_repo_tags
    # distinct repo dirs pass through unchanged
    assert distinct_repo_tags([
        Path("backend/kb-core-out/graph.json"),
        Path("web/kb-core-out/graph.json"),
    ]) == ["backend", "web"]
    # same-named repo dirs are widened to stay distinct
    tags = distinct_repo_tags([
        Path("proj/src/kb-core-out/graph.json"),
        Path("proj/frontend/src/kb-core-out/graph.json"),
    ])
    ...
    tags3 = distinct_repo_tags([
        Path("a/src/kb-core-out/graph.json"),
        Path("b/src/kb-core-out/graph.json"),
        Path("c/src/kb-core-out/graph.json"),
    ])
...
def test_merge_graphs_preserves_import_edge_direction(tmp_path):
    ...
    a = tmp_path / "repo1" / "kb-core-out" / "graph.json"
    b = tmp_path / "repo2" / "kb-core-out" / "graph.json"
    a.parent.mkdir(parents=True, exist_ok=True)
    b.parent.mkdir(parents=True, exist_ok=True)
...
def test_merge_graphs_carries_hyperedges_from_all_inputs(tmp_path):
    ...
    a = tmp_path / "alpha" / "kb-core-out" / "graph.json"
    b = tmp_path / "beta" / "kb-core-out" / "graph.json"
