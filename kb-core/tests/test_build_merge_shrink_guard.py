...
- #2446: with `root` omitted and a non-standard layout (graph.json not under
  <root>/kb-core-out/), the inferred root was wrong, absolute prune_sources
...
import kb_core.build as buildmod
from kb_core.build import build_merge, merge_raw_extraction
...
def test_no_existing_graph_path_unchanged(tmp_path):
    ...
    gp = tmp_path / "kb-core-out" / "graph.json"  # does not exist
...
def test_absolute_prune_custom_layout_derives_root(tmp_path):
    """graph.json directly in <root> (no kb-core-out dir, no marker): the
...
def test_absolute_prune_standard_layout_marker_still_prunes(tmp_path):
    ...
    .kb_core_root marker keeps working without root=."""
    root = tmp_path / "repo"
    out = root / "kb-core-out"
    out.mkdir(parents=True)
    (out / ".kb_core_root").write_text(str(root), encoding="utf-8")
    gp = out / "graph.json"
...
def test_zero_match_prune_warns_instead_of_already_clean(tmp_path, capsys):
    ...
    gp = root / "kb-core-out" / "graph.json"
