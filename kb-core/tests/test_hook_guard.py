...
from kb_core import __main__ as m


# --------------------------------------------------------------------------- #
# Direct-call harness: hermetic w.r.t. the ambient KB_CORE_OUT env.
# --------------------------------------------------------------------------- #
def _invoke(kind, payload, tmp_path, monkeypatch, *, graph=True, out_name="kb-core-out"):
    monkeypatch.setattr("kb_core.paths.KB_CORE_OUT", out_name)
    monkeypatch.setattr("kb_core.paths.KB_CORE_OUT_NAME", out_name)
...
def test_read_nudges(tool_input, tmp_path, monkeypatch):
    ...
    assert "kb-core query" in out, f"{tool_input!r} should nudge"
...
@pytest.mark.parametrize("tool_input", [
    ...
    {"file_path": "kb-core-out/GRAPH_REPORT.md"},  # the graph's own output
    {"file_path": ""},
    {},                                          # nothing at all
])
...
def test_read_respects_custom_output_dir_name(tmp_path, monkeypatch):
    ...
    # not just the literal 'kb_core-out/'.
