...
def test_to_json_commit_fallback_uses_output_repo_not_cwd(tmp_path, monkeypatch):
    ...
    # be in — running `kb_core extract <target>` from another repo's root
    ...
    (target / "kb-core-out").mkdir(parents=True)
    git(target, "init")
    ...
    out = target / "kb-core-out" / "graph.json"
...
def test_to_html_title_uses_portable_path_not_host_absolute():
    ...
    communities = cluster(G)
    with tempfile.TemporaryDirectory() as tmp:
        userish = Path(tmp) / "Users" / "mike" / "proj" / "kb-core-out" / "graph.html"
    ...
    title = m.group(1)
    assert title.startswith("kb-core - ")
    label = title[len("kb-core - "):]
    assert "mike" not in label
    assert "Users" not in label
    assert not label.startswith("/")
    assert "kb-core-out/graph.html" in label or label == "graph.html"


def test_html_document_title_helper_windows_and_relative():
    from kb_core.exporters.html import _html_document_title

    assert _html_document_title(r"C:\Users\mike\proj\kb-core-out\graph.html") == "kb-core-out/graph.html"
    assert _html_document_title("/home/u/proj/kb-core-out/graph.html") == "kb-core-out/graph.html"
    assert _html_document_title("kb-core-out/graph.html") == "kb-core-out/graph.html"
...
def test_to_html_neighbor_links_have_no_inline_onclick_xss():
    ...
    (from a document or a scraped `kb-core add` URL) inject a live event handler
...
def test_obsidian_safe_stem_all_dots_label_falls_back_to_unnamed():
    ...
    from kb_core.export import _obsidian_safe_stem
...
# ── Existing-vault safety: kb_core must not clobber user notes / .obsidian (#1506) ──
...
def test_to_obsidian_preserves_existing_user_notes_and_obsidian_config():
    ...
    collides with a kb-core node name, nor their .obsidian/ graph settings."""
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # non-colliding kb_core note still written
...
def test_to_obsidian_rerun_updates_own_notes_but_not_user_files():
    """A re-run overwrites kb-core's own prior notes (via the manifest) but leaves a
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert (out / "Database.md").exists()  # kb_core re-wrote its own
...
def test_to_obsidian_rerun_prunes_removed_nodes():
    """#1896: re-exporting into the same vault must delete kb-core's own notes for
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # surviving kb_core notes and the user's own note remain
        assert (out / "Database.md").exists() and (out / "Server.md").exists()
...
def test_backup_no_graph_json(tmp_path):
    ...
    from kb_core.export import backup_if_protected
...
def test_backup_semantic_marker(tmp_path):
    """graph.json + .kb_core_semantic_marker → backup taken."""
    from kb_core.export import backup_if_protected
    (tmp_path / "graph.json").write_text('{"nodes":[],"links":[]}')
    (tmp_path / "GRAPH_REPORT.md").write_text("# Report")
    (tmp_path / ".kb_core_semantic_marker").write_text('{"output_tokens": 1234}')
    result = backup_if_protected(tmp_path)
    assert result is not None
    assert result.is_dir()
    assert (result / "graph.json").exists()
    assert (result / "GRAPH_REPORT.md").exists()
    assert (result / ".kb_core_semantic_marker").exists()


def test_backup_curated_labels(tmp_path):
    """graph.json + non-default label in .kb_core_labels.json → backup taken."""
    import json
    from kb_core.export import backup_if_protected
    (tmp_path / "graph.json").write_text('{"nodes":[],"links":[]}')
    (tmp_path / ".kb_core_labels.json").write_text(json.dumps({"0": "Auth Pipeline", "1": "Community 1"}))
    result = backup_if_protected(tmp_path)
    assert result is not None
...
def test_backup_default_labels_only(tmp_path):
    ...
    from kb_core.export import backup_if_protected
    (tmp_path / "graph.json").write_text('{"nodes":[],"links":[]}')
    (tmp_path / ".kb_core_labels.json").write_text(json.dumps({"0": "Community 0", "1": "Community 1"}))
    assert backup_if_protected(tmp_path) is None
...
def test_backup_same_day_no_accumulation(tmp_path):
    ...
    (tmp_path / "graph.json").write_text('{"nodes":[],"links":[]}')
    (tmp_path / ".kb_core_semantic_marker").write_text("{}")
...
def test_backup_env_disable(tmp_path, monkeypatch):
    """KB_CORE_NO_BACKUP=1 disables backup entirely."""
    from kb_core.export import backup_if_protected
    monkeypatch.setenv("KB_CORE_NO_BACKUP", "1")
    (tmp_path / "graph.json").write_text('{"nodes":[],"links":[]}')
    (tmp_path / ".kb_core_semantic_marker").write_text("{}")
    assert backup_if_protected(tmp_path) is None
...
def test_existing_graph_node_count(tmp_path):
    from kb_core.export import existing_graph_node_count, MALFORMED_GRAPH
    p = tmp_path / "graph.json"
...
def test_hyperedge_perimeter_uses_convex_hull_not_member_order():
    ...
    from kb_core.exporters.html import _hyperedge_script
