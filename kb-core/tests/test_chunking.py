"""Tests for token-aware chunking and parallel chunk execution in kb_core.llm."""
import time
...
def test_pack_chunks_packs_small_files_together(tmp_path):
    ...
    from kb_core.llm import _pack_chunks_by_tokens
...
def test_corpus_parallel_runs_chunks_concurrently(tmp_path):
    ...
    from kb_core.llm import extract_corpus_parallel
    ...
    with patch("kb_core.llm.extract_files_direct", side_effect=slow_extract):
        t0 = time.time()
...
def test_corpus_parallel_sequential_when_max_concurrency_is_one(tmp_path):
    ...
    with patch("kb_core.llm.extract_files_direct", side_effect=record):
...
def test_corpus_parallel_merge_order_is_submission_order_not_completion(tmp_path):
    ...
    with patch("kb_core.llm.extract_files_direct", side_effect=latency_skewed):
        result = extract_corpus_parallel(
...
def test_corpus_parallel_continues_after_chunk_failure(tmp_path, capsys):
    ...
    with patch("kb_core.llm.extract_files_direct", side_effect=maybe_fail):
        result = extract_corpus_parallel(
            files, backend="kimi", token_budget=None, chunk_size=1, max_concurrency=1
        )
...
def test_checkpoint_scopes_cache_writes_to_chunk_files(tmp_path):
    ...
    from kb_core.llm import extract_corpus_parallel
    from kb_core.cache import save_semantic_cache, load_cached
    ...
    with patch("kb_core.llm.extract_files_direct", side_effect=stray):
        extract_corpus_parallel(
    ...
    from kb_core.llm import _extraction_system
...
def test_truncated_chunk_is_cached_partial_and_missed_on_reload(tmp_path):
    ...
    from kb_core.llm import extract_corpus_parallel, _extraction_system
    from kb_core.cache import load_cached
    ...
    with patch("kb_core.llm.extract_files_direct", side_effect=truncated):
        extract_corpus_parallel(
...
def test_checkpoint_writes_deep_namespace_in_deep_mode(tmp_path):
    ...
    with patch("kb_core.llm.extract_files_direct", side_effect=ok):
        extract_corpus_parallel(
            [doc], backend="kimi", root=tmp_path,
            token_budget=None, chunk_size=1, max_concurrency=1,
            deep_mode=True,
...
def test_omitted_documents_are_reconciled_and_warned(tmp_path, capsys):
    ...
    with patch("kb_core.llm.extract_files_direct", side_effect=omit_odd):
        result = extract_corpus_parallel(
...
def test_out_of_scope_drop_count_is_zero_when_all_in_scope(tmp_path, capsys):
    ...
    with patch("kb_core.llm.extract_files_direct", side_effect=clean):
        result = extract_corpus_parallel(
            [a], backend="kimi", root=tmp_path,
            token_budget=None, chunk_size=1, max_concurrency=1,
        )
...
def test_checkpoint_caches_sliced_document_chunks(tmp_path, capsys):
    ...
    with patch("kb_core.llm.extract_files_direct", side_effect=sliced):
        extract_corpus_parallel(
            [doc], backend="kimi", root=tmp_path,
            token_budget=None, chunk_size=1, max_concurrency=1,
        )
...
def test_adaptive_retry_returns_directly_when_not_truncated(tmp_path):
    ...
    from kb_core.llm import _extract_with_adaptive_retry
    ...
    with patch("kb_core.llm.extract_files_direct", side_effect=stub):
...
def test_adaptive_retry_caps_at_max_depth(tmp_path, capsys):
    ...
    with patch("kb_core.llm.extract_files_direct", side_effect=always_truncate):
