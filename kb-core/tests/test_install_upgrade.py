...
Pre-fix, the installers wrote a "## kb-core" section with report-first
instructions and skipped writing if the marker was already present. So users
who installed kb-core and then upgraded to the fixed package still had the
...
# wrote a variant of "ALWAYS read kb_core-out/GRAPH_REPORT.md before ...".
_OLD_CLAUDE_SECTION = """\
## kb-core
...
Rules:
- ALWAYS read kb-core-out/GRAPH_REPORT.md before reading any source files, running grep/glob searches, or answering codebase questions. The graph is your primary map of the codebase.
- IF kb-core-out/wiki/index.md EXISTS, navigate it instead of reading raw files
- For cross-module "how does X relate to Y" questions, prefer `kb-core query "<question>"`, `kb-core path "<A>" "<B>"`, or `kb-core explain "<concept>"` over grep — these traverse the graph's EXTRACTED + INFERRED edges instead of scanning files
...
_OLD_HOOK_PAYLOAD_SNIPPET = "Read kb-core-out/GRAPH_REPORT.md for god nodes and community structure before searching raw files"


def _assert_no_report_first(text: str, ctx: str) -> None:
    assert "ALWAYS read kb-core-out/GRAPH_REPORT.md" not in text, (
...
def _assert_query_first(text: str, ctx: str) -> None:
    assert "kb-core query" in text, (
        f"{ctx}: new 'kb-core query' guidance missing after upgrade"
    )
...
def test_claude_install_upgrades_stale_section(tmp_path, monkeypatch):
    ...
    `kb-core claude install` again after upgrading to a fixed package."""
    ...
    # Pre-existing non-kb_core content must be preserved
...
def test_claude_install_upgrades_stale_hook_payload(tmp_path, monkeypatch):
    ...
    # Since #522 the nudge text lives in the `kb_core hook-guard` subcommand, not
    ...
    assert "hook-guard" in new_settings_text, (
        "new hook payload should route to the `kb-core hook-guard` subcommand"
    )
