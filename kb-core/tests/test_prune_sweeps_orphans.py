...
On kb-core's own package, pruning `kb-core/callflow_html.py` removed 137 of its
139 nodes and stranded `kb_core_callflow_html_py_path` (label `Path`) and
`kb_core_callflow_html_py_counter` (label `Counter`) permanently.
...
from kb_core.build import build_from_json, build_merge
