...
def _capture_dedup(monkeypatch):
    ...
    Patching `kb_core.build` does reach the CLI's `_build` / `_build_merge`
    aliases, even though it refers to them by those names: the

        from kb_core.build import build as _build, build_merge as _build_merge
...
def _assert_spied(seen: dict, entry_point: str) -> None:
    ...
    assert entry_point in seen, (
        ...
        f"kb_core.build symbol — the spy is inert and every dedup assertion "
