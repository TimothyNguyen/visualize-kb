...
def test_kb_core_openai_model_wins_over_openai_model(monkeypatch):
    # model_env_key (KB_CORE_OPENAI_MODEL) is resolved at call time and takes
    # precedence over the import-time OPENAI_MODEL default.
    monkeypatch.setenv("OPENAI_MODEL", "env-default-model")
    monkeypatch.setenv("KB_CORE_OPENAI_MODEL", "kb-core-override-model")
    reloaded = importlib.reload(llm)
    try:
        assert reloaded._default_model_for_backend("openai") == "kb-core-override-model"
    finally:
        monkeypatch.undo()
        importlib.reload(llm)
