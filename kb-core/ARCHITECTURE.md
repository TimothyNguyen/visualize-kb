...
kb-core is a Claude Code skill backed by a Python library. The skill orchestrates the library; the library can be used standalone.
...
Each stage lives in its own module and they communicate through plain Python dicts and NetworkX graphs - no shared state, no side effects outside `kb-core-out/`. Most stages are a single function; `analyze.py` and `export.py` are sets of sibling functions rather than one entry point.
...
| `callflow_html.py` | `write_callflow_html(...)` | kb-core-out files → Mermaid architecture/call-flow HTML |
...
1. Add an `extract_<lang>(path: Path) -> dict` function following the existing pattern (tree-sitter parse → walk nodes → collect `nodes` and `edges` → call-graph second pass for INFERRED `calls` edges). New languages go in their own module under `kb_core/extractors/` - see `kb_core/extractors/MIGRATION.md`; `extract.py` re-exports them while the existing ones are ported out of it.
...
All external input passes through `kb_core/security.py` before use:
...
- Graph file paths → `validate_graph_path()` (must resolve inside `kb-core-out/`)
