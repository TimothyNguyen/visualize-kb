...
cat > "${PROJECT_ROOT}/kb-core-out/.kb_core_chunk_0N.json" <<'CHUNK_JSON'
PASTED_JSON
CHUNK_JSON
```

Repeat for every chunk. Each chunk's JSON must land in its own `kb-core-out/.kb_core_chunk_NN.json` before Step B3 runs.
