...
Run once per project to make kb-core always-on in your agent sessions:

```bash
kb-core agents install
```

This writes a `## kb-core` section to the local `AGENTS.md` that instructs your agent to check the graph before answering codebase questions and rebuild it after code changes. No manual `/kb-core` needed in future sessions.

```bash
kb-core agents uninstall  # remove the section
```
