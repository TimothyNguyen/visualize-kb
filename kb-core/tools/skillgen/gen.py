"""skillgen: render kb-core's committed skill artifacts from edited fragments.
...
files under ``kb-core/skill*.md`` and ``kb-core/skills/<platform>/references/``
...
except ModuleNotFoundError:  # Python 3.10 - kb_core supports >=3.10
...
# straight from git instead of from disk. claude's v8 body is kb_core/skill.md;
# every other split host has its own kb_core/skill-<host>.md. Auditing each host
...
_LEGACY_PACKAGE = "graph" + "ify"
_LEGACY_BRAND = _LEGACY_PACKAGE.title()
_LEGACY_ENV = _LEGACY_PACKAGE.upper()
...
# blocks used to be triple-quoted constants in kb_core/__main__.py; they are now
# packaged kb_core/always_on/*.md files the module reads at load. This ref points
...
ALWAYS_ON_BASELINE_REF = f"{_V8_BASELINE_SHA}:{_LEGACY_PACKAGE}/__main__.py"

# The always-on instruction blocks: rendered-file basename -> the __main__.py
# constant it must reproduce. Rendered to kb_core/always_on/<basename>.md from
...
ALWAYS_ON_SANCTIONED_EDITS: dict[str, tuple[tuple[str, str], ...]] = {
    ...
    # invoke a literal `skill` tool with `skill: "kb_core"`, which is
    # host-specific and not valid in every environment.
    "_AGENTS_MD_SECTION": (
        (
            "When the user types `/kb-core`, invoke the `skill` tool with "
            '`skill: "kb-core"` before doing anything else.',
            "When the user types `/kb-core`, use the installed kb-core skill or instructions "
...
# them. Two classes:
#
# 1. SHARED_INTRO_ALLOWLIST — the lean intro consolidation. "## What kb_core is
...
SHARED_INTRO_ALLOWLIST: frozenset[str] = frozenset({
    "## What kb-core is for",  # lean intro; v8 hosts had verbose intro prose, no heading.
})
...
def load_platforms() -> dict[str, Platform]:
    ...
    for key, cfg in data.get("platform", {}).items():
        out[key] = Platform(
            ...
            roundtrip_ref=(
                cfg["roundtrip_ref"].replace("kb_core/", f"{_LEGACY_PACKAGE}/")
                if cfg.get("roundtrip_ref")
                else None
            ),
        )
    return out
...
#   * the inline ``$(cat kb_core-out/.kb_core_python) -c "..."`` python blocks
#     become single-quoted here-strings piped to the interpreter's stdin
#     (``@'...'@ | & (Get-Content kb_core-out\.kb_core_python) -``). A
...
_PY_INVOKE_POSIX = '$(cat kb-core-out/.kb_core_python) -c "'
_PY_INVOKE_PS_OPEN = "@'"
_PY_INVOKE_PS_CLOSE = "'@ | & (Get-Content kb-core-out\\.kb_core_python) -"
_MKDIR_POSIX = "mkdir -p kb-core-out"
_MKDIR_PS = "New-Item -ItemType Directory -Force -Path kb-core-out | Out-Null"
_FIND_CHUNKS_POSIX = "find kb-core-out -maxdepth 1 -name '.kb_core_chunk_*.json' -delete 2>/dev/null"
_FIND_CHUNKS_PS = (
    "Get-ChildItem kb-core-out -Filter '.kb_core_chunk_*.json' -File "
...
def _translate_bash_block(lines: list[str]) -> list[str]:
    ...
    for line in lines:
        ...
        elif not line.strip() or line.lstrip().startswith("#") or line.startswith("kb-core "):
            out.append(line)  # blank lines, comments, and kb_core CLI calls are shell-neutral
        else:
...
def _platform_command_syntax(body: str, platform: Platform) -> str:
    ...
    if platform.key == "codex":
        return body.replace("/kb-core", "$kb-core")
    return body


def _canonical_skill_heading(heading: str) -> str:
    """Compare host-native skill headings independent of invocation sigil."""
    return heading.replace("$kb-core", "/kb-core")
...
def render(platform: Platform) -> list[RenderedArtifact]:
    ...
    if platform.bucket == "monolith":
        ...
        return [RenderedArtifact(platform.skill_dst, _platform_command_syntax(body, platform))]
    ...
    artifacts: list[RenderedArtifact] = [
        RenderedArtifact(platform.skill_dst, _platform_command_syntax(_render_core(platform), platform))
    ]
...
def render_always_on() -> list[RenderedArtifact]:
    """Render the six always-on instruction blocks to kb_core/always_on/*.md.
    ...
    for basename in sorted(ALWAYS_ON_BLOCKS):
        ...
        out.append(RenderedArtifact(f"kb_core/always_on/{basename}.md", body))
    return out
...
def audit_coverage(platform: Platform) -> list[str]:
    ...
    The audit reads the host's OWN v8 skill body (kb-core/skill.md for claude,
    kb-core/skill-<host>.md otherwise) and checks that every v8 heading lands in
    ...
    v8 headings are exempt and documented as deltas, not holes:
      - waves 2-3 consolidations (the lean "## What kb-core is for" intro and the
    ...
    problems: list[str] = []
    baseline_headings = [
        _canonical_skill_heading(
            heading.replace(_LEGACY_PACKAGE, "kb-core").replace(_LEGACY_BRAND, "KB Core")
        )
        for heading in headings(_git_show(_v8_baseline_ref(platform.key)))
    ]
    allowlist = _audit_allowlist(platform.key)

    artifacts = render(platform)
    by_path = {a.path: a.content for a in artifacts}
    core_headings = {_canonical_skill_heading(h) for h in headings(by_path[platform.skill_dst])}

    # Map each reference's rendered heading set.
    ref_headings: dict[str, set[str]] = {}
    for name in platform.reference_sources():
        rel = f"{platform.refs_dst}/{name}.md"
        ref_headings[name] = {_canonical_skill_heading(h) for h in headings(by_path[rel])}
...
def _is_frontmatter_description_line(line: str) -> bool:
    ...
    The unified description (kb-core #1106) rewrites the frontmatter
...
def _is_chunk_cleanup_line(line: str) -> bool:
    ...
    The bare glob ``.kb_core_chunk_*.json`` in the v8 cleanup line aborts the
    whole ``rm`` under fish/zsh when no chunk files exist (no-match is a hard
    error there, unlike bash). The fix (kb-core #1172) drops the glob from the
    ...
    Both the v8 form (bare ``.kb_core_chunk_*.json`` glob, removed) and the fixed
    ...
    if not s.startswith("rm -f"):
        return False
    return ".kb_core_chunk_*.json" in line or ("find " in line and "-name '.kb_core_chunk_" in line)
...
def _is_cache_unlink_fix_line(line: str) -> bool:
    ...
    ``.kb_core_cached.json`` for Part C to merge. The miss branch now deletes it.
    """
    return (
        ".kb_core_cached.json').unlink(missing_ok=True)" in line
        or line.strip() == "else:"
        or "Always (re)write the cache file" in line
        or "stale .kb_core_cached.json" in line
    )
...
def _is_zero_node_guard_fix_line(line: str) -> bool:
    ...
    return (
        ...
        or s == "if not wrote:"
        or "refused to shrink kb-core-out/graph.json" in line
...
def _is_manifest_root_fix_line(line: str) -> bool:
    ...
    relativized to the scan root, matching the native ``kb-core update`` path.
    Both the old bare call (removed) and the new rooted call (added) match here;
    the ``import`` guard avoids matching the ``from kb_core.detect import
    save_manifest`` line.
    """
...
def _is_manifest_stamp_fix_line(line: str) -> bool:
    ...
    ``kb-core extract`` path. The rooted ``save_manifest`` call itself is
...
def _is_no_api_key_fix_line(line: str) -> bool:
    ...
    stating that kb-core needs no API key, and (like the subagent-host skills)
    ...
    """
    return "kb-core needs no API key" in line
...
def _is_shebang_allowlist_fix_line(line: str) -> bool:
    ...
    detection fell through to a bare ``python3`` that lacked kb_core. ``@`` is now
...
def _is_obsidian_usage_comment_line(line: str) -> bool:
    """Whether a line is part of the ``/kb-core`` usage-comment fix (#1681).

    The Usage block's bare ``/kb-core`` comment said "full pipeline on current
...
def _is_uv_from_interpreter_fix_line(line: str) -> bool:
    ...
    Step 1's POSIX interpreter probe ran ``uv tool run kb-core python -c ...``,
    but ``kb-core`` exposes its executable as ``kb-core``, so uv treated
    ``python`` as a missing ``kb-core`` command and the probe silently failed
    (the ``2>/dev/null`` swallowed uv's "use --from" hint), leaving PYTHON on a
    kb-core-less system interpreter. The probe now runs
    ``uv tool run --from kb-core python -c ...``. Both the old (removed) and new
    (added) forms match here.
    """
    return "uv tool run" in line and "kb-core python" in line
...
def _is_semantic_cache_scope_fix_line(line: str) -> bool:
    ...
    return (
        ...
        and ".kb_core_uncached.txt" in stripped
...
def _is_community_label_export_fix_line(line: str) -> bool:
    ...
    ``.kb_core_labels.json`` sidecar carried the names). Step 5 now imports
    ...
    return (
        ...
        or stripped == "from kb_core.export import to_json"
...
def _rebrand_legacy_text(text: str) -> str:
    """Map immutable v8 content to current public and Python identities."""
    text = text.replace(_LEGACY_BRAND, "KB Core").replace(_LEGACY_ENV, "KB_CORE")
    text = text.replace(f"{_LEGACY_PACKAGE}y", "kb-core")
    text = text.replace(f"import {_LEGACY_PACKAGE}", "import kb_core")
    text = text.replace(f"-m {_LEGACY_PACKAGE}", "-m kb_core")
    text = text.replace(f"{_LEGACY_PACKAGE}_", "kb_core_")
    text = text.replace(f"{_LEGACY_PACKAGE}/", "kb_core/")
    text = re.sub(rf"\b{_LEGACY_PACKAGE}\.(?=[A-Za-z_])", "kb_core.", text)
    return text.replace(_LEGACY_PACKAGE, "kb-core").replace("kb-core binary", "kb_core binary")
...
def monolith_roundtrip(platform: Platform) -> list[str]:
    ...
    original_lines = [
        l for l in _rebrand_legacy_text(_normalise(_git_show(platform.roundtrip_ref))).splitlines()
...
def _always_on_constants(ref: str) -> dict[str, str]:
    ...
    return {name: _rebrand_legacy_text(value) for name, value in out.items()}
...
def always_on_roundtrip() -> list[str]:
    ...
    for basename, const_name in sorted(ALWAYS_ON_BLOCKS.items()):
        path = f"kb_core/always_on/{basename}.md"
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        ...
        description="Render and guard kb-core's committed skill artifacts.",
    )
