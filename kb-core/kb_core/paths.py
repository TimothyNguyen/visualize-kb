"""Single source of truth for the kb-core output-directory name.

The output directory is ``kb-core-out`` by default and overridable with the
``KB_CORE_OUT`` env var (worktrees or shared-output setups, #686). It accepts a
relative name (``"kb-core-out-feature"``) or an absolute path
(``"/shared/kb-core-out"``).

This used to be duplicated as an identical ``_KB_CORE_OUT`` constant in
``__main__``, ``cache``, and ``watch``, while ``security`` and ``callflow_html``
hardcoded the literal ``"kb-core-out"`` and silently ignored the override
...
``KB_CORE_OUT`` before the process starts (the normal worktree/shared-output
...
KB_CORE_OUT = os.environ.get("KB_CORE_OUT", "kb-core-out")
...
def _atomic_replace(path: "str | Path", write_fn) -> None:
    ...
    try:
        ...
        try:
            ...
            # back to copy-then-delete, matching kb_core.cache's atomic writer.
            import shutil
...
# Bare directory name even when KB_CORE_OUT is an absolute path. Used by path
# guards that walk parents looking for the output directory by name.
KB_CORE_OUT_NAME = os.path.basename(os.path.normpath(KB_CORE_OUT))


def out_path(*parts: str) -> Path:
    """A path inside the configured output dir, e.g. ``out_path("cache")``.

    ``Path(KB_CORE_OUT) / ...`` resolves correctly for both a relative name
    ("kb-core-out") and an absolute override ("/shared/kb-core-out").
    """
    return Path(KB_CORE_OUT, *parts)
...
def default_graph_json() -> str:
    ...
    commands so a ``KB_CORE_OUT`` override is honoured everywhere, not just where
...
def stem_filename_budget(output_dir: "str | Path", *, reserve: int = 0, limit: int = 200) -> int:
    ...
    ``C:\\Users\\me\\projects\\svc\\kb-core-out\\obsidian`` exceeds MAX_PATH and
...
def load_node_link_graph(path_or_data):
    """Load a kb-core graph.json into a networkx graph, accepting both writers.
    ...
    if not isinstance(data, dict):
        p = Path(data)
        from kb_core.security import check_graph_file_size_cap  # lazy: security imports paths
