...
from kb_core.security import safe_fetch, safe_fetch_text, validate_url
...
def ingest(url: str, target_dir: Path, author: str | None = None, contributor: str | None = None) -> Path:
    """
    Fetch a URL and save it into target_dir as a kb-core-ready file.
    ...
    try:
        ...
        if url_type == "youtube":
            from kb_core.transcribe import download_audio
...
def save_query_result(
    ...
    Files are stored in memory_dir (typically kb-core-out/memory/) with YAML frontmatter
    that kb-core's extractor reads as node metadata. This closes the feedback loop:
    ...
    signals: they are written both to the frontmatter (so `kb-core reflect` can
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Fetch a URL into a kb-core /raw folder")
