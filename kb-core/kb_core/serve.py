...
def _load_graph(graph_path: str) -> nx.Graph:
    try:
        ...
        try:
            ...
            if _legacy(data.get("nodes", [])):
                print(
                    ...
                    "rebuild with `kb-core extract --force` for path-qualified IDs.",
                    file=sys.stderr,
                )
        except Exception:
            pass
        ...
        print(f"error: graph.json is corrupted ({exc}). Re-run /kb-core to rebuild.", file=sys.stderr)
        sys.exit(1)
...
def _max_server_contexts() -> int:
    ...
    ``KB_CORE_MAX_CONTEXTS`` overrides the default. Invalid or blank values
    ...
    """
    raw = os.environ.get("KB_CORE_MAX_CONTEXTS", "").strip()
    if not raw:
        return 8
    try:
...
def _subgraph_to_text(G: nx.Graph, nodes: set[str], edges: list[tuple], token_budget: int = 2000, *, seeds: list[str] | None = None) -> str:
    ...
    for nid in ordered:
        ...
        # corpus document can otherwise inject ANSI escapes, fake kb_core-out
...
def _display_graph_path(graph_path: str) -> str:
    ...
    Relative to the CWD when it sits underneath it — `kb-core-out/graph.json`,
...
def _build_server(graph_path: str):
    ...
    try:
        ...
        raise ImportError('mcp not installed. Run: pip install "kb-core[mcp]"') from e
    try:
    ...
    from kb_core import paths as _paths
    ...
    def _resolve_graph_path(project_path) -> str:
        ...
        resolves to ``<project_path>/<KB_CORE_OUT>/graph.json``, honouring the
        KB_CORE_OUT override so worktree/shared-output setups keep working."""
        if not project_path:
            return _default_graph_path
        return str(Path(project_path) / _paths.KB_CORE_OUT / "graph.json")
        ...
        for _t in _tools:
            ...
            _schema.setdefault("properties", {})["project_path"] = {
                ...
                "description": (
                    ...
                    "kb-core-out/graph.json. Optional — defaults to the graph "
    ...
    def _tool_list_prs(arguments: dict) -> str:
        from kb_core.prs import fetch_prs, fetch_worktrees, format_prs_text, _detect_default_branch
    ...
    def _tool_get_pr_impact(arguments: dict) -> str:
        from kb_core.prs import fetch_pr_files, compute_pr_impact, _gh, _parse_ci
    ...
    def _tool_triage_prs(arguments: dict) -> str:
        ...
        from kb_core.prs import fetch_prs, fetch_worktrees, fetch_pr_files, compute_pr_impact, _STATUS_ORDER, _detect_default_branch
        repo = arguments.get("repo") or None
        base = arguments.get("base") or _detect_default_branch(repo)
        try:
            prs = fetch_prs(repo=repo, base=base)
    ...
    def _load_community_labels() -> dict[int, str]:
        labels_path = Path(active_graph_path).parent / ".kb_core_labels.json"
    ...
    async def list_resources() -> list[types.Resource]:
        ...
        return [
            types.Resource(uri="kb-core://report", name="Graph Report", description="Full GRAPH_REPORT.md", mimeType="text/markdown"),
            types.Resource(uri="kb-core://stats", name="Graph Stats", description="Node/edge/community counts and confidence breakdown", mimeType="text/plain"),
            types.Resource(uri="kb-core://god-nodes", name="God Nodes", description="Top 10 most-connected nodes", mimeType="text/plain"),
            types.Resource(uri="kb-core://surprises", name="Surprising Connections", description="Cross-community surprising connections", mimeType="text/plain"),
            types.Resource(uri="kb-core://audit", name="Confidence Audit", description="EXTRACTED/INFERRED/AMBIGUOUS edge breakdown", mimeType="text/plain"),
            types.Resource(uri="kb-core://questions", name="Suggested Questions", description="Suggested questions for this codebase", mimeType="text/plain"),
        ]

    async def read_resource(uri: AnyUrl) -> str:
        _select_graph(None)  # resources read the server's default graph
        uri_str = str(uri)
        if uri_str == "kb-core://report":
            report_path = Path(active_graph_path).parent / "GRAPH_REPORT.md"
            if report_path.exists():
                return report_path.read_text(encoding="utf-8")
            return "GRAPH_REPORT.md not found. Run kb-core extract first."
        if uri_str == "kb-core://stats":
            return _tool_graph_stats({})
        if uri_str == "kb-core://god-nodes":
            return _tool_god_nodes({"top_n": 10})
        if uri_str == "kb-core://surprises":
            try:
                from kb_core.analyze import surprising_connections
        ...
        if uri_str == "kb-core://questions":
            try:
                from kb_core.analyze import suggest_questions
    ...
    if hasattr(Server, "list_tools"):
        ...
        async def _on_read_resource(ctx, params) -> types.ReadResourceResult:
            ...
            mime = "text/markdown" if str(params.uri).startswith("kb-core://report") else "text/plain"
    ...
    *,
    ...
    try:
        ...
        raise ImportError(
            ...
            'Run: pip install "kb-core[mcp]"'
        ) from e

    # A blank key (e.g. --api-key "" or an empty KB_CORE_API_KEY) must not be
...
def serve_http(
    ...
    *,
    ...
    ``api_key`` (or the ``KB_CORE_API_KEY`` env var) enables a simple header
    ...
    print(
        f"kb-core MCP server (streamable-http) on http://{host}:{port}{path} - {auth_note}",
        file=sys.stderr,
    )
    if host in ("0.0.0.0", "::", "") and not api_key:
        print(
            f"WARNING: binding {host or '0.0.0.0'} with no api-key exposes the graph "
            "unauthenticated on the network. Set --api-key (or KB_CORE_API_KEY).",
            file=sys.stderr,
        )
...
def _main(argv: list[str] | None = None) -> None:
    ...
    parser = argparse.ArgumentParser(
        prog="python -m kb_core.serve",
        description="Serve a kb-core knowledge graph over MCP (stdio or Streamable HTTP).",
    )
    parser.add_argument(
        "graph_path",
        nargs="?",
        default=None,
        help="Path to graph.json (default: kb-core-out/graph.json)",
    )
    parser.add_argument(
        "--graph",
        dest="graph_flag",
    ...
    parser.add_argument(
        "--api-key",
        default=os.environ.get("KB_CORE_API_KEY"),
        help="Require this key on the HTTP transport (env: KB_CORE_API_KEY)",
    )
