    ...
    if _GLOBAL_MANIFEST.exists():
        try:
            ...
            try:
                ...
                print(
                    f"[kb-core global] manifest at {_GLOBAL_MANIFEST} failed to parse ({exc}); "
...
def global_add(source_path: Path, repo_tag: str) -> dict:
    ...
    if existing_path and existing_path != str(source_path.resolve()):
        print(
            f"[kb-core global] warning: repo tag '{repo_tag}' previously pointed to "
