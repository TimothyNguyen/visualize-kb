"""kb-core command dispatch — every non-install subcommand.
...
module. The path-redirect (`kb-core <path>` -> extract) re-enters via a lazy
...
_SEARCH_NUDGE = json.dumps({
    "hookSpecificOutput": {
        ...
        "additionalContext": (
            'MANDATORY: kb-core-out/graph.json exists. You MUST run '
            '`kb-core query "<question>"` before grepping raw files. Only grep '
            'after kb-core has oriented you, or to modify/debug specific lines.'
        ),
    }
...
_READ_NUDGE = json.dumps({
    "hookSpecificOutput": {
        "hookEventName": "PreToolUse",
        "additionalContext": (
            'MANDATORY: kb-core-out/graph.json exists. You MUST run kb-core '
            'before reading source files. Use: `kb-core query "<question>"` '
            '(scoped subgraph), `kb-core explain "<concept>"`, or '
            '`kb-core path "<A>" "<B>"`. Only read raw files after kb-core has '
...
_READ_NUDGE_STALE = json.dumps({
    "hookSpecificOutput": {
        "hookEventName": "PreToolUse",
        "additionalContext": (
            'kb-core-out/graph.json exists but may be STALE for this file (the file '
            'changed after the last build). Prefer `kb-core query "<question>"` for '
            'orientation, and run `kb-core update` to refresh the graph. Reading the '
...
_READ_DENY = json.dumps({
    "hookSpecificOutput": {
        ...
        "permissionDecisionReason": (
            'kb-core strict mode: this project has a fresh knowledge graph that covers '
            'this file. Run `kb-core query "<your question>"` (or `kb-core explain` / '
            '`kb-core path`) FIRST to orient yourself, then re-issue this Read — it '
...
def _stamped_manifest_files(
    ...
    Mirrors the #1890 path normalization in kb_core.llm.
...
def _stale_graph_sources(
    ...
    (.kb_coreignore/.gitignore/--exclude changed) without being listed in the
    ...
    try:
        ...
        root_res = scan_root
    # <out>/kb_core-out/graph.json — relative source_files may be anchored here.
    ...
    def _provably_excluded(c: Path) -> bool:
        ...
        for s in spellings:
            ...
            if any(s.startswith(pref) for pref in excluded_prefixes):
                return True
        try:
            from kb_core.detect import _is_sensitive as _det_sensitive
            if _det_sensitive(c):
                return True
        except Exception:
            pass
    ...
    if kept_alive:
        print(
            f"[kb-core] fail-closed: kept node(s) from {len(kept_alive)} "
...
def _zero_node_stamped_code_sources(
    ...
    documented recovery was deleting kb-core-out/. A stamped file that the
    ...
    try:
        ...
        root_res = scan_root
    # <out>/kb_core-out/graph.json — legacy relative source_files may be
    ...
    from kb_core.extract import _get_extractor
    healed: list[str] = []
...
def _prune_graph_json_sources(graph_path: Path, stale_sources: list[str]) -> int:
    ...
    from kb_core.export import backup_if_protected as _backup
...
class _StageTimer:
    ...
    Monotonic (perf_counter), diagnostic-only: emits ``[kb-core timing] <stage>:
    ...
    def mark(self, stage: str) -> None:
        now = self._now()
        if self.enabled:
            print(f"[kb-core timing] {stage}: {now - self._last:.1f}s", file=sys.stderr)
        self._last = now

    def total(self) -> None:
        if self.enabled:
            print(f"[kb-core timing] total: {self._now() - self.start:.1f}s", file=sys.stderr)
def _enforce_graph_size_cap_or_exit(gp: Path) -> None:
    """Reject oversized graph files before parsing (CLI exit-on-fail flavor).

    Delegates to ``kb_core.security.check_graph_file_size_cap`` and turns the
...
def _touch_query_stamp(graph_path: "Path") -> None:
    """Record that kb-core oriented the agent recently, next to the queried graph.
...
def _query_stamp_fresh() -> bool:
    """True if a query/explain/path ran within KB_CORE_HOOK_STRICT_TTL (default
    1800s) — recent orientation, so strict mode does not block this read."""
    from kb_core.paths import out_path
    try:
        ttl = float(os.environ.get("KB_CORE_HOOK_STRICT_TTL", "1800"))
...
def _run_hook_guard(kind: str, strict: bool = False) -> None:
    ...
    exists, nudges the agent to use kb-core instead of grepping/reading raw files.
    ...
    in-project, fresh code per session is DENIED with a redirect to `kb-core query`
    ...
    try:
        if kind == "search":
            ...
            under_out = "kb-core-out/" in j or (KB_CORE_OUT_NAME.lower() + "/") in j
...
def _clone_repo(
    ...
    Clones into ~/.kb-core/repos/<owner>/<repo> by default so repeated
    ...
    if out_dir:
        ...
        dest = Path.home() / ".kb-core" / "repos" / owner / repo
...
def dispatch_command(cmd: str) -> None:
    if cmd == "provider":
        from kb_core.llm import _custom_providers_path, BACKENDS
        ...
        if subcmd == "list":
            ...
            if not name:
                print("Usage: kb-core provider show <name>", file=sys.stderr)
                sys.exit(1)
            existing = {}
            if global_path.is_file():
                try:
            ...
            if not name:
                print("Usage: kb-core provider add <name> --base-url URL --default-model MODEL --env-key KEY", file=sys.stderr)
                sys.exit(1)
            if name in BACKENDS:
            ...
            if not base_url or not default_model or not env_key:
                ...
                sys.exit(1)
            from kb_core.llm import provider_base_url_ok
            ...
            print(f"Provider '{name}' added. Use with: kb-core extract . --backend {name}")

        elif subcmd == "remove":
            name = sys.argv[3] if len(sys.argv) > 3 else ""
            if not name:
                print("Usage: kb-core provider remove <name>", file=sys.stderr)
                sys.exit(1)
            existing = {}
            if global_path.is_file():
                try:
            ...
            print("Usage: kb-core provider [add|list|show|remove]", file=sys.stderr)
            if subcmd:
                sys.exit(1)
    elif cmd == "prs":
        from kb_core.prs import cmd_prs
        cmd_prs(sys.argv[2:])
    elif cmd == "hook":
        from kb_core.hooks import (
        ...
        if subcmd == "install":
            ...
            print("Usage: kb-core hook [install|uninstall|status]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "query":
        if len(sys.argv) < 3:
        ...
        try:
            ...
            # per-edge below (mirrors kb_core/build.py's _src/_tgt pattern)
            ...
            try:
                from kb_core.build import graph_has_legacy_ids as _legacy
                if _legacy(_raw.get("nodes", [])):
                    print(
                        "[kb-core] note: this graph uses the pre-#1504 node-ID scheme; "
                        "rebuild with `kb-core extract --force` to get path-qualified IDs "
        ...
        # (#2706). The graph is written to <root>/<KB_CORE_OUT_NAME>/graph.json,
        ...
        from kb_core.affected import load_graph
        from kb_core.analyze import god_nodes as _god_nodes
        from kb_core.security import sanitize_label as _sanitize_label
        graph_path = _default_graph_path()
        top_n = 10
        ...
        # kb_core save-result --question Q --answer A [--type T] [--nodes N1 N2 ...]
        #                      [--outcome useful|dead_end|corrected] [--correction TEXT]
        import argparse as _ap

        p = _ap.ArgumentParser(prog="kb-core save-result")
        ...
        p.add_argument("--memory-dir", default=str(Path(_KB_CORE_OUT) / "memory"))
        ...
        from kb_core.ingest import save_query_result as _sqr
        ...
        p = _ap.ArgumentParser(prog="kb-core reflect")
        p.add_argument("--memory-dir", default=str(Path(_KB_CORE_OUT) / "memory"))
        p.add_argument(
            "--out",
            default=str(Path(_KB_CORE_OUT) / "reflections" / "LESSONS.md"),
        )
        ...
        opts = p.parse_args(sys.argv[2:])
        from kb_core.reflect import reflect as _reflect, lessons_fresh as _lessons_fresh

        graph_arg = opts.graph
        if graph_arg is None:
            default_graph = Path(_KB_CORE_OUT) / "graph.json"
        ...
        if _gp is not None:
            _analysis_path = Path(opts.analysis) if opts.analysis else (
                _gp.parent / ".kb_core_analysis.json")
            _labels_path = Path(opts.labels) if opts.labels else (
                _gp.parent / ".kb_core_labels.json")
        ...
        if len(sys.argv) < 4:
            ...
            sys.exit(1)
        from kb_core.serve import _pick_scored_endpoint, _score_nodes
        from networkx.readwrite import json_graph
        import networkx as _nx
        ...
        # Work-memory overlay: a derived experiential hint from `kb_core reflect`,
        # merged in display-only from the .kb_core_learning.json sidecar next to
        # graph.json. No line when the node has no overlay entry.
        try:
            from kb_core.reflect import load_learning_overlay as _llo
            from kb_core.security import sanitize_label as _sl
            _overlay = _llo(gp)
        ...
        if subcmd != "multigraph":
            print(
                "Usage: kb-core diagnose multigraph "
        ...
        from kb_core.diagnostics import (
            diagnose_file,
        ...
        if len(sys.argv) < 3:
            print(
                "Usage: kb-core add <url> [--author Name] [--contributor Name] [--dir ./raw]",
                file=sys.stderr,
            )
            sys.exit(1)
        from kb_core.ingest import ingest as _ingest
        ...
        try:
            ...
            print("Run /kb-core --update in your AI assistant to update the graph.")
        except Exception as exc:
            print(f"error: {exc}", file=sys.stderr)
            sys.exit(1)
        ...
        if not watch_path.exists():
            ...
            sys.exit(1)
        from kb_core.watch import watch as _watch
        ...
        # the configured backend, even when a .kb_core_labels.json already exists.
        ...
        if watch_path is None:
            watch_path = Path(".")
        graph_json = graph_override if graph_override is not None else watch_path / _KB_CORE_OUT / "graph.json"
        if not graph_json.exists():
            print(
                f"error: no graph found at {graph_json} — run /kb-core first",
                file=sys.stderr,
            )
            sys.exit(1)
        from networkx.readwrite import json_graph as _jg
        from kb_core.build import build_from_json
        from kb_core.cluster import cluster, score_all, remap_communities_to_previous
        from kb_core.analyze import (
            god_nodes,
            surprising_connections,
            suggest_questions,
        )
        from kb_core.report import generate
        from kb_core.export import to_json, to_html
        ...
        from kb_core.security import check_graph_file_size_cap as _check_cap
        try:
        ...
        # node-overlap so the existing .kb_core_labels.json keeps attaching
        ...
        # kb_core-out/ dir (another project/tenant's output), write beside it,
        # not into a stray kb_core-out/ in the CWD (#1747). But when `--graph`
        # points at an arbitrary path — e.g. a `backup/graph.json` archived
        # before re-clustering (#934) — fall back to the CWD's kb_core-out/,
        # which is the restore-into-place workflow that test pins. The default
        # (no --graph) case already has graph_json under watch_path/kb_core-out.
        _out_name = Path(_KB_CORE_OUT).name
        if graph_override is not None and graph_json.parent.name == _out_name:
            out = graph_json.parent
        else:
            out = watch_path / _KB_CORE_OUT
        out.mkdir(parents=True, exist_ok=True)
        labels_path = out / ".kb_core_labels.json"
        ...
        if labels_path.exists() and not force_relabel:
            ...
            if _ignored_label_flags:
                print(
                    f"[kb-core] warning: {'/'.join(_ignored_label_flags)} ignored: "
                    f"reusing saved labels at {labels_path}. "
                    f"Run `kb-core label` to relabel, or delete the labels file.",
                    file=sys.stderr,
                )
            ...
            # is told to `kb_core label` for fresh LLM names. Unchanged communities keep
            ...
            from kb_core.cluster import community_member_sigs, label_communities_by_hub
            ...
            # No labels file yet (or `kb_core label` forced a refresh). When run
            ...
            from kb_core.cluster import label_communities_by_hub
            from kb_core.llm import generate_community_labels
        ...
        if not _commit:
            from kb_core.watch import _git_head as _gh
        ...
        from kb_core.export import backup_if_protected as _backup
        from kb_core.exporters.html import _HTML_STALE_MARKER
        _backup(out)
        ...
        if not to_json(G, communities, str(out / "graph.json"),
            ...
            print(
                ...
                "GRAPH_REPORT.md, .kb_core_labels.json and .kb_core_analysis.json "
                "left untouched.",
                file=sys.stderr,
            )
            sys.exit(1)
        tokens = label_token_usage
        from kb_core.report import load_learning_for_report as _llfr
        ...
        (out / ".kb_core_analysis.json").write_text(
        ...
        if not placeholder_only:
            from kb_core.paths import write_json_atomic as _wja
            ...
            # (see reuse above).
            from kb_core.cluster import community_member_sigs as _cms
        ...
        if no_viz:
            ...
            try:
                from kb_core.exporters.html import _viz_node_limit
                viz_limit = _viz_node_limit()
                if viz_limit <= 0:
                    html_target.unlink(missing_ok=True)
                    _clear_html_stale_marker()
                    skip_reason = "KB_CORE_VIZ_NODE_LIMIT=0 disables HTML visualization"
                else:
        ...
        force = os.environ.get("KB_CORE_FORCE", "").lower() in ("1", "true", "yes")
        no_cluster = False
        args = sys.argv[2:]
        ...
        if watch_arg is not None:
            ...
            saved = Path(_KB_CORE_OUT) / ".kb_core_root"
            if saved.exists():
                ...
                # (#3028). Match the other .kb_core_root readers.
        ...
        # user sees their explicit `kb_core update` complete instead of
        ...
        if ok:
            print("Code graph updated. For doc/paper/image changes run /kb-core --update in your AI assistant.")
            if not (
                os.environ.get("GEMINI_API_KEY")
                or os.environ.get("GOOGLE_API_KEY")
                or os.environ.get("MOONSHOT_API_KEY")
                or os.environ.get("DEEPSEEK_API_KEY")
                or os.environ.get("KB_CORE_NO_TIPS")
            ):
        ...
        if len(sys.argv) < 3:
            print("Usage: kb-core check-update <path>", file=sys.stderr)
            sys.exit(1)
        from kb_core.watch import check_update
        ...
        from kb_core.tree_html import write_tree_html, DEFAULT_MAX_CHILDREN
        graph_path = Path(_KB_CORE_OUT) / "graph.json"
        ...
        while i_arg < len(args):
            ...
            if a == "--graph" and i_arg + 1 < len(args):
                ...
                print("Usage: kb-core tree [--graph PATH] [--output HTML]")
                print("  --graph PATH         path to graph.json (default kb-core-out/graph.json)")
                print("  --output HTML        output path (default kb-core-out/GRAPH_TREE.html)")
        ...
        # Usage: kb_core merge-driver %O %A %B  (set in .git/config merge driver)
        if len(sys.argv) < 5:
            print("Usage: kb-core merge-driver <base> <current> <other>", file=sys.stderr)
            sys.exit(1)
        ...
        if merged.number_of_nodes() > _MERGE_MAX_NODES:
            print(
                f"[kb-core merge-driver] merged graph has {merged.number_of_nodes()} nodes, "
        ...
        # kb_core merge-graphs graph1.json graph2.json ... --out merged.json
        args = sys.argv[2:]
        graph_paths: list[Path] = []
        out_path = Path(_KB_CORE_OUT) / "merged-graph.json"
        i = 0
        while i < len(args):
        ...
        if len(graph_paths) < 2:
            print(
                "Usage: kb-core merge-graphs <graph1.json> <graph2.json> [...] [--out merged.json]",
                file=sys.stderr,
            )
            sys.exit(1)
        import networkx as _nx
        from networkx.readwrite import json_graph as _jg
        from kb_core.build import prefix_graph_for_global as _prefix, distinct_repo_tags as _repo_tags
        graphs = []
        for gp in graph_paths:
            ...
            # Normalize edges/links key before loading — kb_core writes "links"
        ...
        # (the kb_core default) by converting MultiGraphs with nx.Graph().
        def _to_simple(g: "_nx.Graph") -> "_nx.Graph":
            ...
            if type(g) is not _nx.Graph:
                return _nx.Graph(g)
            return g
        # Unique repo tag per graph. The bare `kb_core-out/..` dir name is not
        # unique across inputs (src/kb_core-out and frontend/src/kb_core-out both
        ...
        from kb_core.cross_repo_types import link_shared_type_declarations as _link_shared
        ...
        if collected_hyperedges:
            from kb_core.export import attach_hyperedges as _attach
        ...
        if len(sys.argv) < 3:
            print(
                "Usage: kb-core clone <github-url> [--branch <branch>] [--out <dir>]",
                file=sys.stderr,
            )
            sys.exit(1)
        url = sys.argv[2]
        ...
        if subcmd not in ("html", "callflow-html", "obsidian", "wiki", "svg", "graphml", "neo4j", "falkordb"):
            print("Usage: kb-core export <format>", file=sys.stderr)
        ...
        labels_path = Path(_KB_CORE_OUT) / ".kb_core_labels.json"
        labels_path_explicit = False
        report_path = Path(_KB_CORE_OUT) / "GRAPH_REPORT.md"
        ...
        analysis_path = Path(_KB_CORE_OUT) / ".kb_core_analysis.json"
        node_limit = 5000
        no_viz = False
        obsidian_dir = Path(_KB_CORE_OUT) / "obsidian"
        ...
        while i < len(args):
            ...
            if a == "--graph" and i + 1 < len(args):
                ...
                print("Usage: kb-core export callflow-html [GRAPH|DIR] [--graph PATH] [--labels PATH]")
                print("  --report PATH          path to GRAPH_REPORT.md")
                print("  --sections PATH        JSON section definitions")
                print("  --output HTML          output path (default kb-core-out/<project>-callflow.html)")
                ...
                if candidate.name == "graph.json" or candidate.suffix.lower() == ".json":
                    ...
                    graph_path = candidate / _KB_CORE_OUT / "graph.json"
                graph_path_explicit = True
                i += 1
            else:
                i += 1
        ...
        if graph_path_explicit:
            ...
            if not labels_path_explicit:
                labels_path = graph_out_dir / ".kb_core_labels.json"
        ...
        if not graph_path.exists():
            print(f"error: graph not found: {graph_path}. Run /kb-core <path> first.", file=sys.stderr)
            sys.exit(1)

        if subcmd == "callflow-html":
            from kb_core.callflow_html import write_callflow_html as _write_callflow_html
        ...
        from networkx.readwrite import json_graph as _jg
        from kb_core.build import build_from_json as _bfj
        from kb_core.security import check_graph_file_size_cap as _check_cap
        ...
        # that happens, `kb_core export html` previously bailed with
        ...
        if subcmd == "html":
            from kb_core.export import to_html as _to_html
            if no_viz:
            ...
            from kb_core.export import to_obsidian as _to_obsidian, to_canvas as _to_canvas
            ...
            from kb_core.wiki import to_wiki as _to_wiki
            from kb_core.analyze import god_nodes as _god_nodes
            if not communities:
                print(
                    "error: .kb_core_analysis.json is missing or empty — refusing to export wiki to prevent data loss.\n"
                    "Run `kb-core extract .` (or `kb-core cluster-only .`) to regenerate community data first.",
                    file=sys.stderr,
                )
                sys.exit(1)
            if not gods_data:
            ...
            if push_uri:
                ...
                from kb_core.export import to_cypher as _to_cypher
            ...
            if push_uri:
                from kb_core.export import push_to_falkordb as _push
                result = _push(G, uri=push_uri, user=push_user,
                               password=push_password, communities=communities)
                ...
                print(f"cypher.txt written ({out_dir}/cypher.txt) - statements are OpenCypher. "
                      ...
                      f"import), so load a graph with: kb-core export falkordb --push "
                      f"falkordb://localhost:6379")

    elif cmd == "benchmark":
        from kb_core.benchmark import run_benchmark, print_benchmark

        graph_path = sys.argv[2] if len(sys.argv) > 2 else _default_graph_path()
        _enforce_graph_size_cap_or_exit(Path(graph_path))
        # Try to load corpus_words from detect output
        corpus_words = None
        detect_path = Path(".kb_core_detect.json")
        ...
        if subcmd == "add":
            # kb_core global add <graph.json> [--as <tag>]
            args = sys.argv[3:]
            source = None
            tag = None
            i = 0
            while i < len(args):
                if args[i] == "--as" and i + 1 < len(args):
                    ...
                    i += 1
            if not source:
                print("Usage: kb-core global add <graph.json> [--as <repo-tag>]", file=sys.stderr)
                sys.exit(1)
            ...
            if not tag:
                print("Usage: kb-core global remove <repo-tag>", file=sys.stderr); sys.exit(1)
            try:
            ...
            repos = _global_list()
            if not repos:
                print("Global graph is empty. Use 'kb-core global add' to add a project.")
            else:
            ...
            print("Usage: kb-core global [add|remove|list|path]", file=sys.stderr); sys.exit(1)
        ...
        if len(sys.argv) < 3:
            print(
                "Usage: kb-core extract <path> [--backend gemini|kimi|claude|openai|deepseek|ollama] "
        ...
        # --force parity with `kb_core update`: the flag or KB_CORE_FORCE=1
        ...
        while i < len(args):
            ...
            if a == "--backend" and i + 1 < len(args):
                ...
                # before, and `kb_core tree` already documents --output, so the
        ...
        if deep_mode:
            print("[kb-core extract] deep mode enabled: richer semantic extraction")

        # CLI flag wins over env var. Setting KB_CORE_API_TIMEOUT here so
        # _call_openai_compat picks it up without needing a new kwarg path.
        if cli_api_timeout is not None:
            os.environ["KB_CORE_API_TIMEOUT"] = str(cli_api_timeout)
        if cli_max_workers is not None:
            os.environ["KB_CORE_MAX_WORKERS"] = str(cli_max_workers)

        # Resolve output dir. The user-facing contract is "<out>/kb_core-out/"
        # so a fresh checkout writes kb_core-out/ at the project root, matching
        # the skill.md pipeline.
        out_root = (out_dir.resolve() if out_dir else target)
        kb_core_out = out_root / _KB_CORE_OUT
        kb_core_out.mkdir(parents=True, exist_ok=True)
        # Persist corpus-shaping options so later update/watch/hook rebuilds
        # use the same file set as the initial extraction (#1886).
        from kb_core.watch import (
        ...
        # flag-less `kb_core extract` must NOT clobber it back to True, which
        ...
        _effective_gitignore = False if no_gitignore else _read_build_gi(kb_core_out)
        # An explicit list replaces the persisted one; omission reuses it.
        _effective_excludes = cli_excludes or _read_build_ex(kb_core_out)
        _write_build_cfg(
            kb_core_out,
        ...
        from kb_core.detect import (
            detect as _detect,
            detect_incremental as _detect_incremental,
            save_manifest as _save_manifest,
        )
        manifest_path = kb_core_out / "manifest.json"
        existing_graph_path = kb_core_out / "graph.json"
        ...
        if force and code_only and existing_graph_path.exists():
            ...
            print(
                "[kb-core extract] --force --code-only: full AST re-scan, "
                "existing semantic layer preserved (no semantic pass this run)"
            )
        if force:
            print("[kb-core extract] --force: full re-scan, semantic cache reads skipped")
        elif incremental_mode and not manifest_path.exists():
            print(
                "[kb-core extract] manifest.json missing; using existing "
        ...
        if not has_path:
            ...
            print(f"[kb-core extract] incremental scan of {target}")
            ...
            if _healed_sources:
                print(
                    f"[kb-core extract] re-queuing {len(_healed_sources)} "
            ...
            if _healed_sem_sources:
                print(
                    f"[kb-core extract] re-queuing {len(_healed_sem_sources)} "
            ...
            print(f"[kb-core extract] scanning {target}")
            detection = _detect(
                target,
                google_workspace=google_workspace or None,
                extra_excludes=_effective_excludes or None,
        ...
        if code_only and semantic_files:
            print(
                f"[kb-core extract] --code-only: skipping {len(semantic_files)} "
        ...
        if deep_mode and incremental_mode and not code_only:
            ...
            if len(_deep_all) != len(semantic_files):
                print(
                    f"[kb-core extract] deep mode: widening semantic pass from "
        ...
        if incremental_mode:
            ...
            print(
                f"[kb-core extract] {len(code_files)} code, {len(doc_files)} docs, "
            ...
            print(
                f"[kb-core extract] found {len(code_files)} code, "
        ...
        # are no longer invisible in kb_core's own output (#1692).
        ...
        if _unclassified:
            ...
            print(
                f"[kb-core extract] {len(_unclassified)} file(s) not classified "
        ...
        if _sec:
            ...
            print(
                f"[kb-core extract] {len(_sec)} file(s) skipped as potentially sensitive "
        ...
        if needs_llm:
            if backend is None:
                ...
                sys.exit(1)
            if backend == "ollama":
                from kb_core.llm import _validate_ollama_base_url
        ...
        if code_files:
            from kb_core.extract import extract as _ast_extract
            # Anchor the cache at the output root, not the scanned project:
            # with --out, a <target>/kb_core-out/cache/ would leak a
            # kb_core-out/ dir into a project that asked for external output.
            ...
            # #2437/#2438 (the `kb_core update` twin of watch's #2406 fix): an
            ...
            if incremental_mode and existing_graph_path.exists():
                ...
                try:
                    from kb_core.build import _is_ast_tier as _ctx_is_ast_tier
                    from kb_core.security import (
            ...
            print(f"[kb-core extract] AST extraction on {len(code_files)} code files...")
            try:
                ast_result = _ast_extract(code_files, **ast_kwargs)
            except Exception as exc:
                print(f"[kb-core extract] AST extraction failed: {exc}", file=sys.stderr)
        ...
        from kb_core.llm import _extraction_system as _sem_prompt_for
        ...
        if semantic_files:
            ...
            if sem_cache_hits:
                print(f"[kb-core extract] semantic cache: {sem_cache_hits} hit / {sem_cache_misses} miss")

            if uncached_paths:
                print(f"[kb-core extract] semantic extraction on {len(uncached_paths)} files via {backend}...")
                corpus_kwargs: dict = {
                    "backend": backend,
                    "model": model,
                    "root": target,
                ...
                def _progress(idx: int, total: int, _result: dict) -> None:
                    ...
                    print(
                        f"[kb-core extract] chunk {idx + 1}/{total} done",
                        flush=True,
                    )
                ...
                try:
                    ...
                    print(
                        f"[kb-core extract] semantic extraction failed: {exc}",
                        file=sys.stderr,
                    )
                ...
                if uncached_paths and _chunk_stats["succeeded"] == 0:
                    print(
                        f"[kb-core extract] error: all semantic chunks failed "
                ...
                if _dropped_files:
                    print(
                        f"[kb-core extract] dropped {_dropped_items} out-of-scope "
                ...
                try:
                    ...
                    print(f"[kb-core extract] warning: could not write semantic cache: {exc}", file=sys.stderr)
        ...
        try:
            from kb_core.cache import file_hash as _file_hash
            ...
            print(f"[kb-core extract] warning: could not prune semantic cache: {exc}", file=sys.stderr)
        stages.mark("semantic extract")

        pg_result: dict = {"nodes": [], "edges": []}
        if cli_postgres_dsn is not None:
            from kb_core.pg_introspect import introspect_postgres
            print(f"[kb-core extract] introspecting PostgreSQL schema...")
            try:
                pg_result = introspect_postgres(cli_postgres_dsn)
            except (ConnectionError, ImportError) as exc:
                print(f"error: {exc}", file=sys.stderr)
                sys.exit(1)
            print(f"[kb-core extract] PostgreSQL: {len(pg_result['nodes'])} nodes, "
                  f"{len(pg_result['edges'])} edges")

        cargo_result: dict = {"nodes": [], "edges": []}
        if cli_cargo:
            from kb_core.cargo_introspect import introspect_cargo
            print("[kb-core extract] introspecting Cargo workspace...")
            try:
                cargo_result = introspect_cargo(target)
            except (ConnectionError, ImportError, OSError) as exc:
                print(f"error: {exc}", file=sys.stderr)
                sys.exit(1)
            print(f"[kb-core extract] Cargo: {len(cargo_result['nodes'])} nodes, "
        ...
        graph_json_path = kb_core_out / "graph.json"
        analysis_path = kb_core_out / ".kb_core_analysis.json"
        ...
        if no_cluster:
            ...
            from kb_core.build import dedupe_edges as _dedupe_edges, dedupe_nodes as _dedupe_nodes
            ...
            if (
                ...
                if graph_stale_sources:
                    ...
                    if _n_pruned:
                        print(
                            f"[kb-core extract] pruned {_n_pruned} node(s) from "
                            f"{len(graph_stale_sources)} source file(s) no longer "
                            "in the scan (deleted or excluded)."
                        )
                print(
                    "[kb-core extract] no incremental changes detected "
                ...
                try:
                    ...
                    print(f"[kb-core extract] warning: could not write manifest: {exc}", file=sys.stderr)
                stages.total()
                sys.exit(0)
            ...
            if incremental_mode:
                ...
                from kb_core.build import merge_raw_extraction as _merge_raw_extraction
            ...
            from kb_core.build import disambiguate_file_labels_in_nodes as _disamb_labels
            ...
            if _extraction_incomplete and not cli_allow_partial:
                from kb_core.export import MALFORMED_GRAPH as _MALFORMED_GRAPH
                ...
                if _malformed or _shrinks:
                    ...
                    print(
                        "[kb-core extract] error: extraction was incomplete (an AST/"
            ...
            try:
                ...
                (kb_core_out / ".kb_core_root").write_text(
            ...
            try:
                ...
                print(f"[kb-core extract] warning: could not write manifest: {exc}", file=sys.stderr)
            if global_merge:
                from kb_core.global_graph import global_add as _global_add
                _tag = global_repo_tag or target.name
                try:
                    result = _global_add(kb_core_out / "graph.json", _tag)
                    if result["skipped"]:
                        print(f"[kb-core global] '{_tag}' unchanged since last add - skipped.")
                    else:
                        print(f"[kb-core global] '{_tag}' merged into global graph "
                              f"(+{result['nodes_added']} nodes, -{result['nodes_removed']} pruned).")
                except Exception as exc:
                    print(f"[kb-core global] warning: failed to merge into global graph: {exc}", file=sys.stderr)
            stages.total()
            sys.exit(0)
        ...
        from kb_core.cluster import cluster as _cluster, score_all as _score_all
        from kb_core.export import to_json as _to_json
        from kb_core.analyze import god_nodes as _god_nodes, surprising_connections as _surprising
        ...
        if incremental_mode:
            ...
            try:
                ...
                print(f"[kb-core extract] {exc}", file=sys.stderr)
                sys.exit(1)
        else:
        ...
        if G.number_of_nodes() == 0:
            print(
                "[kb-core extract] graph is empty — extraction produced no nodes. "
        ...
        # command was invoked from, so `kb_core extract <target>` run from
        ...
        if not _wrote:
            ...
            # re-attempts.
            print(
                "[kb-core extract] error: extraction was incomplete (an AST/semantic "
        ...
        if merged.get("output_tokens", 0) > 0:
            (kb_core_out / ".kb_core_semantic_marker").write_text(
        ...
        print(f"[kb-core extract] wrote {analysis_path}")
        if incremental_mode:
            _excl_note = f", {len(excluded_files)} excluded" if excluded_files else ""
            print(
                f"[kb-core extract] incremental summary: "
            ...
            print(f"[kb-core extract] semantic cache: {sem_cache_hits} cached, {sem_cache_misses} re-extracted")
        if merged["input_tokens"] or merged["output_tokens"]:
            print(
                f"[kb-core extract] tokens: "
                f"{merged['input_tokens']:,} in / "
        ...
        print(
            "[kb-core extract] next: run "
            f"`kb-core cluster-only {kb_core_out.parent}` "
            "to generate GRAPH_REPORT.md and name communities"
        )
        stages.total()

    elif cmd == "cache-check":
        # kb_core cache-check <files_from> [--root <dir>] [--mode <m> | --deep]
        #                       [--prompt-file <path>]
        ...
        # Writes:
        #   kb_core-out/.kb_core_cached.json   — already-cached nodes/edges/hyperedges
        #   kb_core-out/.kb_core_uncached.txt  — paths that need extraction
        # Stdout: "Cache: N hit, M miss"
        from kb_core.cache import check_semantic_cache
        if len(sys.argv) < 3:
            print("Usage: kb-core cache-check <files_from> [--root <dir>] "
        ...
        out = root / _KB_CORE_OUT
        out.mkdir(parents=True, exist_ok=True)
        if cached_nodes or cached_edges or cached_hyperedges:
            (out / ".kb_core_cached.json").write_text(
                json.dumps({"nodes": cached_nodes, "edges": cached_edges, "hyperedges": cached_hyperedges},
                           ensure_ascii=False),
                encoding="utf-8",
            )
        (out / ".kb_core_uncached.txt").write_text("\n".join(uncached), encoding="utf-8")
        print(f"Cache: {len(files) - len(uncached)} hit, {len(uncached)} miss")

    elif cmd == "merge-chunks":
        # kb_core merge-chunks <chunk_glob_or_files...> --out <path>
        # Concatenates .kb_core_chunk_*.json files written by semantic subagents.
        # Deduplicates nodes by id (first writer wins). Sums token counts.
        import glob as _glob
        if len(sys.argv) < 3:
            print("Usage: kb-core merge-chunks <chunk_files...> --out <path>", file=sys.stderr)
            sys.exit(1)
        ...
        from kb_core.semantic_cleanup import load_validated_semantic_fragment
        for cf in chunk_files:
            chunk, _chunk_errs = load_validated_semantic_fragment(Path(cf))
            if _chunk_errs:
                print(
                    f"[kb-core merge-chunks] warning: skipping invalid chunk {cf}: "
        ...
        if not valid_chunks:
            print(
                f"[kb-core merge-chunks] error: no valid chunks to merge; "
        ...
        # kb_core merge-semantic --cached <path> --new <path> --out <path>
        ...
        if len(sys.argv) < 3:
            print("Usage: kb-core merge-semantic --cached <path> --new <path> --out <path>", file=sys.stderr)
            sys.exit(1)
        ...
        # User ran `kb_core <path>` directly — treat as `kb_core extract <path>`.
        # Common when following the PowerShell note in README (`kb_core .`) or
        ...
        print("Run 'kb-core --help' for usage.", file=sys.stderr)
        sys.exit(1)
