"""Markdown extractor. Moved verbatim from kb-core/extract.py."""
...
from kb_core.extractors.base import _file_stem, _make_id
...
# not a declared dependency of kb_core (see manifest_ingest._parse_apm for the
...
def _active_scan_root() -> "Path | None":
    ...
    fallback stays off.
    """
    import kb_core.extract as _extract
...
def _build_link_index(root: Path) -> "dict[str, list[tuple[int, str, Path]]]":
    ...
    index dot-folders.
    """
    from kb_core.detect import _SKIP_DIRS
