...
`kb_core/extract.py` is being split into this package, one language per PR
...
3. **Facade re-export is mandatory.** `extract.py` must keep exporting every
   moved name (`from kb_core.extractors.<mod> import extract_<lang>  # noqa: F401`
...
4. **Never import from `kb_core.extract` inside this package.** Import
...
- run `grep -c '_name' kb_core/extract.py` AFTER your candidate move;
...
2. `grep -n 'def extract_<lang>' kb_core/extract.py`; the span ends at the
...
3. Save the span to a temp file. Create `kb_core/extractors/<lang>.py` with
   module docstring (`"""<Lang> extractor. Moved verbatim from kb_core/extract.py."""`),
