"""kb-core prs — graph-aware PR dashboard.
...
Usage:
  kb-core prs                   # dashboard of all open PRs
  kb-core prs <number>          # deep dive on one PR
  kb-core prs --triage          # Opus ranks your review queue
  kb-core prs --worktrees       # show worktree → branch → PR mapping
  kb-core prs --conflicts       # PRs sharing graph communities (merge-order risk)
  kb-core prs --base <branch>   # filter to PRs targeting this base (default: v8)
"""
...
def render_dashboard(prs: list[PRInfo], base: str = "v8", show_wrong_base: bool = False) -> None:
    ...
    print()
    print(bold(f"  kb-core prs  ·  base: {base}  ·  {len(actionable)} PRs"))
    print()
...
def _resolve_triage_backend() -> tuple[str, str]:
    """Return (backend, model) using KB_CORE_TRIAGE_BACKEND or first available key."""
    from kb_core.llm import BACKENDS, _get_backend_api_key, _default_model_for_backend

    explicit = os.environ.get("KB_CORE_TRIAGE_BACKEND", "").strip()
    if explicit in BACKENDS:
        model = (os.environ.get("KB_CORE_TRIAGE_MODEL")
...
def triage_with_opus(prs: list[PRInfo], base: str) -> None:
    try:
        from kb_core.llm import BACKENDS, _get_backend_api_key
    except ImportError:
        print(red("  kb_core.llm not available - cannot run triage."), file=sys.stderr)
        sys.exit(1)
