...
# and resolves fine on every filesystem kb_core targets; stripping it would
...
def _community_article(
    ...
    lines += ["---", "", f"*Part of the kb-core knowledge wiki. See {_md_link('index', resolver)} to navigate.*"]
    return "\n".join(lines)
...
def _index_md(
    ...
    lines: list[str] = [
        ...
        "",
        "> Auto-generated by kb_core. Start here — read community articles for context, then drill into god nodes for detail.",
        "",
    ...
    lines += [
        "---",
        "",
        "*Generated by [kb-core](https://github.com/safishamsi/kb-core)*",
    ]
    return "\n".join(lines)
...
def to_wiki(
    ...
    if not communities:
        raise ValueError(
            ...
            "Run `kb-core extract .` or `kb-core cluster-only .` first."
        )
    ...
    if not communities:
        raise ValueError(
            ...
            "Re-run `kb-core extract .` to regenerate .kb_core_analysis.json."
        )
    ...
    # Windows a 200-char slug under an ordinary kb_core-out/wiki/ overruns
