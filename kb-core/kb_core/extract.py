...
# --- migrated to kb_core/extractors/ (see kb_core/extractors/MIGRATION.md) ---
from kb_core.extractors.base import (  # noqa: F401
    _LANGUAGE_BUILTIN_GLOBALS,
    _file_stem,
    _make_id,
    _read_text,
)
from kb_core.extractors.apex import extract_apex  # noqa: F401
from kb_core.extractors.bash import extract_bash  # noqa: F401
from kb_core.extractors.blade import extract_blade  # noqa: F401
from kb_core.extractors.csharp import (
    CsharpNameResolver,
    _resolve_cross_file_csharp_imports,
    _resolve_csharp_type_references,
)
from kb_core.extractors.dart import extract_dart  # noqa: F401
from kb_core.extractors.dm import extract_dm, extract_dmf, extract_dmi, extract_dmm  # noqa: F401
from kb_core.extractors.elixir import extract_elixir  # noqa: F401
from kb_core.extractors.fortran import _cpp_preprocess, extract_fortran  # noqa: F401
from kb_core.extractors.go import _GO_PREDECLARED_FUNCS, extract_go  # noqa: F401
from kb_core.extractors.json_config import extract_json  # noqa: F401
from kb_core.extractors.commonlisp import extract_commonlisp  # noqa: F401
from kb_core.extractors.markdown import extract_markdown, _MD_LINK_INDEX_CACHE  # noqa: F401
from kb_core.extractors.ocaml import extract_ocaml  # noqa: F401
from kb_core.extractors.pascal_forms import extract_delphi_form, extract_lazarus_form  # noqa: F401
from kb_core.extractors.powershell import extract_powershell, extract_powershell_manifest  # noqa: F401
from kb_core.extractors.razor import extract_razor  # noqa: F401
from kb_core.extractors.rust import extract_rust  # noqa: F401
from kb_core.extractors.sln import extract_sln  # noqa: F401
from kb_core.extractors.sql import extract_sql  # noqa: F401
from kb_core.extractors.terraform import extract_terraform  # noqa: F401
from kb_core.extractors.verilog import extract_verilog  # noqa: F401
from kb_core.extractors.zig import extract_zig  # noqa: F401
from kb_core.security import sanitize_metadata
from kb_core.paths import disambiguate_ambiguous_candidates

from kb_core.extractors.models import LanguageConfig, _JS_CACHE_BYPASS_SUFFIXES, _NamespaceExportFact, _StarExportFact, _SymbolAliasFact, _SymbolDeclarationFact, _SymbolExportFact, _SymbolImportFact, _SymbolResolutionFacts, _SymbolUseFact, _WORKSPACE_PACKAGE_CACHE  # noqa: E402,F401

from kb_core.extractors.resolution import (  # noqa: E402,F401
...
def _safe_extract(extractor: Callable, path: Path) -> dict:
    try:
        ...
        if os.environ.get("KB_CORE_DEBUG"):
            import traceback
...
# registry (framework lives in kb_core.resolver_registry). A new language plugs in
...
# kb_core.ruby_resolution; registered here as a second consumer of the framework.
register_language_resolver(
...
# scope. Lives in kb_core.pascal_resolution; registered here as a consumer
...
# graph unreachable from the call site. Lives in kb_core.csharp_dispatch;
...
# Extensions kb_core creates document file nodes for. A link to one of these
...
def _check_tree_sitter_version() -> None:
    ...
    if LANGUAGE_VERSION < 14:
        ...
        raise RuntimeError(
            ...
            f"kb-core requires tree-sitter >= 0.23.0 (Language API v2). "
...
def _xaml_csharp_class_nodes(path: Path) -> dict[str, list[dict]]:
    from kb_core.detect import _is_ignored, _is_noise_dir, _load_kb_coreignore
    ...
    patterns = _load_kb_coreignore(root)
...
def _extract_parallel(
    ...
    if max_workers is None:
        # Honour KB_CORE_MAX_WORKERS env override; otherwise scale to the
        ...
        env_raw = os.environ.get("KB_CORE_MAX_WORKERS", "").strip()
        env_cap = None
        if env_raw:
            try:
                v = int(env_raw)
    ...
    # KB_CORE_MAX_WORKERS, and --max-workers — stays valid on >61-core boxes
    ...
    # Windows post-commit hook exports KB_CORE_MAX_WORKERS=1, so this is the
...
def extract(
    ...
    *,
    ...
    Args:
        ...
        cache_root: explicit root for kb-core-out/cache/ (overrides the
            inferred common path prefix). Pass Path('.') when running on a
            subdirectory so the cache stays at ./kb-core-out/cache/.
        ...
        max_workers: max subprocess count. Defaults to cpu_count (or the
            value of KB_CORE_MAX_WORKERS if set), bounded by len(uncached_work).
    ...
    # was never observed again for the life of the process. `kb_core watch`
    ...
    # common parent of the inputs), which would drop kb_core-out/ inside a
    ...
    # (or the transient failure self-heals) without deleting kb_core-out/.
    ...
    from kb_core.detect import CODE_EXTENSIONS as _CODE_EXTS
    ...
    if _no_extractor:
        ...
        print(
            f"  warning: {_tot} file(s) are classified as code but kb-core has no AST "
    ...
    for _ext, _n in sorted(_missing_dep_count.items(), key=lambda kv: (-kv[1], kv[0])):
        ...
        if _extra and _DEP_MISSING_MARKER in _err_text:
            ...
            _hint = f' Install it with: pip install "kb-core[{_extra}]"'
    ...
    # single-definition god-node guard. Registered in kb_core.resolver_registry so
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python -m kb_core.extract <file_or_dir> ...", file=sys.stderr)
        sys.exit(1)
