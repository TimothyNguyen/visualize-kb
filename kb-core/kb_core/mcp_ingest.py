...
and turns the `mcpServers` map into KB Core nodes and edges.

Symmetry with `serve.py`: KB Core exposes itself AS an MCP server. This module
indexes MCP servers AS a corpus type, completing the loop — an agent that runs
kb-core with `--mcp` can now query its own configured MCP layer.

Entry point:
  extract_mcp_config(path: Path) -> dict[str, list[dict]]

  Returns `{"nodes": [...], "edges": [...]}` compatible with KB Core's
...
def extract_mcp_config(path: Path) -> dict[str, Any]:
    """Parse an MCP config file into KB Core nodes and edges.
...
# ── Node / edge construction (KB Core schema) ────────────────────────────────
...
# only kb_core.ids), so this can no longer drift from extract._file_stem.
from kb_core.extractors.base import _file_stem  # noqa: E402
