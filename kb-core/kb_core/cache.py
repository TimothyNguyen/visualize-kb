...
# Output directory name — override with KB_CORE_OUT env var for worktrees or
# shared-output setups. Accepts a relative name ("kb_core-out-feature") or an
# absolute path ("/shared/kb_core-out"). Single source of truth in kb_core.paths
# (#1423); re-exported here as _KB_CORE_OUT for the existing call sites.
...
try:
    ...
    _EXTRACTOR_VERSION = _pkg_version("kb-core")
except Exception:
...
def _cleanup_stale_ast_entries(ast_base: Path, current_dir: Path) -> None:
    """Remove AST cache entries left behind by other kb-core versions.
...
def prompt_fingerprint(prompt: "str | Path") -> str:
    ...
    its system prompt, :func:`kb_core.llm._extraction_system`) or a Path to the
...
# `kb_core extract --force` / `kb_core update --force` (or KB_CORE_FORCE=1)
...
# read anyway. Override with KB_CORE_MTIME_GRANULARITY_MS (0 disables the
...
def _mtime_granularity_ns() -> int:
    ...
    """
    raw = os.environ.get("KB_CORE_MTIME_GRANULARITY_MS", "").strip()
    if raw:
        try:
            ms = float(raw)
        except ValueError:
...
def _stat_sig_fresh(entry: object, st: "os.stat_result") -> bool:
    ...
    Entries written by an older kb-core carry no ``indexed_at_ns``; they are
...
def _stat_key_to_relative(key: str, anchor: Path) -> str:
    ...
    Local duplicate of :func:`kb_core.detect._to_relative_for_storage` —
...
def _stat_key_to_absolute(key: str, anchor: Path) -> str:
    ...
    written by an older kb-core remains readable.
    """
    p = Path(key)
    if p.is_absolute():
        return str(p)
...
def _ensure_stat_index(root: Path, cache_root: "Path | None" = None) -> None:
    ...
    # kb_core-out/ (#1747). _stat_index_anchor is the separate KEY anchor:
...
def file_hash(path: Path, root: Path = Path("."), cache_root: "Path | None" = None) -> str:
    ...
    # kb_core-out/cache/stat-index.json inside the analyzed source tree even when
...
def _relativize_source_files_in(payload: dict, root: Path) -> None:
    ...
    Mirror of :func:`kb_core.watch._relativize_source_files` so cached
    ...
    :func:`kb_core.detect._to_relative_for_storage`.
    """
    try:
...
def _normalize_source_file_value(src: "str | Path", root_resolved: Path) -> str:
    ...
    ``kb-core update`` runs with a machine-specific ``source_file`` (#2197).
...
def _id_anchor(path_str: str, rel_str: str) -> str:
    ...
    """
    from kb_core.ids import normalize_id  # ids imports only re/unicodedata: no cycle
...
def _absolutize_ids_in(payload: dict, path: "str | Path", root: Path) -> None:
    ...
    stored form (``$kb-core-root$_pkg_mod_base``) is a storage encoding, not a valid id,
...
def cache_dir(root: Path = Path("."), kind: str = "ast",
    ...
    AST entries live in kb-core-out/cache/ast/v{version}-s{schema}/, namespaced
    by kb-core version and cache-key schema because they depend on extractor
    code and key semantics, not just file contents. Semantic entries are still
    NOT version-namespaced (re-extraction
    costs LLM calls, #1252): they live in kb-core-out/cache/semantic/, with
    deep-mode entries beside them in kb-core-out/cache/semantic-deep/.
...
def load_cached(path: Path, root: Path = Path("."), kind: str = "ast",
    ...
    Cache value: stored as kb-core-out/cache/{kind}/{hash}.json (AST entries
    ...
    AST entries written by other kb-core versions — including the legacy
...
def save_cached(path: Path, result: dict, root: Path = Path("."), kind: str = "ast",
    ...
    Stores as kb-core-out/cache/{kind}/{hash}.json where hash = SHA256 of current file contents.
...
def cached_files(root: Path = Path(".")) -> set[str]:
    ...
    base = Path(root).resolve() / _KB_CORE_OUT / "cache"
    hashes: set[str] = set()
    # Legacy flat entries
    if base.is_dir():
...
def prune_semantic_cache(root: Path, live_hashes: set[str]) -> int:
    ...
    extraction-spec) can share one kb-core-out/, and a wholesale sweep would
...
def check_semantic_cache(
    ...
    if legacy:
        warnings.warn(
            ...
            "vintages. Re-run with --force (or KB_CORE_FORCE=1) to re-extract them "
    ...
    if corrupt:
        warnings.warn(
            ...
            "run with --force (or KB_CORE_FORCE=1) to rewrite them, or clear the "
