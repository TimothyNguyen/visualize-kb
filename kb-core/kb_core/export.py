...
from kb_core.analyze import _node_community_map
from kb_core.build import edge_data
from kb_core.paths import stem_filename_budget

from kb_core.exporters.graphdb import push_to_falkordb, push_to_neo4j  # noqa: E402,F401
...
_BACKUP_ARTIFACTS = [
    ...
    "GRAPH_REPORT.md",
    ".kb_core_labels.json",
    ".kb_core_analysis.json",
    "manifest.json",
    ".kb_core_semantic_marker",
    "cost.json",
]
...
def backup_if_protected(out_dir: Path) -> "Path | None":
    ...
    - .kb_core_semantic_marker is present (graph cost real LLM tokens), or
    - .kb_core_labels.json contains at least one non-default community label
    ...
    Set KB_CORE_NO_BACKUP=1 to disable.
    """
    if os.environ.get("KB_CORE_NO_BACKUP"):
        return None
    out = Path(out_dir)
    if not (out / "graph.json").exists():
        return None

    is_semantic = (out / ".kb_core_semantic_marker").exists()
    is_curated = False
        ...
        for name in _BACKUP_ARTIFACTS:
            ...
            if src.exists():
                try:
                    ...
                    pass
        if copied:
            print(f"[kb-core] backed up {reason} graph ({copied} files) -> {backup_dir.name}/")
        return backup_dir
    except Exception as exc:
        import sys
        print(f"[kb-core] warning: backup failed ({exc}) - continuing with overwrite", file=sys.stderr)
        return None
...
def _yaml_str(s: str) -> str:
    ...
    See `kb_core.ingest._yaml_str` for the full rationale; duplicated here to
...
from kb_core.exporters.base import COMMUNITY_COLORS  # noqa: E402,F401

from kb_core.exporters.html import to_html  # noqa: E402,F401
...
def to_json(G: nx.Graph, communities: dict[int, list[str]], output_path: str, *, force: bool = False, built_at_commit: str | None = None, community_labels: dict[int, str] | None = None) -> bool:
        ...
        if not oversized:
            ...
            if not raw.strip():
                ...
                try:
                    ...
                    import sys as _sys
                    print(
                        f"[kb-core] WARNING: existing {existing_path} could not be "
            ...
            if new_n < existing_n:
                import sys as _sys
                print(
                    f"[kb-core] WARNING: new graph has {new_n} nodes but existing "
                    ...
                    f"Run a full rebuild (/kb-core .) to be safe, or pass force=True "
    ...
    if "hyperedges" not in getattr(G, "graph", {}):
        ...
        if _prev_hyperedges:
            print(
                f"[kb-core] WARNING: graph carries no hyperedge metadata but "
...
def to_cypher(G: nx.Graph, output_path: str) -> None:
    lines = ["// Neo4j Cypher import - generated by /kb-core", ""]
...
# A frontmatter tag entry in kb_core's own namespace, e.g. "  - kb_core/document".
_KB_CORE_TAG_RE = re.compile(r"^\s*-\s+kb-core/\S")
...
# Community notes carry no frontmatter; kb_core identifies its own by the
# Dataview query it writes into every one of them.
_COMMUNITY_QUERY_MARKER = "FROM #community/"


def _is_kb_core_note(path: Path) -> bool:
    """Whether a vault note carries kb-core's own frontmatter signature.

    Every note kb-core writes opens with a YAML frontmatter block tagging it in
    the ``kb-core/`` namespace::
    ...
    user's note that merely mentions kb-core in its prose is not adopted.

    Community overview notes are recognised separately: they carry no
    frontmatter at all, so they are identified by kb-core's own filename prefix
    ...
    for line in head.splitlines()[1:]:
        if line.strip() == "---":
            return False  # frontmatter closed without a kb_core tag
        if _KB_CORE_TAG_RE.match(line):
            return True
    return False


def _adopt_pre_manifest_notes(out: Path) -> set[str]:
    """Names of notes in *out* that kb-core itself wrote before manifests existed.

    Deliberately limited to top-level ``*.md``: those are the only files kb-core
    can identify as its own from their content. ``.obsidian/graph.json`` is NOT
    adopted — kb-core writes one, but so does Obsidian, and with no manifest
    ...
    try:
        ...
        return set()
    return {p.name for p in candidates if _is_kb_core_note(p)}
...
def to_obsidian(
    ...
    # .obsidian/ config. Track the files kb_core owns in a manifest; a pre-existing
    # file NOT in the manifest is the user's and is never overwritten.
    _manifest_path = out / ".kb_core_obsidian_manifest.json"
    try:
    ...
    if not _manifest_existed:
        ...
        # kb_core owns, so every note it wrote last time reads as the user's and
        # is skipped. The re-export then writes fresh notes BESIDE the stale ones
        # and the vault carries two generations, with a warning claiming kb_core
        # "did not create" files it did (#2863). Adopt the notes that carry
        # kb_core's own frontmatter, once, so the manifest starts out honest.
        _owned |= _adopt_pre_manifest_notes(out)
    _written: list[str] = []
    _skipped: list[str] = []

    def _owned_write(rel_name: str, content: str) -> bool:
        """Write a kb-core-owned file, refusing to overwrite a pre-existing file
        kb-core didn't create. Returns True if written."""
        target = out / rel_name
    ...
    # Map file_type → kb_core tag
    _FTYPE_TAG = {
        "code": "kb-core/code",
        "document": "kb-core/document",
        "paper": "kb-core/paper",
        "image": "kb-core/image",
    }
    ...
    for node_id, data in G.nodes(data=True):
        ...
        ftype_tag = _FTYPE_TAG.get(ftype, f"kb-core/{ftype}" if ftype else "kb-core/document")
        dom_conf = _dominant_confidence(node_id)
        conf_tag = f"kb-core/{dom_conf}"
    ...
    # view — but never clobber an existing .obsidian/graph.json kb_core doesn't own
    ...
    # manifest says kb_core owns are candidates, and anything written or skipped
    ...
    if pruned:
        print(
            f"[kb-core] pruned {pruned} note(s) for nodes no longer in the graph",
            file=sys.stderr,
        )

    # Persist the manifest of files kb_core owns, so a re-run can safely update its
    ...
    if _skipped:
        ...
        print(
            f"[kb-core] WARNING: skipped {len(_skipped)} pre-existing file(s) kb-core "
            f"did not create, to avoid overwriting your notes: {shown}. "
            f"Export into an empty directory (or the default kb-core-out/obsidian) "
