...
KB Core nodes and edges. NOT a full SCIP protobuf implementation —
...
def ingest_scip_json(
    ...
    """Convert a SCIP-style JSON document into KB Core nodes and edges.
...
def _scip_relation_for(rel: dict[str, Any]) -> str:
    """Pick the KB Core relation tag for a SCIP relationship dict.
...
def _make_scip_node_id(symbol: str, source_file: str) -> str:
    """Derive a stable KB Core node ID from a SCIP symbol identifier.
...
def _scip_kind_to_file_type(kind: str) -> str:
    """Map SCIP symbol kind to a KB Core file_type."""
