...
def _portable_root_label(root: str) -> str:
    ...
    filesystem, and makes `kb-core update .`, `kb-core update ./proj`, and
    `kb-core update /abs/path/proj` all label the header `proj`. Only the
...
def load_learning_for_report(graph_path) -> dict | None:
    ...
    Reads the ``.kb_core_learning.json`` overlay (preferred sources) next to
    ...
    try:
        gp = _Path(graph_path)
        from kb_core.reflect import load_learning_overlay, load_memory_docs, aggregate_lessons
...
def generate(
    ...
    if gap_count > 0 or amb_pct > 20:
        ...
        if thin_communities:
            lines.append(f"- **{len(thin_communities)} thin communities (<{min_community_size} nodes) omitted from report** — run `kb-core query` to explore isolated nodes.")
        if amb_pct > 20:
    ...
    # Preferred sources come from the .kb_core_learning.json sidecar; the
