...
# ``_max_graph_file_bytes`` (which lets ``KB_CORE_MAX_GRAPH_BYTES`` override it).
...
def _max_graph_file_bytes() -> int:
    ...
    Honors the ``KB_CORE_MAX_GRAPH_BYTES`` environment variable so users with
    ...
    take effect.
    """
    raw = os.environ.get("KB_CORE_MAX_GRAPH_BYTES", "").strip()
    if not raw:
...
def safe_fetch(url: str, max_bytes: int = _MAX_FETCH_BYTES, timeout: int = 30) -> bytes:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 kb-core/1.0"})
...
def validate_graph_path(path: str | Path, base: Path | None = None) -> Path:
    ...
    *base* defaults to the `kb-core-out` directory relative to CWD.
    Also requires the base directory to exist, so a caller cannot
    trick kb-core into reading files before any graph has been built.
    ...
    if base is None:
        ...
        for candidate in [resolved_hint, *resolved_hint.parents]:
            if candidate.name == KB_CORE_OUT_NAME:
                base = candidate
                break
        if base is None:
            base = Path(KB_CORE_OUT).resolve()

    base = base.resolve()
    if not base.exists():
        raise ValueError(
            f"Graph base directory does not exist: {base}. "
            "Run /kb-core first to build the graph."
        )
    ...
    try:
        ...
        raise ValueError(
            ...
            "Only paths inside kb-core-out/ are permitted."
        )
...
def check_graph_file_size_cap(path: Path) -> None:
    ...
    ``KB_CORE_MAX_GRAPH_BYTES`` env var can be set before import and still
    apply.
    ...
    if size > cap:
        raise ValueError(
            ...
            f"(set KB_CORE_MAX_GRAPH_BYTES=<bytes> or "
            f"KB_CORE_MAX_GRAPH_BYTES=<N>GB to raise the limit)"
        )
