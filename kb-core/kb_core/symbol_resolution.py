...
from kb_core.ids import make_id as _shared_make_id
...
def _module_stem(module_name: str | None) -> str:
    """Return the final module component used to match KB Core source stems."""
...
def find_unique_python_symbol(
    ...
    """Resolve one imported symbol to exactly one KB Core node id."""
...
def resolve_cross_file_raw_calls(
    ...
    This intentionally preserves KB Core's existing behavior:
...
def _bash_make_id(*parts: str) -> str:
    ...
    Previously an inline copy to dodge an import cycle; ``kb_core.ids`` is
    dependency-free, so it can be imported directly.
    """
    return _shared_make_id(*parts)


from kb_core.extractors.base import _file_stem as _bash_file_stem  # canonical recipe (no import cycle: base imports only kb_core.ids)
...
def resolve_bash_source_edges(
    ...
    ``bash_sources[].target_path`` contract (KB Core static-analysis policy):
        ...
        - Relative paths: resolved against the *source file's* directory
          ...
          KB Core, NOT bash runtime semantics. At runtime, ``source ./X``
    ...
    for result, path in zip(per_file, path_by_index):
        ...
        for source in bash_sources:
            ...
            # KB Core static-analysis policy (NOT bash runtime semantics;
