...
from kb_core.paths import out_path as _out_path
...
def _model_name() -> str:
    return os.environ.get("KB_CORE_WHISPER_MODEL", _DEFAULT_MODEL)
...
def download_audio(url: str, output_dir: Path) -> Path:
    ...
    """
    from kb_core.security import validate_url
...
def build_whisper_prompt(god_nodes: list[dict]) -> str:
    ...
    domain hint from these labels and passes it via KB_CORE_WHISPER_PROMPT or
    as initial_prompt — no separate API call needed here.
    """
    if not god_nodes:
        return _FALLBACK_PROMPT

    override = os.environ.get("KB_CORE_WHISPER_PROMPT")
    if override:
        return override
