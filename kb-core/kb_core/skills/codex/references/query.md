# kb-core reference: query, path, explain

Load this when the user asks a question against an existing graph, or runs `$kb-core path` or `$kb-core explain`. The core's query stub points here for the full traversal flow. These flows use the `kb-core query` CLI when it is available and fall back to an inline NetworkX traversal otherwise.
...
if not Path('kb-core-out/graph.json').exists():
    print('ERROR: No graph found. Run $kb-core <path> first to build the graph.')
    raise SystemExit(1)
"
```
If it fails, stop and tell the user to run `$kb-core <path>` first.
...
import json, re
from pathlib import Path
data = json.loads(Path('kb-core-out/graph.json').read_text(encoding='utf-8'))
vocab = set()
for n in data['nodes']:
    for c in re.findall(r'[^\W\d_]+', n.get('label','') or '', re.UNICODE):
        ...
        for p in parts:
            ...
            if 3 <= len(t) <= 30:
                vocab.add(t)
Path('kb-core-out/.vocab.txt').write_text('\n'.join(sorted(vocab)), encoding='utf-8')
print(f'vocab: {len(vocab)} tokens')
"
```

2. Read `kb-core-out/.vocab.txt`. Then for the user's question, select **up to 12 tokens from this exact list** that semantically match the query intent. Hard constraints:
...
```bash
kb-core query "QUESTION"
# or: kb-core query "QUESTION" --dfs --budget 3000
```

If the CLI is unavailable, load `kb-core-out/graph.json` and run the traversal inline:
...
```bash
$(cat kb-core-out/.kb_core_python) -m kb_core save-result --question "ORIGINAL_QUESTION" --answer "ANSWER" --type query --nodes NODE1 NODE2
```
...
```bash
$(cat kb-core-out/.kb_core_python) -m kb_core save-result --question "Path from NODE_A to NODE_B" --answer "ANSWER" --type path_query --nodes NODE_A NODE_B
```
...
```bash
$(cat kb-core-out/.kb_core_python) -m kb_core save-result --question "Explain NODE_NAME" --answer "ANSWER" --type explain --nodes NODE_NAME
```
