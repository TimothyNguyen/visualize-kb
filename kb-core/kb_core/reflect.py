"""Deterministic "work memory" reflection over kb-core-out/memory/.

`kb-core reflect` reads the Q&A memory docs that `kb-core save-result` files back
...
and a given ``now``. When a graph (`graph.json` + `.kb_core_analysis.json`) is available
the lessons are also grouped by community label; without it they degrade to a single
flat section.

The artifact lands at ``kb-core-out/reflections/LESSONS.md`` rather than inside the wiki
because ``kb-core export wiki`` deletes every ``wiki/*.md`` on each run — a lessons file
...
from kb_core.ingest import OUTCOMES
...
LEARNING_SIDECAR_NAME = ".kb_core_learning.json"
...
def _load_node_community(graph_path: Path, analysis_path: Path,
    ...
    Mirrors how `kb-core export wiki` reads graph.json + .kb_core_analysis.json +
    .kb_core_labels.json. Community membership in the analysis sidecar is keyed by
...
def render_lessons_md(agg: dict[str, Any]) -> str:
    ...
    out: list[str] = [
        "# Lessons",
        "",
        f"_Auto-generated by `kb-core reflect` from {agg['total']} session "
        f"{'memory' if agg['total'] == 1 else 'memories'} in kb-core-out/memory/. "
...
def lessons_fresh(out_path: Path, memory_dir: Path,
    ...
    Lets ``kb-core reflect --if-stale`` skip a redundant run — e.g. when the git
...
def reflect(memory_dir: Path, out_path: Path,
    ...
    if graph_path is not None:
        ...
        analysis_path = Path(analysis_path) if analysis_path else (
            graph_path.parent / ".kb_core_analysis.json")
        labels_path = Path(labels_path) if labels_path else (
            graph_path.parent / ".kb_core_labels.json")
...
# graph.json as ``.kb_core_learning.json``. It carries which nodes have proven
...
def _resolve_source_path(src: str, graph_path: Path) -> Path | None:
    ...
    live in ``<root>/kb-core-out/`` (so its own dir is not the root) or directly
    ...
    Order: the committed ``.kb_core_root`` marker (#686/#1423 — authoritative for
    an absolute/elsewhere ``KB_CORE_OUT`` override); then the layout-appropriate
    root *first* — graph.json's parent's parent for the ``kb-core-out`` layout,
    ...
    try:
        recorded = (out_dir / ".kb_core_root").read_text(encoding="utf-8-sig").strip()
        if recorded:
    ...
    if out_dir.name == KB_CORE_OUT_NAME:
...
def write_learning_sidecar(agg: dict[str, Any], graph_path: Path,
    ...
    """Write ``.kb_core_learning.json`` next to ``graph_path`` deterministically.
