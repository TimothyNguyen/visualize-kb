"""Entity deduplication pipeline for kb-core knowledge graphs.
...
from kb_core._minhash import MinHash, MinHashLSH
    ...
    for loser in losers:
        ...
        if lose_file == keep_file:
            if _norm(lose_label) != _norm(keep_label):
                print(
                    f"[kb-core] note: node '{nid}' was extracted twice from "
            ...
            print(
                f"[kb-core] WARNING: node '{nid}' is minted by two different files — "
                ...
                f"dropped node is lost. To keep them distinct, run 'kb-core extract' "
                f"per subfolder and merge with 'kb-core merge-graphs'.",
                file=sys.stderr,
            )
...
def deduplicate_entities(
    ...
    *,
    ...
    total = len(remap)
    msg = f"[kb-core] Deduplicated {total} node(s)"
...
def _llm_tiebreak(
    ...
    *,
    ...
    try:
        from kb_core.llm import BACKENDS, _format_backend_env_keys, _get_backend_api_key
        if backend not in BACKENDS:
            print(f"[kb-core] --dedup-llm: unknown backend {backend!r}, skipping LLM tiebreaker.", flush=True)
            return
        if not _get_backend_api_key(backend):
            env_keys = _format_backend_env_keys(backend)
            print(f"[kb-core] --dedup-llm: {env_keys} not set, skipping LLM tiebreaker.", flush=True)
            return
    except ImportError:
        return
    ...
    try:
        from kb_core.llm import _call_llm
    except ImportError as exc:
        # F-038: previously this silent fallback hid the fact that `_call_llm`
        # didn't exist in `kb_core.llm` at all, so `--dedup-llm` was a no-op.
        # Surface the import failure so future regressions are visible.
        print(
            f"[kb-core] --dedup-llm: cannot import _call_llm ({exc}); skipping LLM tiebreaker.",
            flush=True,
        )
        return
    ...
    for batch_start in range(0, len(ambiguous), batch_size):
        ...
        try:
            ...
            print(f"[kb-core] --dedup-llm batch failed: {exc}", flush=True)
