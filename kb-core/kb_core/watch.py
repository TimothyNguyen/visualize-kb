...
# Single source of truth in kb_core.paths (#1423); re-exported as _KB_CORE_OUT.
from kb_core.paths import KB_CORE_OUT as _KB_CORE_OUT, is_absolute_any_platform
...
_BUILD_CONFIG_FILENAME = ".kb_core_build.json"
...
def _apply_resource_limits() -> None:
    ...
    KB_CORE_REBUILD_MEMORY_LIMIT_MB caps RSS-ish memory. Uses RLIMIT_DATA on
    ...
    try:
        ...
        pass
    mb = os.environ.get("KB_CORE_REBUILD_MEMORY_LIMIT_MB", "").strip()
    if not mb:
        return
    try:
...
def _git_head(cwd: Path | str | None = None) -> str | None:
    ...
    inherits the caller's working directory, so `kb-core update <target>`
...
def _reconcile_existing_graph(
    ...
    *,
    ...
    .kb_coreignore + persisted ``--exclude`` patterns — unambiguous
    ...
    ``kb-core update``) so the incremental/hook path keeps preserving a
    ...
    from kb_core.build import _load_existing_graph
    ...
    from kb_core.build import _is_ast_tier
    ...
    try:
        from kb_core.build import _norm_source_file as _nsf
        ...
        def _ignored_now(identity: str) -> bool:
            ...
            .kb_coreignore/--exclude matches evict on every rebuild;
        ...
        if newly_ignored_files:
            print(
                f"[kb-core watch] pruned {newly_ignored_nodes} node(s) from "
        ...
        if excluded_alive_files:
            print(
                f"[kb-core watch] fail-closed: kept {excluded_alive_nodes} node(s) "
                ...
                "changed?). Add them to .kb_coreignore if the exclusion is intentional."
            )
        ...
        print(
            "[kb-core watch] reconcile of existing graph failed "
...
def _node_community_map(graph_data: dict) -> dict[str, int]:
    ...
    for node in graph_data.get("nodes", []):
        ...
        try:
            ...
            print(
                f"[kb-core watch] Skipping node with invalid community id: "
...
def _check_shrink(
    ...
    *,
    ...
    plain ``kb-core update`` after deleting a function refresh the graph without
    ...
    if rebuilt_sources is not None:
        from kb_core.build import _norm_source_file
    ...
    print(
        f"[kb-core] WARNING: new graph has {len(new_nodes)} nodes but existing "
...
def _stabilize_rebuild_cwd(watch_path: Path) -> bool:
    ...
    Path('.').resolve(), and relative kb-core-out mkdirs raise FileNotFoundError
    before the normal rebuild error handling can run. Hooks that know the repo
    root export KB_CORE_REPO_ROOT so the rebuild can recover by chdir'ing there.
    """
    if watch_path.is_absolute():
        return True

    repo_root = os.environ.get("KB_CORE_REPO_ROOT", "").strip()
    ...
    try:
        ...
        print(
            "[kb-core watch] Rebuild failed: current working directory "
            "no longer exists and KB_CORE_REPO_ROOT is not set."
        )
        return False
...
def _reconcile_graph_html(out: Path, graph_data: dict) -> str | None:
    ...
    from kb_core.exporters.html import _HTML_STALE_MARKER, _viz_node_limit
    ...
    def clear_stale_marker() -> None:
        try:
            ...
            print(
                "[kb-core watch] graph.html stale marker could not be cleared; "
    ...
    try:
        from kb_core.export import to_html
        ...
        if had_html:
            print(
                "[kb-core watch] Stale graph.html left unchanged; "
                f"regeneration will be retried: {exc}"
            )
        else:
            print(f"[kb-core watch] Missing graph.html could not be regenerated: {exc}")
        return None
    ...
    if not written or not html_target.exists():
        if had_html:
            ...
            print("[kb-core watch] Missing graph.html has no useful community view; skipped.")
        return None
    clear_stale_marker()
    return "rendered"
...
def _rebuild_code(
    ...
    *,
    ...
    JSON to kb-core-out/graph.json (mirrors ``extract --no-cluster``).
    ...
    if acquire_lock:
        ...
        with _rebuild_lock(out, blocking=block_on_lock) as got:
            if not got:
                print("[kb-core watch] Rebuild already in progress for "
    ...
    try:
        from kb_core.extract import extract, _get_extractor
        from kb_core.detect import detect
        from kb_core.build import build_from_json, _is_ast_tier, _norm_source_file as _nsf
        from kb_core.cluster import cluster, remap_communities_to_previous, score_all
        from kb_core.analyze import god_nodes, surprising_connections, suggest_questions
        ...
        # keep. Two tiers: .kb_coreignore + persisted --exclude patterns are
        ...
        from kb_core.detect import ignored_predicate
        ...
        if not code_files and not existing_graph.exists():
            print("[kb-core watch] No code files found - nothing to rebuild.")
            return False
        ...
        if changed_paths is not None:
            ...
            if not wanted and not deleted_paths:
                print("[kb-core watch] No tracked code files in change set - skipping rebuild.")
                return True
        ...
        # gate reports them unchanged forever and only deleting kb_core-out/
        ...
        if no_cluster:
            ...
            candidate_graph_data = {
                **{k: v for k, v in result.items() if k not in ("edges", "nodes")},
                ...
                # `kb_core update --no-cluster` can't silently drop it -
            ...
            (out / ".kb_core_root").write_text(str(watch_path), encoding="utf-8")
            ...
            if same_graph:
                print("[kb-core watch] No code-graph changes detected (--no-cluster); outputs left untouched.")
            else:
                print(
                    "[kb-core watch] Rebuilt (no clustering): "
                    f"{len(candidate_graph_data.get('nodes', []))} nodes, "
                    f"{len(candidate_graph_data.get('links', []))} edges"
                )
                print(f"[kb-core watch] graph.json updated in {out}")
            return True
        ...
        if existing_graph_data:
            ...
            if same_topology:
                ...
                if html_action == "rendered":
                    print(
                        "[kb-core watch] No code-graph topology changes detected; "
                    ...
                    print("[kb-core watch] No code-graph topology changes detected; outputs left untouched.")
                return True
        ...
        # Without this, an incremental `kb_core update` launders stale names into
        ...
        if stale:
            print(
                f"[kb-core watch] community set changed since labeling "
        ...
        if no_change:
            graph_tmp.unlink(missing_ok=True)
            print("[kb-core watch] No code-graph changes detected; graph.json/GRAPH_REPORT.md left untouched.")
        else:
            if not _check_shrink(
                force, existing_graph_data, candidate_graph_data,
                tmp=graph_tmp,
        ...
        if callflow_files and not no_change:
            try:
                from kb_core.callflow_html import write_callflow_html
                for cf in callflow_files:
                    write_callflow_html(
                        graph=out / "graph.json",
                        report=out / "GRAPH_REPORT.md",
                        labels=out / ".kb_core_labels.json",
                        output=cf,
                        verbose=False,
                    )
            except Exception as cf_err:
                print(f"[kb-core watch] callflow HTML update skipped: {cf_err}")
        ...
        if not no_change:
            print(f"[kb-core watch] Rebuilt: {G.number_of_nodes()} nodes, "
            ...
            print(f"[kb-core watch] {products} updated in {out}")
        return True

    except Exception as exc:
        print(f"[kb-core watch] Rebuild failed: {exc}")
        return False
...
def check_update(watch_path: Path) -> bool:
    ...
    re-extraction via `/kb-core --update` — this function only signals
    that the update is needed.
    """
    flag = Path(watch_path) / _KB_CORE_OUT / "needs_update"
    if flag.exists():
        print(f"[kb-core check-update] Pending non-code changes in {watch_path}.")
        print("[kb-core check-update] Run `/kb-core --update` to apply semantic re-extraction.")
    return True


def _notify_only(watch_path: Path) -> None:
    """Write a flag file and print a notification (fallback for non-code-only corpora)."""
    flag = watch_path / _KB_CORE_OUT / "needs_update"
    flag.parent.mkdir(parents=True, exist_ok=True)
    flag.write_text("1", encoding="utf-8")
    print(f"\n[kb-core watch] New or changed files detected in {watch_path}")
    print("[kb-core watch] Non-code files changed - semantic re-extraction requires LLM.")
    print("[kb-core watch] Run `/kb-core --update` in Claude Code to update the graph.")
    print(f"[kb-core watch] Flag written to {flag}")
...
def _batch_triggers_rebuild(batch: list[Path]) -> bool:
    ...
    until the next code event or a manual `kb-core update` (#2580).
    """
...
def watch(watch_path: Path, debounce: float = 3.0) -> None:
    ...
    to run /kb-core --update (LLM extraction required).
    ...
    # Load .kb_coreignore patterns ONCE at startup so the handler does not
    ...
    ignore_patterns = _load_kb_coreignore(
        watch_root_for_ignore,
        gitignore=_read_build_gitignore(watch_path / _KB_CORE_OUT),
    )
    ...
    class Handler(FileSystemEventHandler):
        def on_any_event(self, event):
            ...
            # Check .kb_coreignore BEFORE the extension/dotfile/out filters so
            ...
            if any(part.startswith(".") for part in filter_parts):
                return
            if _KB_CORE_OUT in filter_parts:
                return
    ...
    print(f"[kb-core watch] Watching {watch_path.resolve()} - press Ctrl+C to stop")
    print(f"[kb-core watch] Code changes rebuild graph automatically. "
          f"Doc/image changes require /kb-core --update.")
    print(f"[kb-core watch] Debounce: {debounce}s")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Watch a folder and auto-update the kb-core graph")
