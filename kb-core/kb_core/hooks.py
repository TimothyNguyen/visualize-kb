# git hook integration - install/uninstall kb_core post-commit and post-checkout hooks
...
_HOOK_MARKER = "# kb-core-hook-start"
_HOOK_MARKER_END = "# kb-core-hook-end"
_CHECKOUT_MARKER = "# kb-core-checkout-hook-start"
_CHECKOUT_MARKER_END = "# kb-core-checkout-hook-end"

# __PINNED_PYTHON__ is replaced at install time with the absolute path of the
# Python interpreter that ran `kb_core hook install`.  For uv-tool and pipx
...
# when the kb-core launcher is not on PATH (common in GUI clients and CI).
#
# Probes check availability with importlib.util.find_spec instead of importing
# the package: a probe that imports kb-core wholesale executes the full package
...
_GFY_PROBE="import importlib.util, sys; sys.exit(0 if importlib.util.find_spec('kb-core') else 1)"
KB_CORE_PYTHON=""
_PINNED='__PINNED_PYTHON__'
if [ -n "$_PINNED" ] && [ -x "$_PINNED" ] && "$_PINNED" -c "$_GFY_PROBE" 2>/dev/null; then
    KB_CORE_PYTHON="$_PINNED"
fi
# Second probe: read kb-core-out/.kb_core_python (written by the skill and
# CLI; survives uv-tool reinstalls and is the same source the README documents).
if [ -z "$KB_CORE_PYTHON" ]; then
    _GFY_PYTHON_FILE="kb-core-out/.kb_core_python"
    if [ -f "$_GFY_PYTHON_FILE" ]; then
        ...
        if [ -n "$_FROM_FILE" ] && [ -x "$_FROM_FILE" ] && "$_FROM_FILE" -c "$_GFY_PROBE" 2>/dev/null; then
            KB_CORE_PYTHON="$_FROM_FILE"
        fi
    fi
fi
# Third probe: resolve via the kb-core launcher on PATH.
if [ -z "$KB_CORE_PYTHON" ]; then
    KB_CORE_BIN=$(command -v kb-core 2>/dev/null)
    if [ -n "$KB_CORE_BIN" ]; then
        # Windows pip layout: Scripts/kb-core(.exe) sits beside ..\\python.exe
    ...
    if [ -z "$KB_CORE_PYTHON" ] && [ -n "$KB_CORE_BIN" ]; then
        ...
        case "$KB_CORE_BIN" in
            *.exe) _GFY_HEAD="" ;;
            *)     _GFY_HEAD=$(head -c 256 "$KB_CORE_BIN" 2>/dev/null | tr -d '\\000') ;;
        esac
        case "$_GFY_HEAD" in
        ...
        case "$_SHEBANG" in
            */env\\ *) KB_CORE_PYTHON="${_SHEBANG#*/env }" ;;
            *)         KB_CORE_PYTHON="$_SHEBANG" ;;
        esac
        # Allowlist: only keep characters valid in a filesystem path to prevent
        # injection if the shebang contains shell metacharacters.
        case "$KB_CORE_PYTHON" in
            *[!a-zA-Z0-9/_.@:\\\\-]*) KB_CORE_PYTHON="" ;;
        esac
        if [ -n "$KB_CORE_PYTHON" ] && ! "$KB_CORE_PYTHON" -c "$_GFY_PROBE" 2>/dev/null; then
            KB_CORE_PYTHON=""
        fi
    fi
fi
# Fourth probe: uv tool environments. `uv tool install` (the README's
# recommended method) puts kb-core in an isolated venv that no ambient
...
# co-installed tool without kb-core never satisfies it.
if [ -z "$KB_CORE_PYTHON" ]; then
    for _GFY_TOOLS in \
        "${UV_TOOL_DIR:-}" \
...
if [ -z "$KB_CORE_PYTHON" ]; then
    if command -v python3 >/dev/null 2>&1 && python3 -c "$_GFY_PROBE" 2>/dev/null; then
        KB_CORE_PYTHON="python3"
    elif command -v python >/dev/null 2>&1 && python -c "$_GFY_PROBE" 2>/dev/null; then
        KB_CORE_PYTHON="python"
    else
        echo "[kb-core hook] could not locate a Python with kb-core installed. Add the kb-core bin dir to PATH or re-run 'kb-core hook install' from the env where kb-core lives." >&2
        exit 0
    fi
fi
"""
...
changed_raw = os.environ.get('KB_CORE_CHANGED', '')
changed = [Path(f.strip()) for f in changed_raw.strip().splitlines() if f.strip()]

if not changed:
    sys.exit(0)

print(f'[kb-core hook] {len(changed)} file(s) changed - rebuilding graph...')

try:
    from kb_core.watch import _rebuild_code, _apply_resource_limits
    _apply_resource_limits()
    _timeout = int(os.environ.get('KB_CORE_REBUILD_TIMEOUT', '600'))
    if _timeout > 0:
        if hasattr(signal, 'SIGALRM'):
            signal.signal(signal.SIGALRM, lambda *_: (_ for _ in ()).throw(TimeoutError(f'kb-core rebuild exceeded {_timeout}s')))
            signal.alarm(_timeout)
        else:
            def _bail():
                print(f'[kb-core hook] kb-core rebuild exceeded {_timeout}s', flush=True)
                os._exit(1)
            _watchdog = threading.Timer(_timeout, _bail)
            _watchdog.daemon = True
            _watchdog.start()
    _force = os.environ.get('KB_CORE_FORCE', '').lower() in ('1', 'true', 'yes')
    _root = Path('.')
    _out = os.environ.get('KB_CORE_OUT', 'kb-core-out')
    _saved = Path(_out) / '.kb_core_root'
    if _saved.exists():
    ...
    try:
        ...
        if _md.is_dir() and any(_md.glob('*.md')):
            from kb_core.reflect import reflect as _reflect
    ...
    print(f'[kb-core hook] {exc}')
    sys.exit(1)
except Exception as exc:
    print(f'[kb-core hook] Rebuild failed: {exc}')
    sys.exit(1)
"""
...
try:
    ...
    if _timeout > 0:
        if hasattr(signal, 'SIGALRM'):
            ...
            def _bail():
                print(f'[kb-core] kb-core rebuild exceeded {_timeout}s', flush=True)
                os._exit(1)
            _watchdog = threading.Timer(_timeout, _bail)
            _watchdog.daemon = True
            _watchdog.start()
    ...
    print(f'[kb-core] Rebuild failed: {exc}')
    sys.exit(1)
"""

# Cross-platform detached-launch shim (#1161). The hooks used to background the
# rebuild with `nohup "$KB_CORE_PYTHON" -c "..." &`, but Git for Windows' bundled
...
# still returned 0, so the graph just went stale with no signal. kb_core already
...
__REBUILD_BODY__
'''
_log = os.environ.get('KB_CORE_REBUILD_LOG') or os.path.join(os.path.expanduser('~'), '.cache', 'kb-core-rebuild.log')
try:
...
def _detached_launch(rebuild_body: str) -> str:
    ...
    Python process via ``$KB_CORE_PYTHON``.
    ...
    The launcher writes the child's output to ``$KB_CORE_REBUILD_LOG`` and
    ...
    return '"$KB_CORE_PYTHON" -c "' + launcher + '"\n'
...
# fires these hooks; the canonical kb_core-out/ belongs to the primary checkout,
...
# rebuild ("failed to remove kb_core-out/: Directory not empty") (#1809, #1806).
...
# Installed by: kb-core hook install
...
# churn run-to-run. Pinning it makes kb-core-out reproducible.
...
# explicit KB_CORE_MAX_WORKERS still wins for users who want parallelism.
if [ -n "${WINDIR:-}" ] || [ -n "${MSYSTEM:-}" ]; then
    export KB_CORE_MAX_WORKERS="${KB_CORE_MAX_WORKERS:-1}"
fi
...
# Skip when only kb-core-out/ artifacts changed (avoids rebuild loop when graph outputs are tracked in git)
_NON_GRAPH=$(echo "$CHANGED" | grep -v '^kb-core-out/' || true)
if [ -z "$_NON_GRAPH" ]; then
    exit 0
fi

""" + _PYTHON_DETECT + """
export KB_CORE_CHANGED="$CHANGED"
...
_KB_CORE_LOG="${HOME}/.cache/kb-core-rebuild.log"
mkdir -p "$(dirname "$_KB_CORE_LOG")"
export KB_CORE_REBUILD_LOG="$_KB_CORE_LOG"
echo "[kb-core hook] launching background rebuild (log: $_KB_CORE_LOG)"
""" + _detached_launch(_REBUILD_BODY_COMMIT) + """# kb-core-hook-end
"""


_CHECKOUT_SCRIPT = """\
# kb-core-checkout-hook-start
...
# Only run if kb-core-out/ exists (graph has been built before)
if [ ! -d "kb-core-out" ]; then
    exit 0
fi
...
# Honor the same opt-out as post-commit: without this, KB_CORE_SKIP_HOOK=1
...
mkdir -p "$(dirname "$_KB_CORE_LOG")"
export KB_CORE_REBUILD_LOG="$_KB_CORE_LOG"
echo "[kb-core] Branch switched - launching background rebuild (log: $_KB_CORE_LOG)"
""" + _detached_launch(_REBUILD_BODY_CHECKOUT) + """# kb-core-checkout-hook-end
"""


def _load_kb_corerc(root: Path) -> dict[str, str | int]:
    """Load key/value options from <root>/.kb_corerc if present.

    Supported options:
      viz_node_limit: integer >= 0 (e.g. viz_node_limit=0)
    """
    rc_path = root / ".kb_corerc"
...
def _hooks_dir(root: Path) -> Path:
    ...
    try:
        ...
        if res.returncode != 0:
            ...
            print(
                f"[kb-core hooks] git could not resolve the hooks path for "
...
def _install_hook(
    ...
    an existing kb-core block in-place."""
...
def _uninstall_hook(hooks_dir: Path, name: str, marker: str, marker_end: str) -> str:
    """Remove kb-core section from a git hook using start/end markers."""
    hook_path = hooks_dir / name
    if not hook_path.exists():
        return f"no {name} hook found - nothing to remove."
    content = hook_path.read_text(encoding="utf-8")
    if marker not in content:
        return f"kb-core hook not found in {name} - nothing to remove."
    new_content = re.sub(
    ...
    return f"kb-core removed from {name} at {hook_path} (other hook content preserved)"
...
def _pinned_python() -> str:
    ...
    means callers must fall back to the `kb-core` launcher on PATH — safe
    degradation.
    """
...
def _merge_attr_line() -> str:
    """The .gitattributes line assigning the kb-core merge driver to graph.json.

    The graph lives under the configured output directory (kb_core.paths,
    KB_CORE_OUT env override). gitattributes patterns are repo-relative, so an
    absolute output-dir override cannot be expressed there — fall back to the
    default name in that case.
    """
    from kb_core.paths import KB_CORE_OUT
    out = KB_CORE_OUT
    if not out or Path(out).is_absolute() or "\\" in out:
        out = "kb-core-out"
    return f"{out.rstrip('/')}/graph.json merge=kb-core"


def _has_merge_attr(content: str) -> bool:
    """True if a (non-comment) `<...>graph.json ... merge=kb-core` line exists."""
    for raw in content.splitlines():
        line = raw.strip()
        if not line or line.startswith("#"):
            continue
        fields = line.split()
        if fields and fields[0].endswith("graph.json") and "merge=kb-core" in fields[1:]:
            return True
    return False
...
def _register_merge_driver(root: Path) -> str:
    ...
    README and CHANGELOG 0.7.0 document `kb-core merge-driver` as being set up
    ...
    same way the hook scripts pin it, so the driver works even when the kb-core
    ...
    if pinned:
        ...
        driver = f'"{pinned}" -m kb-core merge-driver %O %A %B'
    else:
        driver = "kb-core merge-driver %O %A %B"
    try:
        for key, value in (
            ("merge.kb-core.name", "kb-core graph.json union merge"),
            ("merge.kb-core.driver", driver),
        ):
            _sp.run(
...
def _unregister_merge_driver(root: Path) -> str:
    ...
    import subprocess as _sp
    for key in ("merge.kb-core.name", "merge.kb-core.driver"):
        try:
...
def _merge_driver_status(root: Path) -> str:
    ...
    try:
        res = _sp.run(
            ["git", "-C", str(root), "config", "--get", "merge.kb-core.driver"],
            capture_output=True, text=True,
        )
...
def install(path: Path = Path(".")) -> str:
    """Install kb-core post-commit and post-checkout hooks in the nearest git repo."""
    root = _git_root(path)
    if root is None:
    ...
    cfg = _load_kb_corerc(root)
    viz_limit = cfg.get("viz_node_limit")
    if viz_limit is not None:
        # Use the `:-` default form (like KB_CORE_MAX_WORKERS below) so an
        # explicit `KB_CORE_VIZ_NODE_LIMIT=... git commit` still wins over the
        # baked project default — persisting config must not clobber a per-run
        # override.
        viz_export = f'export KB_CORE_VIZ_NODE_LIMIT="${{KB_CORE_VIZ_NODE_LIMIT:-{viz_limit}}}"\n'
    else:
        viz_export = ""
...
def uninstall(path: Path = Path(".")) -> str:
    """Remove kb-core post-commit and post-checkout hooks."""
    root = _git_root(path)
    if root is None:
        raise RuntimeError(f"No git repository found at or above {path.resolve()}")
...
def status(path: Path = Path(".")) -> str:
    """Check if kb-core hooks are installed."""
    root = _git_root(path)
    if root is None:
        return "Not in a git repository."
    hooks_dir = _user_hooks_dir(_hooks_dir(root))
    # status is a read-only diagnostic: a malformed .kb_corerc must not turn it
    ...
    def _check(name: str, marker: str) -> str:
        ...
        if marker not in text:
            return "not installed (hook exists but kb-core not found)"
        if cfg_limit is not None:
            # Baked as `"${KB_CORE_VIZ_NODE_LIMIT:-<n>}"` so a per-run override
            # wins; match the default <n>, and still accept the older bare
            # `"<n>"` form from hooks installed before that change.
            m = re.search(
                r'export KB_CORE_VIZ_NODE_LIMIT="(?:\$\{KB_CORE_VIZ_NODE_LIMIT:-(\d+)\}|(\d+))"',
                text,
            )
