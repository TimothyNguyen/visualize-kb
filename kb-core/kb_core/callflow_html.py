#!/usr/bin/env python3
"""
callflow_html.py — Generate call-flow architecture HTML from kb-core knowledge graph outputs.

Reads graph.json plus optional GRAPH_REPORT.md, .kb_core_labels.json, and sections JSON,
...
Usage:
  python3 -m kb-core export callflow-html
  python3 -m kb-core export callflow-html /path/to/project/kb-core-out/graph.json
  python3 -m kb-core export callflow-html --graph /path/to/graph.json --output docs/architecture.html
"""
...
from kb_core.paths import KB_CORE_OUT, KB_CORE_OUT_NAME
...
def normalize_node(raw: dict, index: int) -> dict:
    """Normalize a kb-core node across common graph.json schema variants."""
    node = dict(raw)
...
def normalize_edge(raw: dict, index: int) -> dict | None:
    """Normalize kb-core edges while preserving original fields."""
    edge = dict(raw)
...
def _node_link_payload(data: dict) -> tuple[list, list] | None:
    """Read current kb-core graph.json via NetworkX's node-link parser."""
...
def load_labels(path: str | Path | None) -> dict:
    """Load community labels from .kb_core_labels.json, tolerating wrapper keys."""
...
def infer_project_name(graph_path: str, meta: dict) -> str:
    ...
    if path.parent.name == KB_CORE_OUT_NAME and len(path.parents) > 1:
        return path.parents[1].name
    return path.parent.name or "Project"


def resolve_kb_core_paths(args) -> dict:
    """Resolve project root, kb-core output dir, and optional files."""
    base = Path(args.project).expanduser() if args.project else Path.cwd()
    if args.kb_core_out:
        kb_core_out = Path(args.kb_core_out).expanduser()
    elif args.graph:
        kb_core_out = Path(args.graph).expanduser().parent
    elif (base / "graph.json").exists():
        kb_core_out = base
    else:
        kb_core_out = base / KB_CORE_OUT

    project_root = kb_core_out.parent if kb_core_out.name == KB_CORE_OUT_NAME else base
    graph = Path(args.graph).expanduser() if args.graph else kb_core_out / "graph.json"
    report = Path(args.report).expanduser() if args.report else kb_core_out / "GRAPH_REPORT.md"
    labels = Path(args.labels).expanduser() if args.labels else kb_core_out / ".kb_core_labels.json"
    sections = Path(args.sections).expanduser() if args.sections else None
    return {
        "base": project_root,
        "kb_core_out": kb_core_out,
        "graph": graph,
        "report": report,
        "labels": labels,
        "sections": sections,
...
def node_importance(node: dict) -> float:
    """Use kb-core centrality fields when available."""
...
def generate_header(sections: list, meta: dict, lang: str) -> str:
    ...
    if lang.startswith("zh"):
        ...
        subtitle = (
            f"由 kb-core 知识图谱生成：{meta.get('node_count', '?')} 个节点、"
        ...
        subtitle = (
            f"Generated from kb-core knowledge graph: {meta.get('node_count', '?')} nodes, "
...
def generate_section_cards(sec: dict, nodes: list, section_edges: list, lang: str) -> str:
    ...
    note = pick_text(
        lang,
        f"本节由 kb-core 社区聚类生成。关系概况：{relation_text}。图表优先展示高置信、跨节点调用或使用关系，完整节点清单位于表格中。",
        f"This section comes from kb-core community clustering. Relationship summary: {relation_text}. The diagram prioritizes high-confidence calls or usage relationships; the table keeps the broader node inventory.",
    )
...
class CallflowOptions:
    ...
    def __init__(
        ...
        *,
        kb_core_out: str | Path | None = None,
        ...
        self.kb_core_out = str(kb_core_out) if kb_core_out is not None else None
...
def write_callflow_html(
    ...
    *,
    ...
    """Generate call-flow architecture HTML from kb-core output files."""
    ...
    paths = resolve_kb_core_paths(args)
    if not paths["graph"].exists():
        raise FileNotFoundError(
            f"kb-core output not found: {paths['graph']}. "
            "Run kb-core first or pass --graph /path/to/graph.json."
        )
    ...
    if args.output:
        ...
        output_path = paths["kb_core_out"] / f"{safe_filename(meta['project_name'])}-callflow.html"
...
""")
  ...
  <p>Generated: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')} · kb-core callflow-html</p>
</div>
""")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate call-flow architecture HTML from kb-core knowledge graph outputs"
    )
    parser.add_argument("project", nargs="?", default=None, help="Project root or kb-core output directory")
    parser.add_argument("--kb-core-out", default=None, help="Path to kb-core output directory")
    ...
    parser.add_argument("--labels", default=None, help="Path to .kb_core_labels.json")
