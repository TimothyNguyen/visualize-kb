...
CLI: ``kb-core tree [--graph PATH] [--output HTML] [--root PATH]
    ...
    *,
    ...
    for src_file, syms in sorted(by_file.items()):
        ...
        for n in syms:
            ...
            # Skip the redundant file-name node kb_core emits (bare basename or
            # the directory-qualified form from the #2032 disambiguation pass).
            if n.get("file_type") == "code":
                from kb_core.build import _is_file_node_label
...
def write_tree_html(
    ...
    *,
    ...
    title = f"{tree['name']} — kb-core tree viewer"
