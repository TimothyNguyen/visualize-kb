"""html — moved verbatim from kb-core/export.py."""
...
def _viz_node_limit() -> int:
    """Return the effective viz node limit, honoring KB_CORE_VIZ_NODE_LIMIT env var.
    ...
    """
    import os
    raw = os.environ.get("KB_CORE_VIZ_NODE_LIMIT")
...
// (kb-core add) can contain a double-quote; dropping the stringified id
...
def _html_document_title(output_path: str) -> str:
    ...
    configured output-dir bare name (``kb-core-out`` / ``KB_CORE_OUT``
    ...
    marker = KB_CORE_OUT_NAME
    for i, part in enumerate(parts):
        if part == marker or part.startswith("kb-core-out"):
            return "/".join(parts[i:])
...
def to_html(
    ...
    if G.number_of_nodes() > limit:
        ...
        raise ValueError(
            ...
            f"(limit: {limit}). Use --no-viz, raise KB_CORE_VIZ_NODE_LIMIT, "
    ...
    # best-effort from the sibling .kb_core_learning.json next to the output
