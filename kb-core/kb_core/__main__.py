...
from kb_core.install import (  # noqa: E402,F401
    ...
    _codex_agents_md_install,
    _codex_agents_md_uninstall,
    _copy_skill_file,
...
# AGENTS.md section for Codex -- reads AGENTS.md in the project root for
...
def _run_cli() -> None:
    ...
    if not any(arg in _silent_cmds for arg in sys.argv):
        ...
        # overrides (claude/windows honor CLAUDE_CONFIG_DIR) check the dir
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help", "-?"):
        ...
        print()
        print("Commands:")
        print("  install [--platform P]  copy skill to platform config dir (claude|windows|codex|copilot|vscode)")
        ...
        print("  cluster init [DIR]       create a cluster spec (compose several repos into one graph)")
        print("    --name NAME              cluster name (default: directory name)")
        print("  cluster add <path|url>   add a member repo to the cluster")
        print("    --as <tag>               member tag (default: repo directory name)")
        print("  cluster remove <tag>     remove a member")
        print("  cluster locate <tag> <path>  record where a member lives on this machine")
        print("  cluster build            compose the members into one cluster graph")
        print("    --force                  rebuild even when nothing changed")
        print("    --no-links               skip the declared cross-repo links")
        print("    --no-refs                do not write cluster-ref.json markers into members")
        print("  cluster check            validate the spec and members (exit 1 on error)")
        print("  cluster status           list members and whether each resolves")
