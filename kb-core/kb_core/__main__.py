"""kb-core CLI - `kb-core install` sets up the Claude Code skill."""
...
# Output directory — override with KB_CORE_OUT env var for worktrees or shared-output setups.
# Accepts a relative name ("kb_core-out-feature") or an absolute path ("/shared/kb_core-out").
# Defined once in kb_core.paths so the security/callflow path guards honour the
# same override (#1423).
from kb_core.paths import KB_CORE_OUT as _KB_CORE_OUT

# Install/uninstall subsystem moved to kb_core/install.py; re-exported here so
# `from kb_core.__main__ import <name>` keeps working unchanged.
from kb_core.install import (  # noqa: E402,F401
    ...
    _resolve_kb_core_exe,
    _skill_registration,
    _strip_kb_core_hook,
    _strip_kb_core_md_section,
...
from kb_core.cli import (  # noqa: E402,F401
    dispatch_command,
    _StageTimer,
    _clone_repo,
    _default_graph_path,
...
def __getattr__(name: str) -> str:
    ...
    # brick the CLI on `import kb_core.__main__` (#1121 follow-up).
...
def _check_skill_version(skill_dst: Path) -> None:
    """Warn if the installed skill is from an older kb-core version."""
    ...
    try:
        ...
        return
    if not skill_exists:
        print("  warning: skill dir exists but SKILL.md is missing. Run 'kb-core install' to repair.", file=sys.stderr)
        return
    ...
    if "references/" in body and not (skill_dst.parent / "references").exists():
        print("  warning: skill references/ sidecar is missing. Run 'kb-core install' to repair.", file=sys.stderr)
    try:
    ...
    if installed != __version__:
        if _version_tuple(installed) > _version_tuple(__version__):
            # The skill on disk is NEWER than the running package. `kb_core install`
            ...
            print(
                f"  warning: skill is from kb-core {installed}, but the package is "
                f"{__version__} (older). Upgrade the package "
                f"(e.g. 'uv tool upgrade kb-core' or 'pip install -U kb-core'); "
                f"running 'kb-core install' would downgrade the skill.",
                file=sys.stderr,
            )
        else:
            print(f"  warning: skill is from kb-core {installed}, package is {__version__}. Run 'kb-core install' to update.", file=sys.stderr)
...
# `kb_core hook-guard` subcommand (see _run_hook_guard). The previous hooks
...
# silently vanished — users had to invoke /kb_core by hand (#522). Moving the
...
# The always-on instruction blocks are packaged markdown under kb_core/always_on/,
...
# when a graph exists. Emitted by `kb_core hook-guard gemini`. The old hook was a
...
_KIRO_STEERING_MARKER = "kb-core: A knowledge graph of this project"
        ...
        "PreToolUse": [
            {
                ...
                "hooks": [
                    {
                        "type": "command",
                        # Use the kb_core CLI itself so the hook is shell-agnostic:
...
def main() -> None:
    ...
    stdout early, kb-core treats it as success instead of crashing with an
...
def _run_cli() -> None:
    ...
    if len(sys.argv) >= 2 and sys.argv[1] in ("-v", "--version", "version"):
        print(f"kb-core {__version__}")
        return

    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help", "-?"):
        print("Usage: kb-core <command>")
        print()
        print("Commands:")
        ...
        print("  uninstall               remove kb-core from all detected platforms in one shot")
        print("    --purge                 also delete kb-core-out/ directory")
        ...
        print("                            (default kb-core-out/graph.json)")
        ...
        print("  clone <github-url>      clone a GitHub repo locally and print its path for /kb-core")
        ...
        print("    --out <path>            output path (default: kb-core-out/merged-graph.json)")
        print("    --branch <branch>       checkout a specific branch (default: repo default)")
        print("    --out <dir>             clone to a custom directory (default: ~/.kb-core/repos/<owner>/<repo>)")
        ...
        print("                            (also: KB_CORE_FORCE=1 env var; use after refactors that delete code)")
        print("    --no-cluster            skip clustering, write raw extraction only")
        print("  cluster-only <path>     rerun clustering on an existing graph.json and regenerate report")
        print("    --no-viz                skip graph.html generation (useful for >5000 node graphs / CI)")
        print("    --graph <path>          path to graph.json (default <path>/kb-core-out/graph.json)")
        ...
        print("  save-result             save a Q&A result to kb-core-out/memory/ for graph feedback loop")
        ...
        print("    --memory-dir DIR        memory directory (default: kb-core-out/memory)")
        print("  reflect                 aggregate kb-core-out/memory/ outcomes into a deterministic lessons doc")
        print("    --memory-dir DIR        memory directory (default: kb-core-out/memory)")
        print("    --out FILE              output path (default: kb-core-out/reflections/LESSONS.md)")
        print("    --graph PATH            graph.json, for community grouping + dropping stale nodes (optional)")
        print("    --analysis PATH         .kb_core_analysis.json (optional, auto-detected next to --graph)")
        print("    --labels PATH           .kb_core_labels.json (optional, auto-detected next to --graph)")
        ...
        print("                            manifest gate and semantic cache reads (env: KB_CORE_FORCE=1)")
        ...
        print("    --out DIR, --output DIR output dir (default: <path>); writes <DIR>/kb-core-out/")
        print("    --google-workspace      export .gdoc/.gsheet/.gslides shortcuts via gws before extraction")
        print("    --no-gitignore         ignore .gitignore and .git/info/exclude (prioritizes .kb_coreignore)")
        print("    --no-cluster            skip clustering, write raw extraction only")
        ...
        print("  global add <graph.json>  add/update a project graph in the global graph (~/.kb-core/global-graph.json)")
        ...
        print("  cursor install          write .cursor/rules/kb_core.mdc (Cursor)")
        print("  cursor uninstall        remove .cursor/rules/kb_core.mdc")
        print("  claude install          write kb-core section to CLAUDE.md + PreToolUse hook (Claude Code)")
        print("  claude uninstall        remove kb-core section from CLAUDE.md + PreToolUse hook")
        print("  codebuddy install       write kb-core section to CODEBUDDY.md + PreToolUse hook (CodeBuddy)")
        print("  codebuddy uninstall     remove kb-core section from CODEBUDDY.md + PreToolUse hook")
        print("  codex install           write kb-core section to AGENTS.md (Codex)")
        print("  codex uninstall         remove kb-core section from AGENTS.md")
        print(
            "  opencode install        write kb-core section to AGENTS.md + tool.execute.before plugin (OpenCode)"
        )
        print(
            "  opencode uninstall      remove kb-core section from AGENTS.md + plugin"
        )
        print(
        ...
        print("  aider install           write kb-core section to AGENTS.md (Aider)")
        print("  aider uninstall         remove kb-core section from AGENTS.md")
        print(
            "  copilot install         copy kb-core skill to ~/.copilot/skills (GitHub Copilot CLI)"
        )
        print("  copilot uninstall       remove kb-core skill from ~/.copilot/skills")
        print(
        ...
        print(
            "  claw install            write kb-core section to AGENTS.md (OpenClaw)"
        )
        print("  claw uninstall          remove kb-core section from AGENTS.md")
        print(
            "  droid install           write kb-core section to AGENTS.md (Factory Droid)"
        )
        print("  droid uninstall        remove kb-core section from AGENTS.md")
        print("  trae install            write kb-core section to AGENTS.md (Trae)")
        print("  trae uninstall         remove kb-core section from AGENTS.md")
        print("  trae-cn install         write kb-core section to AGENTS.md (Trae CN)")
        print("  trae-cn uninstall      remove kb-core section from AGENTS.md")
        print(
        ...
        print(
            "  hermes install          write skill to ~/.hermes/skills/kb-core/ (Hermes)"
        )
        print("  hermes uninstall        remove skill from ~/.hermes/skills/kb-core/")
        print(
            "  kiro install            write skill to .kiro/skills/kb-core/ + steering file (Kiro IDE/CLI)"
        )
        print("  kiro uninstall          remove skill + steering file")
        print("  pi install              write skill to ~/.pi/agent/skills/kb-core/ (Pi coding agent)")
        print("  pi uninstall            remove skill from ~/.pi/agent/skills/kb-core/")
        print("  devin install           write skill to ~/.config/devin/skills/kb-core/ (Devin CLI)")
        print("  devin uninstall         remove skill from ~/.config/devin/skills/kb-core/")
        print()
        return
    ...
    if cmd not in _FREE_TEXT_CMDS and any(a in {"-h", "--help", "-?"} for a in sys.argv[2:]):
        print(f"Run 'kb-core --help' for full usage.")
        return
