...
from kb_core.google_workspace import (
    GOOGLE_WORKSPACE_EXTENSIONS,
    convert_google_workspace_file,
    google_workspace_enabled,
)
from kb_core.paths import KB_CORE_OUT, out_path
...
# attacker-controllable (kb_core runs on cloned/shared folders), and .docx/.xlsx
...
def classify_file(path: Path) -> FileType | None:
    ...
    from kb_core.manifest_ingest import is_package_manifest_path
...
def xlsx_extract_structure(path: Path) -> dict:
    ...
    Returns a nodes/edges dict compatible with the kb-core extract pipeline.
...
def convert_office_file(path: Path, out_dir: Path, root: "Path | None" = None) -> Path | None:
    ...
    # byte-identical sidecars — unbounded duplicates when kb_core-out/ is
    ...
    import unicodedata
    if root is None:
        # Default layout: out_dir is <root>/<kb_core-out>/converted.
        root = out_dir.parent.parent
    try:
        key = path.resolve().relative_to(Path(root).resolve()).as_posix()
    except (ValueError, OSError):
        # Not under the scan root (custom KB_CORE_OUT layouts, --include
...
def count_words(path: Path) -> int:
    try:
        ...
        # cannot help, and `kb_core update` hangs with no output. os.stat
...
_SKIP_DIRS = {
    ...
    "kb-core-out",  # never treat the default output as source input (#524)
    ...
    ".kb-core",  # kb_core's own extraction cache — never index self-generated data
...
def _parse_gitignore_line(raw: str) -> str:
    """Parse one raw line from a .kb_coreignore file per gitignore spec.
...
def _git_info_exclude(vcs_root: Path) -> Path | None:
    ...
    a directory without any ``.gitignore`` entry. kb-core only read
    ``.gitignore``/``.kb_coreignore``, so it walked into those worktree copies and
...
def _read_ignore_text(path: Path) -> str:
    ...
    for enc in (fallback, "latin-1"):
        ...
        if key not in _warned_ignore_encodings:
            ...
            print(
                f"[kb-core] WARNING: {path} is not valid UTF-8; read it as "
...
def _load_dir_own_ignore(d: Path, *, gitignore: bool = True) -> list[tuple[Path, str]]:
    """Read .gitignore/.kb_coreignore directly inside *d* (not its ancestors).

    Merges .gitignore and .kb_coreignore for this one directory (#1363):
    .gitignore is read first and .kb_coreignore last, so .kb_coreignore
    patterns (including `!` negations) win on conflict via last-match-wins;
    adding a .kb_coreignore can only ever exclude MORE, never re-include a
    .gitignore-excluded file (#945 kept: a dir with only a .gitignore still
    gets sensible defaults).

    Shared by `_load_kb_coreignore` (ancestor chain, loaded once before the
    ...
    for fname in ((".gitignore", ".kb_coreignore") if gitignore else (".kb_coreignore",)):
        ignore_file = d / fname
...
def _load_kb_coreignore(root: Path, *, gitignore: bool = True) -> list[tuple[Path, str]]:
    """Read .kb_coreignore files and return (anchor_dir, pattern) pairs.
    ...
    # per-directory .gitignore/.kb_coreignore — so load it first (lowest priority
...
# `kb_core watch` process spanning many repos could still accumulate keys over
...
def _is_ignored(
    ...
    *,
    ...
    """Return True if the path should be ignored per .kb_coreignore patterns.
...
def _is_scan_ignored(
    ...
    *,
    ...
    contains only .kb_coreignore/--exclude rules, which remain authoritative
...
def ignored_predicate(
    ...
    *,
    ...
    .kb_coreignore/.gitignore chain (_load_kb_coreignore), CLI/persisted
    ...
    """
    root = root.resolve()
    patterns = _load_kb_coreignore(root, gitignore=gitignore)
    explicit_patterns = _load_kb_coreignore(root, gitignore=False)
    # Only shell out to git when .gitignore actually contributes patterns beyond
    # the explicit (.kb_coreignore/--exclude) set: with no .gitignore in play,
    ...
    cache: dict[Path, bool] = {}
    explicit_cache: dict[Path, bool] = {}
    # root's own ignore file is the last entry of _load_kb_coreignore's chain.
...
def detect(root: Path, *, follow_symlinks: bool | None = None, google_workspace: bool | None = None, extra_excludes: list[str] | None = None, cache_root: Path | None = None, gitignore: bool = True) -> dict:
    root = root.resolve()
    configured_out_dir = root / KB_CORE_OUT
    ...
    # .kb_coreinclude support was removed (#2112): its loader and matchers had
    ...
    if (root / ".kb_coreinclude").is_file():
        import sys as _sys
        print(
            "[kb-core] WARNING: .kb_coreinclude is no longer supported "
    ...
    def _wc(path: Path) -> int:
        ...
        from kb_core import cache as _cache
        return _cache.cached_word_count(path, root, count_words, cache_root=cache_root)

    skipped_sensitive: list[str] = []
    unclassified: list[str] = []
    # Files/dirs dropped by a .gitignore/.kb_coreignore rule. Recorded so an
    ...
    ignore_patterns = _load_kb_coreignore(root, gitignore=gitignore)
    explicit_ignore_patterns = _load_kb_coreignore(root, gitignore=False)
    ...
    # so they win over any .kb_coreignore/.gitignore rules (#947).
    if extra_excludes:
        for pat in extra_excludes:
            line = _parse_gitignore_line(pat)
            if line:
    ...
    # Always include kb_core-out/memory/ - query results filed back into the graph
    memory_dir = root / KB_CORE_OUT / "memory"
    scan_paths = [root]
    ...
    def _on_walk_error(err: OSError) -> None:
        ...
        print(
            f"[kb-core] WARNING: could not scan {target} ({err}); "
    ...
    converted_dir = out_base / KB_CORE_OUT / "converted"
        ...
        if ftype:
            if p.suffix.lower() in GOOGLE_WORKSPACE_EXTENSIONS:
                if not google_workspace:
                    skipped_sensitive.append(
                        ...
                        "or set KB_CORE_GOOGLE_WORKSPACE=1]"
                    )
                    continue
                try:
            ...
            if p.suffix.lower() in OFFICE_EXTENSIONS:
                ...
                if md_path:
                    ...
                    skipped_sensitive.append(str(p) + " [office conversion failed - pip install kb-core[office]]")
                continue
    ...
    return {
        ...
        "kb_coreignore_patterns": len(ignore_patterns),
...
def _to_relative_for_storage(key: str, root: Path) -> str:
    ...
    fallback in :func:`kb_core.watch._relativize_source_files` so the
...
def load_manifest(
    ...
    *,
    ...
    manifests with absolute keys pass through unchanged, so a kb-core-out/
...
def save_manifest(
    ...
    *,
    ...
    kind="ast"      — written by `kb-core update` (AST-only rebuild). Stamps
                      ast_hash; preserves an existing semantic_hash only when
                      the file content is unchanged (mtime + hash match).
    kind="semantic" — written by `kb-core extract` after semantic extraction.
    ...
    .kb_coreignore/.gitignore/--exclude) are dropped instead of surviving
...
def detect_incremental(
    ...
    *,
    ...
    kind="semantic" (default for extract): a file is "changed" when its
        ...
        semantic extraction pass. Use this for `kb-core extract` so that
        files touched by `kb-core update` (AST-only) are re-extracted
        semantically.
    kind="ast": a file is "changed" when its ast_hash is missing or its
        content has changed. Use this for `kb-core update`.
