...
def _platform_skill_destination(platform_name: str, *, project: bool = False, project_dir: Path | None = None) -> Path:
    ...
    if platform_name == "gemini":
        if project:
            return (project_dir or Path(".")) / ".gemini" / "skills" / "kb-core" / "SKILL.md"
        if platform.system() == "Windows":
            return Path.home() / ".agents" / "skills" / "kb-core" / "SKILL.md"
        return Path.home() / ".gemini" / "skills" / "kb-core" / "SKILL.md"

    if platform_name == "opencode":
        if project:
            return (project_dir or Path(".")) / ".opencode" / "skills" / "kb-core" / "SKILL.md"
        return Path.home() / ".config" / "opencode" / "skills" / "kb-core" / "SKILL.md"

    if platform_name == "hermes":
        if project:
            return (project_dir or Path(".")) / ".hermes" / "skills" / "kb-core" / "SKILL.md"
        # On Windows, Hermes scans %LOCALAPPDATA%\hermes\skills, not ~/.hermes (#1403).
        if platform.system() == "Windows":
            local_appdata = Path(os.environ.get("LOCALAPPDATA") or (Path.home() / "AppData" / "Local"))
            return local_appdata / "hermes" / "skills" / "kb-core" / "SKILL.md"
        return Path.home() / ".hermes" / "skills" / "kb-core" / "SKILL.md"

    if platform_name == "devin":
        if project:
            return (project_dir or Path(".")) / ".devin" / "skills" / "kb-core" / "SKILL.md"
        return Path.home() / ".config" / "devin" / "skills" / "kb-core" / "SKILL.md"

    if platform_name == "amp":
        if project:
            return (project_dir or Path(".")) / ".agents" / "skills" / "kb-core" / "SKILL.md"
        return Path.home() / ".config" / "agents" / "skills" / "kb-core" / "SKILL.md"
    ...
    if platform_name in ("antigravity", "antigravity-windows"):
        ...
        return Path.home() / ".gemini" / "config" / "skills" / "kb-core" / "SKILL.md"
...
def _packaged_skill_refs_dir(platform_name: str) -> Path | None:
    ...
    ``kb-core/skills/<bundle>/references/``. Reuse keys (e.g. trae-cn) point at
    their twin's bundle.
...
def _claude_pretooluse_hooks(strict: bool = False) -> "list[dict]":
    """kb-core's Claude/Codebuddy PreToolUse hooks, resolved at install time.
...
_PLATFORM_CONFIG: dict[str, dict] = {
    ...
    "opencode": {
        ...
        "skill_dst": Path(".config") / "opencode" / "skills" / "kb-core" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "opencode",
    },
    "kilo": {
        "skill_file": "skill-kilo.md",
        "skill_dst": Path(".config") / "kilo" / "skills" / "kb-core" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "kilo",
    },
    "aider": {
        ...
        "skill_dst": Path(".aider") / "kb-core" / "SKILL.md",
        "claude_md": False,
    },
    "copilot": {
    ...
    "claw": {
        ...
        "skill_dst": Path(".openclaw") / "skills" / "kb-core" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "claw",
    },
    "droid": {
        "skill_file": "skill-droid.md",
        "skill_dst": Path(".factory") / "skills" / "kb-core" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "droid",
    },
    "trae": {
        "skill_file": "skill-trae.md",
        "skill_dst": Path(".trae") / "skills" / "kb-core" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "trae",
    },
    "trae-cn": {
        ...
        "skill_file": "skill-trae.md",
        "skill_dst": Path(".trae-cn") / "skills" / "kb-core" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "trae",
    },
    "hermes": {
        ...
        "skill_file": "skill-claw.md",
        "skill_dst": Path(".hermes") / "skills" / "kb-core" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "claw",
    },
    "kiro": {
        "skill_file": "skill-kiro.md",
        "skill_dst": Path(".kiro") / "skills" / "kb-core" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "kiro",
    },
    "pi": {
        "skill_file": "skill-pi.md",
        "skill_dst": Path(".pi") / "agent" / "skills" / "kb-core" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "pi",
    },
    "codebuddy": {
        ...
        "skill_file": "skill.md",
        "skill_dst": Path(".codebuddy") / "skills" / "kb-core" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "claude",
    },
    "antigravity": {
        ...
        "skill_file": "skill.md",
        "skill_dst": Path(".agents") / "skills" / "kb-core" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "claude",
    },
    "antigravity-windows": {
    ...
    "kimi": {
        # Reuses claude's split bundle (shares skill.md).
        "skill_file": "skill.md",
        "skill_dst": Path(".kimi") / "skills" / "kb-core" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "claude",
    },
    "amp": {
    ...
    "devin": {
        ...
        # User scope: ~/.config/devin/skills/kb_core/SKILL.md
        # Project scope: .devin/skills/kb_core/SKILL.md (overridden in _platform_skill_destination)
        "skill_dst": Path(".config") / "devin" / "skills" / "kb-core" / "SKILL.md",
        "claude_md": False,
    },
}
...
def install(platform: str = "claude", *, project: bool = False, project_dir: Path | None = None) -> None:
    ...
    if platform == "kilo":
        # Kilo Code also supports a native /kb_core command file.
        command_src = Path(__file__).parent / "command-kilo.md"
        if not command_src.exists():
            print(
                f"error: command-kilo.md not found in package - reinstall kb-core",
                file=sys.stderr,
            )
            sys.exit(1)
        command_dst = Path.home() / ".config" / "kilo" / "command" / "kb_core.md"
    ...
    if platform == "codebuddy":
        ...
        registration = _skill_registration("~/.codebuddy/skills/kb-core/SKILL.md")
...
_CODEBUDDY_MD_MARKER = "## kb-core"
_AGENTS_MD_MARKER = "## kb-core"
_GEMINI_MD_MARKER = "## kb-core"
def _gemini_hook() -> dict:
    """Gemini CLI BeforeTool hook, resolved to a shell-agnostic `kb-core` call."""
    exe = _resolve_kb_core_exe()
    if " " in exe and not exe.startswith('"'):
        exe = f'"{exe}"'
    return {
...
def _install_gemini_hook(project_dir: Path) -> None:
    ...
    if not isinstance(before_tool, list):
        _refuse_to_modify(settings_path)
    hooks["BeforeTool"] = [
        h for h in before_tool if "kb-core" not in str(h)
    ]
...
def _uninstall_gemini_hook(project_dir: Path) -> None:
    ...
    filtered = [h for h in before_tool if "kb-core" not in str(h)]
...
def gemini_uninstall(project_dir: Path | None = None, *, project: bool = False, remove_user_skill: bool | None = None) -> None:
    """Remove the kb-core section from GEMINI.md, uninstall hook, and remove skill file.
    ...
    if cleaned is None:
        print("kb-core section not found in GEMINI.md - nothing to do")
        return
    if cleaned:
...
_ANTIGRAVITY_RULES_PATH = Path(".agents") / "rules" / "kb_core.md"
_ANTIGRAVITY_WORKFLOW_PATH = Path(".agents") / "workflows" / "kb_core.md"
...
Follow the kb-core skill to run the full pipeline.

If no path argument is given, use `.` (current directory).
"""
def _kiro_install(project_dir: Path) -> None:
    """Write kb-core skill + steering file for Kiro IDE/CLI."""
    project_dir = project_dir or Path(".")

    # Skill file + references/ sidecar + .kb_core_version stamp via the shared
    ...
    # Steering file → .kiro/steering/kb_core.md (always-on)
    steering_dir = project_dir / ".kiro" / "steering"
    steering_dir.mkdir(parents=True, exist_ok=True)
    steering_dst = steering_dir / "kb_core.md"
    if steering_dst.exists() and steering_dst.read_text(encoding="utf-8") == _always_on("kiro-steering"):
        print(f"  .kiro/steering/kb_core.md  ->  already configured (no change)")
    else:
        # File is wholly kb_core-owned. Overwrite on upgrade so older
        ...
        print(f"  .kiro/steering/kb_core.md  ->  always-on steering {action}")

    print()
    print("Kiro will now read the knowledge graph before every conversation.")
    print("Use /kb-core to build or update the graph.")
def _kiro_uninstall(project_dir: Path) -> None:
    """Remove kb-core skill + steering file for Kiro."""
    project_dir = project_dir or Path(".")
    removed = []

    # Skill + .kb_core_version + references/ sidecar + empty-dir walk.
    ...
    steering_dst = project_dir / ".kiro" / "steering" / "kb_core.md"
...
def _antigravity_finalize(skill_dst: Path, project_dir: Path) -> None:
    ...
    writes ``.agents/rules/kb_core.md`` and ``.agents/workflows/kb_core.md``
    ...
    if rules_path.exists():
        ...
        if _always_on("antigravity-rules").strip() != existing.strip():
            ...
            print(f"kb-core rule updated at {rules_path.resolve()}")
        else:
            print(f"kb-core rule already configured at {rules_path.resolve()} (no change)")
    else:
        rules_path.write_text(_always_on("antigravity-rules"), encoding="utf-8")
        print(f"kb-core rule written to {rules_path.resolve()}")

    # .agents/workflows/kb_core.md
    ...
    if wf_path.exists():
        ...
        if _ANTIGRAVITY_WORKFLOW.strip() != existing.strip():
            ...
            print(f"kb-core workflow updated at {wf_path.resolve()}")
        else:
            print(f"kb-core workflow already configured at {wf_path.resolve()} (no change)")
    else:
        wf_path.write_text(_ANTIGRAVITY_WORKFLOW, encoding="utf-8")
        print(f"kb-core workflow written to {wf_path.resolve()}")
def _antigravity_install(project_dir: Path) -> None:
    """Install kb-core for Google Antigravity (global skill + .agents/rules + .agents/workflows)."""
    # Copy the skill to ~/.gemini/config/skills/kb_core/SKILL.md (global), then
    ...
    print("codebase questions. Run /kb-core first to build the graph.")
    print()
    print(
    ...
    print('    "command": "uv",')
    print(
        '    "args": ["run", "--with", "kb-core", "--with", "mcp", "-m", "kb_core.serve", "${workspace.path}/kb-core-out/graph.json"]'
    )
    print("  }")
def _antigravity_uninstall(project_dir: Path, *, project: bool = False) -> None:
    """Remove kb-core Antigravity rules, workflow, and skill files."""
    # Remove rules file
    rules_path = project_dir / _ANTIGRAVITY_RULES_PATH
    if rules_path.exists():
        rules_path.unlink()
        print(f"kb-core rule removed from {rules_path.resolve()}")
    else:
        print("No kb-core Antigravity rule found - nothing to do")

    # Remove workflow file
    wf_path = project_dir / _ANTIGRAVITY_WORKFLOW_PATH
    if wf_path.exists():
        wf_path.unlink()
        print(f"kb-core workflow removed from {wf_path.resolve()}")

    # Remove skill file
    skill_dst = _platform_skill_destination("antigravity", project=project, project_dir=project_dir)
    if skill_dst.exists():
        skill_dst.unlink()
        print(f"kb-core skill removed from {skill_dst}")
    version_file = skill_dst.parent / ".kb_core_version"
    if version_file.exists():
        version_file.unlink()
    refs_dir = skill_dst.parent / "references"
    ...
    for d in (
        ...
        try:
            ...
            break
_CURSOR_RULE_PATH = Path(".cursor") / "rules" / "kb_core.mdc"
_CURSOR_RULE = """\
---
...
**MANDATORY: Before using Read, Grep, Glob, or Bash to explore the codebase, you MUST run kb-core first:**
- `kb-core query "<question>"` — scoped subgraph for any codebase or architecture question
- `kb-core path "<A>" "<B>"` — dependency path between two symbols
- `kb-core explain "<concept>"` — all nodes related to a concept

This applies to YOU and to every subagent you spawn. Include this rule explicitly in every subagent prompt that involves code exploration. Do not skip kb-core because files are "already known" or because you are executing a plan — the graph surfaces cross-file dependencies and INFERRED edges that grep and Read cannot find.

Only use Read/Grep/Glob directly when:
1. kb-core has already oriented you and you need to modify or debug specific lines
2. `kb-core-out/graph.json` does not exist yet

- If `kb-core-out/wiki/index.md` exists, navigate it instead of reading raw files
- Read `kb-core-out/GRAPH_REPORT.md` only for broad architecture review when query/path/explain do not surface enough context
- After modifying code files, run `kb-core update .` to keep the graph current (AST-only, no API cost)
"""
def _cursor_install(project_dir: Path) -> None:
    """Write .cursor/rules/kb_core.mdc with alwaysApply: true."""
    ...
    if rule_path.exists() and rule_path.read_text(encoding="utf-8") == _CURSOR_RULE:
        print(f"kb-core rule at {rule_path} already configured (no change)")
        return
    # File is wholly kb_core-owned. Overwrite on upgrade so older
    # report-first wording does not silently linger (issue #580).
    action = "updated" if rule_path.exists() else "written"
    rule_path.write_text(_CURSOR_RULE, encoding="utf-8")
    print(f"kb-core rule {action} at {rule_path.resolve()}")
    print()
    print("Cursor will now always include the knowledge graph context.")
    print("Run /kb-core . first to build the graph if you haven't already.")
def _cursor_uninstall(project_dir: Path) -> None:
    """Remove .cursor/rules/kb_core.mdc."""
    rule_path = (project_dir or Path(".")) / _CURSOR_RULE_PATH
    if not rule_path.exists():
        print("No kb-core Cursor rule found - nothing to do")
        return
    rule_path.unlink()
    print(f"kb-core Cursor rule removed from {rule_path.resolve()}")
# Devin CLI — .windsurf/rules/kb_core.md (always-on context)
# Devin reads .windsurf/rules/*.md files the same way Windsurf IDE does.
_DEVIN_RULES_PATH = Path(".windsurf") / "rules" / "kb_core.md"
_DEVIN_RULES = """\
## kb-core
...
Rules:
- For codebase or architecture questions, when `kb-core-out/graph.json` exists, first run `kb-core query "<question>"` (or `kb-core path "<A>" "<B>"` / `kb-core explain "<concept>"`). These return a scoped subgraph, usually much smaller than `GRAPH_REPORT.md` or raw grep output.
- If kb-core-out/wiki/index.md exists, navigate it instead of reading raw files
...
def _devin_rules_install(project_dir: Path) -> None:
    """Write .windsurf/rules/kb_core.md for always-on Devin context."""
...
def _devin_rules_uninstall(project_dir: Path) -> None:
    """Remove .windsurf/rules/kb_core.md."""
    rules_path = (project_dir or Path(".")) / _DEVIN_RULES_PATH
...
export const KB CorePlugin = async ({ directory }) => {
  let reminded = false;

  return {
    "tool.execute.before": async (input, output) => {
      if (reminded) return;
      if (!existsSync(join(directory, "kb-core-out", "graph.json"))) return;
      ...
      if (input.tool === "bash") {
        ...
        output.args.command =
          'echo "[kb-core] Knowledge graph available. Read kb-core-out/GRAPH_REPORT.md for god nodes and architecture context before searching files." ; ' +
          output.args.command;
        reminded = true;
      }
    },
...
"""
_KILO_PLUGIN_PATH = Path(".kilo") / "plugins" / "kb_core.js"
...
def _install_kilo_plugin(project_dir: Path) -> None:
    """Write kb_core.js plugin and register it without rewriting user JSONC."""
...
def _uninstall_kilo_plugin(project_dir: Path) -> None:
    """Remove kb_core.js plugin and deregister it without rewriting user JSONC."""
    plugin_file = project_dir / _KILO_PLUGIN_PATH
...
_OPENCODE_PLUGIN_JS = """\
// kb-core OpenCode plugin
// Injects a knowledge graph reminder before bash tool calls when the graph exists.
//
// IMPORTANT: keep the reminder string free of backticks and $(...) constructs.
// The hook prepends `echo "<reminder>" && <cmd>` to the user's bash command;
// backticks inside the double-quoted echo trigger bash command substitution,
// which both corrupts tool output and silently executes the very kb-core
  ...
  return {
    "tool.execute.before": async (input, output) => {
      ...
      if (input.tool === "bash") {
        ...
        output.args.command =
          'echo "[kb-core] knowledge graph at kb-core-out/. For focused questions, run kb-core query with your question (scoped subgraph, usually much smaller than GRAPH_REPORT.md) instead of grepping raw files. Read GRAPH_REPORT.md only for broad architecture context." ; ' +
          output.args.command;
        reminded = true;
      }
    },
...
"""
_OPENCODE_PLUGIN_PATH = Path(".opencode") / "plugins" / "kb_core.js"
_OPENCODE_CONFIG_PATH = Path(".opencode") / "opencode.json"
def _install_opencode_plugin(project_dir: Path) -> None:
    """Write kb_core.js plugin and register it in opencode.json."""
...
def _uninstall_opencode_plugin(project_dir: Path) -> None:
    """Remove kb_core.js plugin and deregister from opencode.json."""
    plugin_file = project_dir / _OPENCODE_PLUGIN_PATH
    if plugin_file.exists():
        plugin_file.unlink()
...
def _agents_install(project_dir: Path, platform: str) -> None:
    """Write the kb-core section to the local AGENTS.md for always-on platforms."""
...
def _amp_legacy_cleanup() -> None:
    """Best-effort removal of the pre-fix ~/.amp/skills/kb-core install dir.

    Older kb-core versions wrote the Amp skill to ~/.amp/skills, which Amp does
    ...
    """
    legacy = Path.home() / ".amp" / "skills" / "kb-core"
    if legacy.exists():
...
def _agents_platform_install(project_dir: Path | None = None) -> None:
    """`kb-core agents install`: skill into ~/.agents/skills + AGENTS.md.
    ...
    rendered hooks reference, which points at `kb-core agents install`. The bare
    `kb-core install --platform agents` path stays skill-only (via install()),
...
def _agents_platform_uninstall(project_dir: Path | None = None) -> None:
    """`kb-core agents uninstall`: remove the skill and the AGENTS.md section."""
...
def _project_install(platform_name: str, project_dir: Path | None = None, strict: bool = False) -> None:
    ...
    if platform_name in ("claude", "windows"):
        ...
        # root. `agents` -> ./.agents/skills/kb_core/SKILL.md.
        skill_dst = _copy_skill_file(platform_name, project=True, project_dir=project_dir)
        _print_project_git_add_hint([_project_scope_root(skill_dst, project_dir)])
    else:
        install(platform=platform_name, project=True, project_dir=project_dir)
...
def claude_uninstall(project_dir: Path | None = None, *, project: bool = False, remove_user_skill: bool | None = None) -> None:
    ...
    Mirrors gemini_uninstall: the bare `kb-core uninstall` and `kb-core claude
...
def codebuddy_install(project_dir: Path | None = None) -> None:
    """Install the kb-core skill and CODEBUDDY.md section for CodeBuddy."""
...
def _install_codebuddy_hook(project_dir: Path) -> None:
    """Add kb-core PreToolUse hook to .codebuddy/settings.json."""
...
def _uninstall_codebuddy_hook(project_dir: Path) -> None:
    """Remove kb-core PreToolUse hook from .codebuddy/settings.json."""
    settings_path = project_dir / ".codebuddy" / "settings.json"
    if not settings_path.exists():
        return
    try:
...
def codebuddy_uninstall(project_dir: Path | None = None, *, project: bool = False, remove_user_skill: bool | None = None) -> None:
    """Remove the kb-core skill tree (SKILL.md + references/) and the CODEBUDDY.md section.
    ...
    if cleaned is None:
        print("kb-core section not found in CODEBUDDY.md - nothing to do")
        return
...
def dispatch_install_cli(cmd: str) -> bool:
    ...
    if cmd == "install":
        ...
        if subcmd == "install":
            ...
            print("Usage: kb-core codebuddy [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "gemini":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: kb-core gemini [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "cursor":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: kb-core cursor [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "vscode":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
        ...
        if subcmd == "install":
            ...
            print("Usage: kb-core kilo [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "kiro":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: kb-core kiro [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "devin":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: kb-core devin [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "pi":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: kb-core pi [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "amp":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: kb-core amp [install|uninstall]", file=sys.stderr)
            sys.exit(1)
        ...
        if subcmd == "install":
            ...
            print(f"Usage: kb-core {cmd} [install|uninstall]", file=sys.stderr)
            sys.exit(1)
        ...
        if subcmd == "install":
            ...
            print("Usage: kb-core antigravity [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    return True
