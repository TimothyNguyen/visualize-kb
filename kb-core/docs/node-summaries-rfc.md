...
This RFC proposes an optional way for kb-core to expose compact file-level
...
`kb-core query`, `kb-core explain`, MCP node lookup, and graph navigation.

## Goals

- Help agents choose relevant files with less context.
- Preserve kb-core's offline, deterministic behavior by default.
...
- Replacing `kb-core explain`; summaries should make `explain` more useful.
...
- Display a summary in `kb-core explain <node>` when one exists.
- Include summaries in `kb-core serve` / MCP node lookup when one exists.
...
{
  "label": "extract.py",
  "source_file": "kb_core/extract.py",
...
be fetched through `kb-core explain`, `kb-core query`, or direct file reads
when needed.
...
```bash
kb-core . --summarize-nodes
kb-core explain "extract.py"
```
...
3. Surface summaries in `kb-core explain`.
4. Surface summaries in `kb-core serve` / MCP node lookup.
...
- Extend summaries to class or module-level nodes if file-level summaries prove
  useful.
- Allow `kb-core query` to include summaries for returned nodes under a budget.
- Add cache/freshness metadata if summaries are generated separately from the
  main graph.

## Questions for maintainers and users

1. Should kb-core prefer one artifact (`graph.json`) or keep generated text in a
   sidecar?
2. Should deterministic file-level summaries be generated during graph creation,
   or only through an explicit command such as `kb-core summarize`?
