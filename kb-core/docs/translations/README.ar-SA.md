...
**مهارة لمساعد برمجة الذكاء الاصطناعي.** اكتب `/kb-core` في Claude Code أو Codex أو OpenCode أو Cursor أو Gemini CLI أو GitHub Copilot CLI أو VS Code Copilot Chat أو Aider أو OpenClaw أو Factory Droid أو Trae أو Hermes أو Kiro أو Google Antigravity — يقرأ ملفاتك ويبني رسماً بيانياً للمعرفة ويعيد إليك البنية التي لم تكن تعلم بوجودها. افهم قاعدة الكود بشكل أسرع. اكتشف "السبب" وراء القرارات المعمارية.

متعدد الوسائط بالكامل. أضف كوداً أو ملفات PDF أو markdown أو لقطات شاشة أو رسوماً بيانية أو صور سبورة أو صوراً بلغات أخرى أو ملفات فيديو وصوت — يستخرج kb-core المفاهيم والعلاقات من كل ذلك ويربطها في رسم بياني واحد. يتم نسخ مقاطع الفيديو محلياً باستخدام Whisper. يدعم 25 لغة برمجة عبر tree-sitter AST (Python, JS, TS, Go, Rust, Java, C, C++, Ruby, C#, Kotlin, Scala, PHP, Swift, Lua, Zig, PowerShell, Elixir, Objective-C, Julia, Verilog, SystemVerilog, Vue, Svelte, Dart).

> يحتفظ Andrej Karpathy بمجلد `/raw` يضع فيه الأوراق البحثية والتغريدات ولقطات الشاشة والملاحظات. kb-core هو الإجابة على تلك المشكلة — **71.5 مرة** أقل في الرموز لكل استعلام مقارنةً بقراءة الملفات الخام، مستمر عبر الجلسات، صادق حول ما تم العثور عليه مقابل ما تم استنتاجه.

</div>

```
/kb-core .                        # يعمل مع أي مجلد — الكود، الملاحظات، الأوراق البحثية، كل شيء
```
...
أضف ملف `.kb-coreignore` لاستبعاد المجلدات:
...
> **الحزمة الرسمية:** اسم حزمة PyPI هو `kb-core` (تثبيت بـ `pip install kb-core`). الحزم الأخرى المسماة `kb-core*` على PyPI ليست تابعة لهذا المشروع. المستودع الرسمي الوحيد هو [safishamsi/kb-core](https://github.com/safishamsi/kb-core).
...
| Claude Code (Windows) | `kb-core install` (كشف تلقائي) أو `kb-core install --platform windows` |
...
ملاحظة: يستخدم Codex `$` بدلاً من `/` للمهارات، لذا اكتب `$kb-core .`.
...
```
/kb-core                          # المجلد الحالي
/kb-core ./raw                    # مجلد محدد
/kb-core ./raw --update           # إعادة استخراج الملفات المتغيرة فقط
/kb-core ./raw --directed         # رسم بياني موجّه
/kb-core ./raw --no-viz           # تقرير + JSON فقط، بدون HTML
/kb-core ./raw --obsidian         # إنشاء Obsidian vault

/kb-core add https://arxiv.org/abs/1706.03762   # جلب ورقة بحثية
/kb-core add <video-url>                         # تحميل صوت، نسخ، إضافة
/kb-core query "ما الذي يربط Attention بالمحسِّن؟"
/kb-core path "DigestAuth" "Response"
/kb-core explain "SwinTransformer"

kb-core hook install              # تثبيت Git hooks
kb-core update ./src              # إعادة استخراج ملفات الكود، بدون LLM
kb-core watch ./src               # تحديث تلقائي للرسم البياني
```
...
**Git hooks** (`kb-core hook install`) — يثبّت خطافات post-commit وpost-checkout.

## الخصوصية

يرسل kb-core محتوى الملفات إلى API نموذج مساعد الذكاء الاصطناعي الخاص بك للاستخراج الدلالي من المستندات والأوراق البحثية والصور. تتم معالجة ملفات الكود محلياً عبر tree-sitter AST. يتم نسخ ملفات الفيديو والصوت محلياً باستخدام faster-whisper. لا قياس عن بُعد، لا تتبع للاستخدام.
...
## مبني على kb-core — Penpax

[**Penpax**](https://safishamsi.github.io/penpax.ai) هو الطبقة المؤسسية فوق kb_core. حيث يحوّل kb-core مجلداً من الملفات إلى رسم بياني للمعرفة، يطبّق Penpax نفس الرسم البياني على حياتك المهنية بأكملها — باستمرار.
