...
**Sun'iy intellektga asoslangan kod yordamchilari uchun ko'nikma.** Claude Code, Codex, OpenCode, Cursor, Gemini CLI, GitHub Copilot CLI, VS Code Copilot Chat, Aider, OpenClaw, Factory Droid, Trae, Hermes, Kiro yoki Google Antigravity da `/kb-core` deb yozing — u sizning fayllaringizni o'qiydi, bilim grafini quradi va siz bilmagan tuzilmani sizga qaytaradi. Kod bazasini tezroq tushuning. Arxitektura qarorlari ortidagi "nima uchun" savoliga javob toping.

To'liq multimodal. Kod, PDF, markdown, ekran tasvirlari, diagrammalar, doska suratlari, boshqa tillardagi tasvirlar, video va audio fayllarni qo'shing — kb-core ularning barchasidan tushuncha va aloqalarni chiqarib, bitta grafga birlashtiradi. Videolar Whisper yordamida mahalliy ravishda transkripsiya qilinadi, sizning korpusingizdan olingan domen-maxsus so'rov bilan. tree-sitter AST orqali 25 ta dasturlash tilini qo'llab-quvvatlaydi (Python, JS, TS, Go, Rust, Java, C, C++, Ruby, C#, Kotlin, Scala, PHP, Swift, Lua, Zig, PowerShell, Elixir, Objective-C, Julia, Verilog, SystemVerilog, Vue, Svelte, Dart).

> Andrej Karpati maqolalar, tvitlar, ekran tasvirlari va eslatmalarni saqlaydigan `/raw` papkasini yuritadi. kb-core ushbu muammoga javob — xom fayllarni o'qishga nisbatan har bir so'rov uchun **71,5 marta** kamroq token, sessiyalar orasida saqlanadi, topilgan va xulosa qilinganlar haqida halol.

```
/kb-core .                        # istalgan papka bilan ishlaydi — kod, eslatmalar, maqolalar, hammasi
```
...
Papkalarni istisno qilish uchun `.kb-coreignore` faylini qo'shing:
...
> **Rasmiy paket:** PyPI dagi paket nomi `kb-core` (`pip install kb-core` orqali o'rnatiladi). PyPI dagi boshqa `kb-core*` nomli paketlar bu loyiha bilan bog'liq emas. Yagona rasmiy repozitoriy — [safishamsi/kb-core](https://github.com/safishamsi/kb-core).
...
| Claude Code (Windows) | `kb-core install` (avto-aniqlash) yoki `kb-core install --platform windows` |
...
Eslatma: Codex ko'nikmalar uchun `/` o'rniga `$` ishlatadi, shuning uchun `$kb-core .` deb kiriting.
...
```
/kb-core                          # joriy katalog
/kb-core ./raw                    # ma'lum bir papka
/kb-core ./raw --mode deep        # INFERRED qirralarini agressivroq chiqarish
/kb-core ./raw --update           # faqat o'zgargan fayllarni qayta chiqarish
/kb-core ./raw --directed         # yo'naltirilgan graf
/kb-core ./raw --cluster-only     # mavjud grafda klasterlashni qayta ishga tushirish
/kb-core ./raw --no-viz           # HTML siz, faqat hisobot + JSON
/kb-core ./raw --obsidian         # Obsidian vault yaratish (opsional)

/kb-core add https://arxiv.org/abs/1706.03762   # maqolani olish
/kb-core add <video-url>                         # audio yuklab olish, transkripsiya qilish va qo'shish
/kb-core query "Attention ni optimallashtiruvchi bilan nima bog'laydi?"
/kb-core path "DigestAuth" "Response"
/kb-core explain "SwinTransformer"

kb-core hook install              # Git ilgaklarini o'rnatish
kb-core update ./src              # kod fayllarini qayta chiqarish, LLM siz
kb-core watch ./src               # grafni avtomatik yangilash
```
...
**Git ilgaklari** (`kb-core hook install`) — post-commit va post-checkout ilgaklarini o'rnatadi.

## Maxfiylik

kb-core hujjatlar, maqolalar va tasvirlardan semantik chiqarish uchun fayl mazmunini AI yordamchingizning model API siga yuboradi. Kod fayllari tree-sitter AST orqali mahalliy ravishda qayta ishlanadi. Video va audio fayllar faster-whisper bilan mahalliy ravishda transkripsiya qilinadi. Hech qanday telemetriya, foydalanishni kuzatish yo'q.
...
## kb-core ustida qurilgan — Penpax

[**Penpax**](https://safishamsi.github.io/penpax.ai) — kb-core ustidagi korporativ qatlam. kb-core fayllar papkasini bilim grafiga aylantirganidek, Penpax o'sha yondashuvni butun ish hayotingizga uzluksiz qo'llaydi.
