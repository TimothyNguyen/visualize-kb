...
**Une compétence pour assistant de code IA.** Tapez `/kb-core` dans Claude Code, Codex, OpenCode, Cursor, Gemini CLI, GitHub Copilot CLI, VS Code Copilot Chat, Aider, OpenClaw, Factory Droid, Trae, Hermes, Kiro ou Google Antigravity — il lit vos fichiers, construit un graphe de connaissances et vous révèle une structure que vous ne voyiez pas auparavant. Comprenez une base de code plus rapidement. Trouvez le « pourquoi » derrière les décisions architecturales.

Entièrement multimodal. Déposez du code, des PDFs, du markdown, des captures d'écran, des diagrammes, des photos de tableau blanc, des images dans d'autres langues, ou des fichiers vidéo et audio — kb-core extrait les concepts et les relations de tout cela et les connecte en un seul graphe. Les vidéos sont transcrites localement avec Whisper grâce à un prompt adapté au domaine. 25 langages de programmation supportés via tree-sitter AST (Python, JS, TS, Go, Rust, Java, C, C++, Ruby, C#, Kotlin, Scala, PHP, Swift, Lua, Zig, PowerShell, Elixir, Objective-C, Julia, Verilog, SystemVerilog, Vue, Svelte, Dart).

> Andrej Karpathy maintient un dossier `/raw` où il dépose des articles, tweets, captures d'écran et notes. kb-core est la réponse à ce problème — 71,5 fois moins de tokens par requête versus la lecture des fichiers bruts, persistant entre les sessions, honnête sur ce qui a été trouvé versus déduit.

```
/kb-core .                        # fonctionne sur n'importe quel dossier — code, notes, articles, tout
```
...
Ajoutez un fichier `.kb-coreignore` pour exclure des dossiers :
...
> **Package officiel :** Le package PyPI s'appelle `kb-core` (installer avec `pip install kb-core`). Les autres packages nommés `kb-core*` sur PyPI ne sont pas affiliés à ce projet. Le seul dépôt officiel est [safishamsi/kb-core](https://github.com/safishamsi/kb-core).
...
| Claude Code (Windows) | `kb-core install` (détection automatique) ou `kb-core install --platform windows` |
...
Note : Codex utilise `$` au lieu de `/` pour les compétences, tapez donc `$kb-core .`.
...
```
/kb-core                          # répertoire courant
/kb-core ./raw                    # dossier spécifique
/kb-core ./raw --mode deep        # extraction d'arêtes INFERRED plus agressive
/kb-core ./raw --update           # ne réextraire que les fichiers modifiés
/kb-core ./raw --directed         # graphe dirigé
/kb-core ./raw --cluster-only     # relancer le clustering sur le graphe existant
/kb-core ./raw --no-viz           # pas d'HTML, juste rapport + JSON
/kb-core ./raw --obsidian         # générer un vault Obsidian (opt-in)

/kb-core add https://arxiv.org/abs/1706.03762   # récupérer un article
/kb-core add <video-url>                         # télécharger l'audio, transcrire, ajouter
/kb-core query "qu'est-ce qui connecte Attention à l'optimiseur ?"
/kb-core path "DigestAuth" "Response"
/kb-core explain "SwinTransformer"

kb-core hook install              # installer les hooks Git
kb-core update ./src              # réextraire les fichiers de code, sans LLM
kb-core watch ./src               # mise à jour automatique du graphe
```
...
**Hooks Git** (`kb-core hook install`) — installe des hooks post-commit et post-checkout.

## Confidentialité

kb-core envoie le contenu des fichiers à l'API du modèle de votre assistant IA pour l'extraction sémantique des docs, articles et images. Les fichiers de code sont traités localement via tree-sitter AST. Les fichiers vidéo et audio sont transcrits localement avec faster-whisper. Aucune télémétrie, aucun suivi d'utilisation.
...
## Construit sur kb-core — Penpax

[**Penpax**](https://safishamsi.github.io/penpax.ai) est la couche enterprise au-dessus de kb_core. Là où kb-core transforme un dossier de fichiers en graphe de connaissances, Penpax applique le même graphe à toute votre vie professionnelle — en continu.
