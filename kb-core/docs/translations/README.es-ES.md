...
**Una habilidad para asistentes de código IA.** Escribe `/kb-core` en Claude Code, Codex, OpenCode, Cursor, Gemini CLI, GitHub Copilot CLI, VS Code Copilot Chat, Aider, OpenClaw, Factory Droid, Trae, Hermes, Kiro o Google Antigravity — lee tus archivos, construye un grafo de conocimiento y te devuelve estructura que no sabías que existía. Entiende una base de código más rápido. Encuentra el «por qué» detrás de las decisiones arquitectónicas.

Totalmente multimodal. Deposita código, PDFs, markdown, capturas de pantalla, diagramas, fotos de pizarras, imágenes en otros idiomas, o archivos de video y audio — kb-core extrae conceptos y relaciones de todo ello y los conecta en un solo grafo. Los videos se transcriben localmente con Whisper usando un prompt adaptado al dominio derivado de tu corpus. 25 lenguajes de programación soportados mediante tree-sitter AST (Python, JS, TS, Go, Rust, Java, C, C++, Ruby, C#, Kotlin, Scala, PHP, Swift, Lua, Zig, PowerShell, Elixir, Objective-C, Julia, Verilog, SystemVerilog, Vue, Svelte, Dart).

> Andrej Karpathy mantiene una carpeta `/raw` donde deposita papers, tweets, capturas de pantalla y notas. kb-core es la respuesta a ese problema — 71,5 veces menos tokens por consulta versus leer los archivos sin procesar, persistente entre sesiones, honesto sobre lo que encontró versus lo que infirió.

```
/kb-core .                        # funciona con cualquier carpeta — tu código, notas, papers, todo
```
...
Añade un archivo `.kb-coreignore` para excluir carpetas:
...
> **Paquete oficial:** El paquete PyPI se llama `kb-core` (instalar con `pip install kb-core`). Otros paquetes llamados `kb-core*` en PyPI no están afiliados con este proyecto. El único repositorio oficial es [safishamsi/kb-core](https://github.com/safishamsi/kb-core).
...
| Claude Code (Windows) | `kb-core install` (detección automática) o `kb-core install --platform windows` |
...
Nota: Codex usa `$` en lugar de `/` para habilidades, así que escribe `$kb-core .`.
...
```
/kb-core                          # directorio actual
/kb-core ./raw                    # carpeta específica
/kb-core ./raw --mode deep        # extracción de aristas INFERRED más agresiva
/kb-core ./raw --update           # re-extraer solo archivos modificados
/kb-core ./raw --directed         # grafo dirigido
/kb-core ./raw --cluster-only     # re-ejecutar clustering en grafo existente
/kb-core ./raw --no-viz           # sin HTML, solo informe + JSON
/kb-core ./raw --obsidian         # generar vault de Obsidian (opt-in)

/kb-core add https://arxiv.org/abs/1706.03762   # obtener un paper
/kb-core add <video-url>                         # descargar audio, transcribir, añadir
/kb-core query "¿qué conecta Attention con el optimizador?"
/kb-core path "DigestAuth" "Response"
/kb-core explain "SwinTransformer"

kb-core hook install              # instalar hooks de Git
kb-core update ./src              # re-extraer archivos de código, sin LLM
kb-core watch ./src               # actualización automática del grafo
```
...
**Hooks de Git** (`kb-core hook install`) — instala hooks post-commit y post-checkout.

## Privacidad

kb-core envía contenido de archivos a la API del modelo de tu asistente IA para extracción semántica de documentos, papers e imágenes. Los archivos de código se procesan localmente mediante tree-sitter AST. Los archivos de video y audio se transcriben localmente con faster-whisper. Sin telemetría, sin seguimiento de uso.
...
## Construido sobre kb-core — Penpax

[**Penpax**](https://safishamsi.github.io/penpax.ai) es la capa enterprise sobre kb_core. Donde kb-core convierte una carpeta de archivos en un grafo de conocimiento, Penpax aplica el mismo grafo a toda tu vida laboral — continuamente.
